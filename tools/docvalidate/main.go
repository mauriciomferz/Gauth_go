...
func main() {
    ...
    for _, r := range results {
        if len(r.Errors) > 0 { exitCode = 1; break }
        if *strict && r.DuplicateFM { exitCode = 2 }
    }
    ...
    _ = filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
        ...
        if d.IsDir() {
            ...
            if path != root { // allow root '.'
                if strings.HasPrefix(base, ".") || base == "node_modules" || base == "bin" || base == "build" { return fs.SkipDir }
            }
            return nil
        }
        ...
        if res.Meta != nil {
            if cat := res.Meta["category"]; cat != "" { categories[cat]++ }
        }
...
func normalizeStatus(s string) (canonical string, changed bool) {
    ...
    if v, ok := m[strings.ToLower(s)]; ok { return v, true }
    return s, false
}
...
func init() {
    ...
    for _, c := range expandedCategories { allowedCategories[c] = struct{}{} }
}

func validateFile(path string) fileResult {
    content, err := os.ReadFile(path)
    if err != nil { return fileResult{Path: path, Errors: []string{fmt.Sprintf("read error: %v", err)}} }
    lines := strings.Split(string(content), "\n")
    res := fileResult{Path: path}
    if len(lines) == 0 { res.Errors = append(res.Errors, "empty file"); return res }
    ...
    for i := 1; i < len(lines); i++ {
        if fmStartRe.MatchString(lines[i]) { end = i; break }
    }
    if end == -1 {
    ...
    for _, k := range requiredKeys {
        if mb[k] == "" { res.Errors = append(res.Errors, fmt.Sprintf("missing required key '%s'", k)) }
    }
    ...
    if owners := mb["owners"]; owners != "" {
        ...
        for _, p := range parts {
            ...
            if p == "" { res.Errors = append(res.Errors, "owners list contains empty entry") }
        }
    }
    ...
    for i := end + 1; i < len(lines) && i < end+50; i++ { // limit scan window
        ...
        if strings.HasPrefix(strings.ToLower(ln), "title:") { dup = true; break }
        if strings.HasPrefix(ln, "#") { break }
    }
    if dup { res.DuplicateFM = true; res.Errors = append(res.Errors, "duplicate front matter detected (second title: before heading)") }
    return res
}
...
func printSummary(results []fileResult, categories map[string]int) {
    var errCount int
    for _, r := range results { if len(r.Errors) > 0 { errCount++ } }
    ...
    if errCount > 0 {
        ...
        for _, r := range results {
            ...
            fmt.Printf("- %s\n", r.Path)
            for _, e := range r.Errors { fmt.Printf("    • %s\n", e) }
        }
    } else {
    ...
    for _, r := range results { warningCount += len(r.Warnings) }
    if warningCount > 0 {
        fmt.Println("\n⚠️  Warnings (non-fatal):")
        for _, r := range results {
            if len(r.Warnings) == 0 { continue }
            fmt.Printf("- %s\n", r.Path)
            for _, w := range r.Warnings { fmt.Printf("    • %s\n", w) }
        }
    }
    // Category histogram
...
func writeTaxonomyIndex(categories map[string]int) error {
    if len(categories) == 0 { return errors.New("no categories to write") }
    keys := make([]string, 0, len(categories))
    for k := range categories { keys = append(keys, k) }
    sort.Strings(keys)
    ...
    for _, k := range keys { b.WriteString(fmt.Sprintf("| %s | %d |\n", k, categories[k])) }
    ...
    defer f.Close()
    w := bufio.NewWriter(f)
    if _, err := w.WriteString(b.String()); err != nil { return err }
    return w.Flush()
}