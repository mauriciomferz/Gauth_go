...
func TestAttestationExternalRedisBackend(t *testing.T) {
    ...
    if seen { t.Fatalf("expected nonce unseen initially") }
    if err := backend.Record(nonce); err != nil { t.Fatalf("record failed: %v", err) }
    seen2, _ := backend.Seen(nonce)
    if !seen2 { t.Fatalf("expected nonce seen after record") }
}
