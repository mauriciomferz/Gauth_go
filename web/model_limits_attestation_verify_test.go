...
func TestModelLimitsAttestationVerify(t *testing.T) {
    ...
    srv := NewBetaServer("")
    if internalCrypto.GlobalEdDSARegistry == nil || internalCrypto.GlobalEdDSARegistry.Active() == nil { t.Fatalf("eddsa registry inactive") }
    ...
    if w2.Code != 200 { t.Fatalf("attestation status=%d", w2.Code) }
    var att map[string]any
    if err := json.Unmarshal(w2.Body.Bytes(), &att); err != nil { t.Fatalf("unmarshal att: %v", err) }
    sig := att["signature"].(string)
    kid := att["sig_kid"].(string)
    if sig == "" || kid == "" { t.Fatalf("signature fields missing") }
    ...
    if w3.Code != 200 { t.Fatalf("verify status=%d", w3.Code) }
    var vresp struct { Success bool `json:"success"`; Valid bool `json:"valid"`; Kid string `json:"kid"`; Combined string `json:"combined_hash"` }
    if err := json.Unmarshal(w3.Body.Bytes(), &vresp); err != nil { t.Fatalf("unmarshal verify: %v body=%s", err, w3.Body.String()) }
    if !vresp.Success || !vresp.Valid || vresp.Kid != kid || vresp.Combined == "" { t.Fatalf("verify response invalid %+v", vresp) }
    ...
    if err := json.Unmarshal(attBytes, &attStruct); err != nil { t.Fatalf("unmarshal struct: %v", err) }
    ...
    var vresp2 struct { Success bool `json:"success"`; Valid bool `json:"valid"`; Error string `json:"error"` }
    if err := json.Unmarshal(w4.Body.Bytes(), &vresp2); err != nil { t.Fatalf("unmarshal tampered verify: %v body=%s", err, w4.Body.String()) }
    if !vresp2.Success || vresp2.Valid { t.Fatalf("expected invalid signature: %+v", vresp2) }
}
...
func TestModelLimitsAttestationKeys(t *testing.T) {
    ...
    var resp struct { Success bool `json:"success"`; Keys []struct { Kid string `json:"kid"`; PublicB64 string `json:"public_b64"` } `json:"keys"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal keys: %v", err) }
    if !resp.Success || len(resp.Keys) == 0 || resp.Keys[0].Kid == "" || resp.Keys[0].PublicB64 == "" { t.Fatalf("keys response invalid %+v", resp) }
    // Basic public key size check
    pb, err := base64.RawStdEncoding.DecodeString(resp.Keys[0].PublicB64)
    if err != nil || len(pb) != ed25519.PublicKeySize { t.Fatalf("public key decode/size invalid") }
}
