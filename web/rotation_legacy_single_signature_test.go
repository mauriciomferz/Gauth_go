...
func TestRotationSummary_LegacySingleSignature(t *testing.T) {
	...
	if err := notary.SignRotationDescriptor(pk1, pk2, r1); err != nil {
	...
	if resp.Summary.Kid == "" || resp.Summary.Signature == "" {
		t.Fatalf("expected single signature fields populated")
	}
	if resp.Summary.Threshold != 0 {
		t.Fatalf("threshold should be zero in legacy path")
	}
	if resp.Summary.SatisfiedWeight != 1 {
		t.Fatalf("satisfied_weight should be 1 for single-sign path")
	}
	if len(resp.Summary.Signatures) != 1 {
		t.Fatalf("signatures array should contain exactly one entry in legacy path")
	}
}
