...
```
http://localhost:8080/agentauth1.html
```
...
export AGENTAUTH_WEB_PORT=8080

# Enable profiling (optional)
export AGENTAUTH_ENABLE_PPROF=1
export AGENTAUTH_PPROF_PORT=6060
```
...
AGENTAUTH_WEB_PORT=9000 ./bin/web-server
```
...
go test ./pkg/agentauth ./pkg/verification ./pkg/registry ./pkg/pip
...
```
web/static_ui/
├── agentauth1.html    # Main dashboard HTML
├── agentauth1.css     # Styles with light/dark themes
└── agentauth1.js      # Interactive JavaScript application
```

### Adding New Features
1. Add HTML markup in `agentauth1.html`
2. Style with CSS in `agentauth1.css`
3. Add interactivity in `agentauth1.js`
...
- `POST /api/v1/agentauth/tokens/create` - Create extended token
- `POST /api/v1/agentauth/tokens/validate` - Validate token
- `POST /api/v1/agentauth/pvp/verify` - Verify identity chain
- `POST /api/v1/agentauth/registry/verify-entity` - Verify entity registration
