...
func TestPolicyManifestDeterministic(t *testing.T) {
    ...
    if json.Unmarshal(w.Body.Bytes(), &body1) != nil { t.Fatalf("json1 decode fail") }
    hash1, ok := body1["manifest_hash"].(string); if !ok || hash1 == "" { t.Fatalf("missing manifest_hash") }
    sig1, ok := body1["signature"].(string); if !ok || sig1 == "" { t.Fatalf("missing signature") }
    etag := w.Header().Get("ETag"); if etag == "" { t.Fatalf("missing etag header") }
    ...
    if json.Unmarshal(w3.Body.Bytes(), &body3) != nil { t.Fatalf("json3 decode fail") }
    if body3["manifest_hash"] != hash1 { t.Fatalf("hash mismatch %s vs %s", body3["manifest_hash"], hash1) }
    if body3["signature"] != sig1 { t.Fatalf("signature changed unexpectedly") }
}
...
func TestPolicyManifestSignatureVerifyAndTamper(t *testing.T) {
    ...
    if json.Unmarshal(w.Body.Bytes(), &payload) != nil { t.Fatalf("json decode fail") }
    ...
    for _, c := range capsRaw {
        ...
        if v, ok := cm["deprecated_after"].(string); ok && v != "" { capEntry.DeprecatedAfter = v }
        if v, ok := cm["sunset_after"].(string); ok && v != "" { capEntry.SunsetAfter = v }
        if v, ok := cm["versions"].([]any); ok && len(v) > 0 {
            vers := make([]string, 0, len(v))
            for _, vv := range v { vers = append(vers, vv.(string)) }
    ...
    for _, a := range actsRaw {
        ...
        if rr, ok := am["required"].([]any); ok {
            for _, r := range rr { reqArr = append(reqArr, r.(string)) }
        }
    ...
    if recomputedHash != hashStr { t.Fatalf("manifest_hash mismatch recomputed=%s stored=%s", recomputedHash, hashStr) }
    ...
    if !ed25519.Verify(key.Public, msg, sigBytes) { t.Fatalf("signature verify failed") }
    // Tamper capability version and expect verification failure
    if len(mcaps) == 0 { t.Fatalf("no capabilities to tamper") }
    ...
    if ed25519.Verify(key.Public, msgTamper, sigBytes) { t.Fatalf("tampered signature unexpectedly valid") }
}
...
func TestPolicyManifestSigningUnavailable(t *testing.T) {
    ...
    r := httptest.NewRequest("GET", "/api/v1/policy/manifest", nil)
    srv.router.ServeHTTP(w, r)
    if w.Code != 500 { t.Fatalf("expected 500 got %d body=%s", w.Code, w.Body.String()) }
    var errBody map[string]any
    if json.Unmarshal(w.Body.Bytes(), &errBody) != nil { t.Fatalf("error body json decode failed") }
    if v := errBody["code"]; v != "signing_unavailable" { t.Fatalf("unexpected code %v", v) }
    if v := errBody["error"]; v != "signing_unavailable" { t.Fatalf("unexpected error legacy field %v", v) }
    if ok, _ := errBody["success"].(bool); ok { t.Fatalf("success field should be false") }
}
...
func TestPolicyManifestETagMismatch(t *testing.T) {
    ...
    if w1.Code != 200 { t.Fatalf("status1 %d", w1.Code) }
    realETag := w1.Header().Get("ETag")
    if realETag == "" { t.Fatalf("missing real etag") }
    // Second request with mismatching If-None-Match
    w2 := httptest.NewRecorder()
    r2 := httptest.NewRequest("GET", "/api/v1/policy/manifest", nil)
    r2.Header.Set("If-None-Match", "W/\"deadbeef\"")
    srv.router.ServeHTTP(w2, r2)
    if w2.Code != 200 { t.Fatalf("expected 200 for mismatching etag got %d", w2.Code) }
    if w2.Header().Get("ETag") != realETag { t.Fatalf("etag changed unexpectedly mismatch scenario") }
    if len(w2.Body.Bytes()) == 0 { t.Fatalf("expected body on mismatching etag") }

    // Metrics counter should increment on each successful 200 emission
    mem, ok := srv.metrics.(*metrics.Memory)
    if !ok { t.Fatalf("metrics type assertion failed") }
    if mem.PolicyManifestEmitted() < 2 { t.Fatalf("expected at least 2 emissions counter=%d", mem.PolicyManifestEmitted()) }
}
...
func TestPolicyManifestSignerInterface(t *testing.T) {
    ...
    if err != nil { t.Fatalf("buildPolicyManifest error: %v", err) }
    if len(canon.Capabilities) == 0 { t.Fatalf("expected capabilities in canonical") }
    active := cryptopkg.GlobalEdDSARegistry.Active()
    if active == nil || len(active.Private) != ed25519.PrivateKeySize { t.Fatalf("active key missing") }
    msg := append([]byte("GAUTH_POLICY_MANIFEST:"), raw...)
    sigDirect := ed25519.Sign(active.Private, msg)
    signer := cryptopkg.GlobalRotatingSigner()
    if signer == nil { t.Fatalf("global rotating signer nil") }
    sigIface, err := signer.Sign(msg)
    if err != nil { t.Fatalf("signer.Sign error: %v", err) }
