...
func TestModelValidateLimits(t *testing.T) {
	...
	if err != nil { t.Fatalf("temp file err=%v", err) }
	jsonData := []byte(`{"model_limits":{"demo-model":{"max_input_tokens":1024}}}`)
	if _, err := f.Write(jsonData); err != nil { t.Fatalf("write err=%v", err) }
	f.Close()
	...
	if resp.Code != 200 { t.Fatalf("expected 200 unknown model got %d body=%s", resp.Code, resp.Body.String()) }
	// Allowed within limit
	resp = postModelValidate(bs, map[string]any{"model_id":"demo-model","input_tokens":100})
	if resp.Code != 200 { t.Fatalf("expected 200 within limit body=%s", resp.Body.String()) }
	// Over limit
