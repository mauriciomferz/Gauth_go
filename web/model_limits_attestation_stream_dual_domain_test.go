...
func TestModelLimitsAttestationStreamDualDomainSignature(t *testing.T) {
    ...
    if err := json.Unmarshal([]byte(attJSON), &att); err != nil { t.Fatalf("unmarshal: %v", err) }
    if att.Signature == "" || att.DomainSignature == "" || att.DomainPrefix != "EXTRA_ATTEST:" { t.Fatalf("missing domain signature fields: %+v", att) }
