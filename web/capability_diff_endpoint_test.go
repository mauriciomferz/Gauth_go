...
func TestCapabilityDiffAddedRemovedModified(t *testing.T) {
    ...
    if w.Code != 200 { t.Fatalf("unexpected status baseline: %d body=%s", w.Code, w.Body.String()) }
    ...
    if err := json.Unmarshal(w.Body.Bytes(), &baselineResp); err != nil { t.Fatalf("unmarshal baseline: %v", err) }
    ...
    if w2.Code != 200 { t.Fatalf("unexpected status mutated: %d body=%s", w2.Code, w2.Body.String()) }
    var diffResp struct{
        Added []struct{ ID string `json:"id"` } `json:"added"`
        Removed []struct{ ID string `json:"id"` } `json:"removed"`
        Modified []struct{ ID string `json:"id"` } `json:"modified"`
    }
    if err := json.Unmarshal(w2.Body.Bytes(), &diffResp); err != nil { t.Fatalf("unmarshal diff: %v", err) }
    // Assertions
    if len(diffResp.Added) != 1 || diffResp.Added[0].ID != "cap.C" { t.Fatalf("expected added cap.C; got %#v", diffResp.Added) }
    if len(diffResp.Removed) != 1 || diffResp.Removed[0].ID != "cap.B" { t.Fatalf("expected removed cap.B; got %#v", diffResp.Removed) }
    foundModified := false
    for _, m := range diffResp.Modified { if m.ID == "cap.A" { foundModified = true } }
    if !foundModified { t.Fatalf("expected modified cap.A; got %#v", diffResp.Modified) }
}
...
func TestCapabilityDiffUnknownBaseline(t *testing.T) {
    ...
    if w.Code != 404 { t.Fatalf("expected 404 for unknown baseline; got %d body=%s", w.Code, w.Body.String()) }
