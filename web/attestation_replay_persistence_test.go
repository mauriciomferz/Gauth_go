...
func TestAttestationReplayPersistenceRestart(t *testing.T) {
    ...
    if store1.Seen(nonce, time.Now()) { t.Fatalf("nonce should not be seen initially") }
    store1.Record(nonce, time.Now())
    if !store1.Seen(nonce, time.Now()) { t.Fatalf("nonce should be seen after record") }
    ...
    if !store2.Seen(nonce, time.Now()) { t.Fatalf("restarted store should detect replay nonce") }
    // Ensure snapshot+compact removes expired entries but retains active.
    if err := store2.SnapshotAndCompact(); err != nil { t.Fatalf("snapshot/compact failed: %v", err) }
    // Touch WAL file to ensure it exists post-rotation.
    if _, err := os.Stat(walPath); err != nil { t.Fatalf("wal file missing after compact: %v", err) }
}
