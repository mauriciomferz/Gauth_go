...
func TestTracingEnabledEmitsSpans(t *testing.T) {
	...
	if w1.Code != 201 {
		t.Fatalf("token create status %d (expected 201)", w1.Code)
	}
	...
	if w2.Code != 200 {
		t.Fatalf("token validate status %d", w2.Code)
	}
	if srv.tracerProvider == nil {
		t.Fatalf("tracerProvider nil when tracing enabled")
	}
	ops := map[string]bool{}
	...
	if !ops["token.issue"] {
		t.Fatalf("missing token.issue span; ops=%v", ops)
	}
	if !ops["token.validate"] {
		t.Fatalf("missing token.validate span; ops=%v", ops)
	}
}
...
func TestTracingDisabledNoSpans(t *testing.T) {
	...
	if err := json.Unmarshal(w1.Body.Bytes(), &createResp); err != nil || createResp.Token.ID == "" {
		t.Fatalf("create parse failed: %v %s", err, w1.Body.String())
	}
	valBody := bytes.NewBufferString(`{"token_id":"` + createResp.Token.ID + `"}`)
	w2 := httptest.NewRecorder()
	srv.router.ServeHTTP(w2, httptest.NewRequest("POST", "/api/v1/token/validate", valBody))
	if w2.Code != 200 {
		t.Fatalf("validate status %d", w2.Code)
	}
	if srv.tracerProvider != nil {
...
func TestTracingSampleRatioZeroEmitsSpans(t *testing.T) {
	...
	if srv.tracerProvider == nil {
		t.Fatalf("tracerProvider nil when tracing enabled ratio=0")
	}
	ops := map[string]bool{}
	for _, sp := range srv.tracerProvider.Spans() {
		ops[sp.Operation] = true
	}
	if !ops["token.issue"] || !ops["token.validate"] {
		t.Fatalf("expected spans with ratio=0; ops=%v", ops)
	}
}
...
func TestTracingSampleRatioMidApproximatelySamples(t *testing.T) {
	...
	if srv.tracerProvider == nil {
		t.Fatalf("tracerProvider nil for mid-ratio test")
	}
	issueCount := 0
	validateCount := 0
	for _, sp := range srv.tracerProvider.Spans() {
	...
	if issueCount <= 5 || issueCount >= 38 {
		t.Fatalf("token.issue spans out of expected range: %d", issueCount)
	}
	if validateCount <= 5 || validateCount >= 38 {
		t.Fatalf("token.validate spans out of expected range: %d", validateCount)
	}
}
