...
func TestRotationV2Endpoint(t *testing.T) {
    ...
    if _, err := tmpFile.Write([]byte(cfgJSON)); err != nil { t.Fatalf("write cfg: %v", err) }
    tmpFile.Close()
    ...
    if w1.Code != 200 { t.Fatalf("expected 200 first call got %d body=%s", w1.Code, w1.Body.String()) }
    var resp1 struct {
    ...
    if err := json.Unmarshal(w1.Body.Bytes(), &resp1); err != nil { t.Fatalf("unmarshal resp1: %v", err) }
    if !resp1.Success { t.Fatalf("success false first response") }
    if resp1.Artifact.Version != 2 { t.Fatalf("expected version 2 got %d", resp1.Artifact.Version) }
    if resp1.Artifact.CanonicalDigest == "" { t.Fatalf("missing canonical digest") }
    ...
    if w2.Code != 200 { t.Fatalf("expected 200 second call got %d body=%s", w2.Code, w2.Body.String()) }
    var resp2 struct {
        Success        bool                            `json:"success"`
        VerifiedWeight int                             `json:"verified_weight"`
        ThresholdMet   bool                            `json:"threshold_met"`
    ...
    if err := json.Unmarshal(w2.Body.Bytes(), &resp2); err != nil { t.Fatalf("unmarshal resp2: %v", err) }
    if !resp2.Success { t.Fatalf("success false second response") }
    if resp2.VerifiedWeight < 0 { t.Fatalf("verified weight invalid: %d", resp2.VerifiedWeight) }
    ...
    if resp2.ThresholdMet && resp2.LatestHash == "" { t.Fatalf("threshold met but continuity latest hash empty") }
}
