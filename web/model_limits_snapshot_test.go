...
func TestModelLimitsSnapshotHashChange(t *testing.T) {
    ...
    getHash := func() string {
        ...
        if w.Code != 200 { t.Fatalf("snapshot status %d body=%s", w.Code, w.Body.String()) }
        var resp struct { Hash string `json:"hash"` }
        if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v body=%s", err, w.Body.String()) }
        return resp.Hash
    }
    ...
    if err := os.WriteFile(tmp.Name(), []byte(updated), 0600); err != nil { t.Fatalf("rewrite: %v", err) }
    ...
    if h1 == h2 { t.Fatalf("expected hash change after reload h1=%s h2=%s", h1, h2) }
}
