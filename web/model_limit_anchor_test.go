...
func TestModelLimitAuditAnchoring(t *testing.T) {
    ...
    exceed := func() {
        ...
        if w.Code != 400 { t.Fatalf("expected 400 exceed got %d body=%s", w.Code, w.Body.String()) }
        if !strings.Contains(w.Body.String(), "model_limit_exceeded") { t.Fatalf("unexpected body: %s", w.Body.String()) }
    }
    ...
    if err != nil { t.Fatalf("read anchor: %v", err) }
    ...
    if lineCount != 1 { t.Fatalf("expected 1 anchor entry got %d", lineCount) }
    if !strings.Contains(first, "\"audit_entries\":2") { t.Fatalf("anchor missing audit_entries=2: %s", first) }
    if !strings.Contains(first, "audit_last_hash") { t.Fatalf("anchor missing last hash field: %s", first) }

    // Verify endpoint
    vw := httptest.NewRecorder()
    req, _ := http.NewRequest(http.MethodGet, "/api/v1/model/limits/audit/anchor/verify", nil)
    bs.router.ServeHTTP(vw, req)
    if vw.Code != 200 { t.Fatalf("verify endpoint status %d body=%s", vw.Code, vw.Body.String()) }
    if !strings.Contains(vw.Body.String(), "\"valid\":true") { t.Fatalf("verify endpoint invalid: %s", vw.Body.String()) }
}
