    ...
    for _, n := range sizes {
        b.Run("size="+localItoa(n), func(b *testing.B) {
            ...
            for i := 0; i < n; i++ {
                ...
                if _, err := chain.Append(ev); err != nil { b.Fatalf("append: %v", err) }
            }
            ...
            for i := 0; i < 5; i++ {
                ...
                if _, err := chain.Append(ev); err != nil { b.Fatalf("append late: %v", err) }
            }
            _, _ = chain.SignTreeHead()
            ...
            for i := 0; i < b.N; i++ {
                ...
                if err != nil { b.Fatalf("generate proof: %v", err) }
                // Collect hashes
                hashes := make([]string, 0, chain.LatestTreeHead().ChainLength)
                for _, ev := range chain.Events() { hashes = append(hashes, ev.Hash) }
                if err := delegation.VerifyConsistencyProofV2(proof, hashes); err != nil { b.Fatalf("verify: %v", err) }
            }
        })
    }
}
