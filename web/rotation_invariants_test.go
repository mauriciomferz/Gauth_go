...
func writeContinuityGapLedger(path string) error {
    ...
    fm.Entries = append(fm.Entries, struct {
        Index int `json:"index"`; Hash string `json:"hash"`; PrevHash string `json:"prev_hash"`; Descriptor *notary.KeyRotationDescriptor `json:"descriptor"`; Timestamp string `json:"timestamp"`
...
func TestRotationSummary_ContinuityGap(t *testing.T) {
    ...
    if err := writeContinuityGapLedger(ledgerPath); err != nil { t.Fatalf("write ledger: %v", err) }
    ...
    if err := led.Load(); err != nil { t.Fatalf("load after write: %v", err) }
    ...
    var resp struct { Success bool `json:"success"`; Code string `json:"code"`; RFC string `json:"rfc"`; Detail map[string]any `json:"detail"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if resp.Code != "rotation_continuity_gap" { t.Fatalf("expected rotation_continuity_gap got %s", resp.Code) }
    if resp.Detail == nil || resp.Detail["index"] == nil { t.Fatalf("expected detail index present: %+v", resp.Detail) }
}
...
func TestRotationSummary_SignatureMissing(t *testing.T) {
    ...
    led := notary.NewRotationLedger(ledgerPath)
    if err := led.Load(); err != nil { t.Fatalf("load: %v", err) }
    ...
    if err := notary.SignRotationDescriptor(priv1, priv2, r1); err != nil { t.Fatalf("sign r1: %v", err) }
    ...
    if err := notary.SignRotationDescriptor(priv2, priv3, r2); err != nil { t.Fatalf("sign r2: %v", err) }
    ...
    var resp struct { Code string `json:"code"`; RFC string `json:"rfc"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if resp.Code != "rotation_signature_missing" { t.Fatalf("expected rotation_signature_missing got %s", resp.Code) }
}
...
func TestRotationSummary_SignatureValid(t *testing.T) {
    ...
    var resp struct { Success bool `json:"success"`; Summary struct { Signature string `json:"signature"`; Kid string `json:"kid"` } `json:"summary"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if !resp.Success || resp.Summary.Signature == "" || resp.Summary.Kid == "rot-valid" { /* kid may be derived; ensure signature decodes */ }
    if _, err := base64.RawURLEncoding.DecodeString(resp.Summary.Signature); err != nil { t.Fatalf("signature base64 decode failed: %v", err) }
}
