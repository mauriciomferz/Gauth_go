...
func TestAnchorEndpointsFullCycle(t *testing.T) {
    ...
    if wPost.Code != http.StatusOK { t.Fatalf("anchor POST status=%d body=%s", wPost.Code, wPost.Body.String()) }
    var postResp struct {
        Success bool   `json:"success"`
        Hash    string `json:"hash"`
        Total   int    `json:"total"`
    }
    if err := json.Unmarshal(wPost.Body.Bytes(), &postResp); err != nil { t.Fatalf("unmarshal post: %v", err) }
    if !postResp.Success || postResp.Hash == "" || postResp.Total < 1 { t.Fatalf("unexpected post resp %+v", postResp) }
    // Latest
    wLatest := perform(srv, http.MethodGet, "/api/v1/beta/capabilities/anchor/latest")
    if wLatest.Code != http.StatusOK { t.Fatalf("latest status=%d", wLatest.Code) }
    // Material
    ...
    if wMaterial.Code != http.StatusOK { t.Fatalf("material status=%d body=%s", wMaterial.Code, wMaterial.Body.String()) }
    var matResp struct {
        Success    bool   `json:"success"`
    ...
    if err := json.Unmarshal(wMaterial.Body.Bytes(), &matResp); err != nil { t.Fatalf("unmarshal material: %v body=%s", err, wMaterial.Body.String()) }
    if !matResp.Success || !matResp.Configured || !matResp.Emitted || matResp.Registry == "" { t.Fatalf("unexpected material resp %+v", matResp) }
    // Status
    wStatus := perform(srv, http.MethodGet, "/api/v1/beta/capabilities/anchor/status")
    if wStatus.Code != http.StatusOK { t.Fatalf("status endpoint code=%d body=%s", wStatus.Code, wStatus.Body.String()) }
    var stResp struct { Success bool `json:"success"`; Registry string `json:"registry_hash"`; Configured bool `json:"configured"` }
    if err := json.Unmarshal(wStatus.Body.Bytes(), &stResp); err != nil { t.Fatalf("unmarshal status: %v", err) }
    if !stResp.Success || stResp.Registry == "" || !stResp.Configured { t.Fatalf("unexpected status resp %+v", stResp) }
}
...
func TestAnchorPostDisabled(t *testing.T) {
    ...
    var resp struct { Success bool `json:"success"`; Code string `json:"code"`; Error string `json:"error"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if resp.Success || resp.Code != "anchoring_disabled" || resp.Error != "capability_anchor_disabled" { t.Fatalf("unexpected error payload %+v", resp) }
}
