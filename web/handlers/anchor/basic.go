...
func postAnchorHandler(c *gin.Context, d Deps) {
    ...
    if prev != "" { payload["previous_hash"] = prev }
    chAt := d.CapabilityRegistryChangeAt()
    if !chAt.IsZero() { payload["registry_last_changed_at"] = chAt.Format(time.RFC3339) }
...
func statusHandler(c *gin.Context, d Deps) {
    ...
    lastWrite := ""
    if !d.CapAnchorLastWrite().IsZero() { lastWrite = d.CapAnchorLastWrite().UTC().Format(time.RFC3339Nano) }
    payload := gin.H{
        "success":                 true,
    ...
    if e, s, h, lw, ok := d.CapAnchorMetrics(); ok {
        ...
        if lw > 0 { payload["last_write_unix"] = lw }
    }
