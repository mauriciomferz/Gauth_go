...
type stubStore struct { entries []anchorint.StoredExternalAnchorReceipt }
func (s *stubStore) Latest() anchorint.StoredExternalAnchorReceipt {
    if len(s.entries) == 0 { return anchorint.StoredExternalAnchorReceipt{} }
    return s.entries[len(s.entries)-1]
}
func (s *stubStore) Entries() []anchorint.StoredExternalAnchorReceipt { return s.entries }
type depsImpl struct { store *stubStore; integrity string; lastVerifyAge uint64 }
...
func (d *depsImpl) ExternalReceiptStore() any {
    if d.store == nil { return nil }
    return d.store
}
func (d *depsImpl) ExternalReceiptStoreLatest() anchorint.StoredExternalAnchorReceipt { if d.store==nil { return anchorint.StoredExternalAnchorReceipt{} }; return d.store.Latest() }
func (d *depsImpl) ExternalReceiptStoreEntries() []anchorint.StoredExternalAnchorReceipt { if d.store==nil { return []anchorint.StoredExternalAnchorReceipt{} }; return d.store.Entries() }
...
func TestExternalReceiptsLatestUnconfigured(t *testing.T) {
    ...
    var resp struct { Success bool `json:"success"`; Configured bool `json:"configured"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if !resp.Success || resp.Configured { t.Fatalf("expected configured=false got %+v", resp) }
}
...
func TestExternalReceiptsVerifyEmpty(t *testing.T) {
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("status %d body=%s", w.Code, w.Body.String()) }
    var resp struct { Success bool `json:"success"`; Configured bool `json:"configured"`; Integrity string `json:"integrity"`; Total int `json:"total"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if !resp.Success || !resp.Configured || resp.Integrity != "empty" || resp.Total != 0 { t.Fatalf("unexpected resp %+v", resp) }
}
...
func TestExternalReceiptsVerifyOK(t *testing.T) {
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("status %d body=%s", w.Code, w.Body.String()) }
    var resp struct { Success bool `json:"success"`; Configured bool `json:"configured"`; Integrity string `json:"integrity"`; Total int `json:"total"`; ChainHead string `json:"chain_head"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if !resp.Success || !resp.Configured || resp.Integrity != "ok" || resp.Total != 2 || resp.ChainHead == "" { t.Fatalf("unexpected resp %+v", resp) }
}
...
func TestExternalReceiptsVerifyMismatch(t *testing.T) {
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("status %d body=%s", w.Code, w.Body.String()) }
    var resp struct { Success bool `json:"success"`; Configured bool `json:"configured"`; Integrity string `json:"integrity"`; Total int `json:"total"`; Details map[string]any `json:"details"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if !resp.Success || !resp.Configured || resp.Integrity != "mismatch" || resp.Total != 2 || resp.Details == nil { t.Fatalf("unexpected resp %+v", resp) }
}
...
func TestExternalReceiptsChainListing(t *testing.T) {
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("status %d body=%s", w.Code, w.Body.String()) }
    var resp struct { Success bool `json:"success"`; Configured bool `json:"configured"`; Total int `json:"total"`; Entries []map[string]any `json:"entries"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if !resp.Success || !resp.Configured || resp.Total != 2 || len(resp.Entries) != 2 { t.Fatalf("unexpected resp %+v", resp) }
}
...
func TestExternalReceiptsLatestEmpty(t *testing.T) {
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("status %d body=%s", w.Code, w.Body.String()) }
    var resp struct { Success bool `json:"success"`; Configured bool `json:"configured"`; Empty bool `json:"empty"` }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if !resp.Success || !resp.Configured || !resp.Empty { t.Fatalf("unexpected resp %+v", resp) }
}
