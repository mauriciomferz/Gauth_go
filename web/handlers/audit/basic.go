...
func verifyHandler(c *gin.Context, d Deps) {
	...
	if err != nil {
		respondError(c, http.StatusInternalServerError,
			"capabilities_audit_read_failed", "read_failed",
			"failed reading capability audit file",
			"rfc111:capabilities_audit_verify", nil)
		return
	}
	var wrapper struct {
	...
	if jerr := json.Unmarshal(b, &wrapper); jerr != nil {
		respondError(c, http.StatusInternalServerError,
			"capabilities_audit_invalid_json", "invalid_json",
			"capability audit wrapper invalid JSON",
			"rfc111:capabilities_audit_verify", nil)
		return
	}
...
func anchorHandler(c *gin.Context, d Deps) {
	if os.Getenv("AGENTAUTH_CAPABILITY_ANCHOR_ENABLE") != "1" {
		respondError(c, http.StatusForbidden, "capability_anchor_disabled",
			"anchoring_disabled", "capability anchoring disabled",
			"rfc111:capability_anchor", nil)
		return
	}
	client := d.AnchorClient()
	if client == nil {
		respondError(c, http.StatusInternalServerError,
			"capability_anchor_client_unavailable", "anchor_client_unavailable",
			"anchor client unavailable", "rfc111:capability_anchor", nil)
		return
	}
	tip := d.CapAuditPrevHash()
	if tip == "" {
		respondError(c, http.StatusBadRequest,
			"capabilities_audit_chain_tip_empty", "chain_tip_empty",
			"capability audit chain tip empty",
			"rfc111:capabilities_audit_anchor", nil)
		return
	}
	rec, err := client.Anchor(tip)
	if err != nil {
		respondError(c, http.StatusInternalServerError,
			"capabilities_audit_anchor_failure", "anchor_failure",
			"failed anchoring capability audit chain tip",
			"rfc111:capabilities_audit_anchor", err.Error())
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"success":     true,
		"hash":        rec.Hash,
		"anchored_at": rec.AnchoredAt.UTC().Format(time.RFC3339),
