...
func TestCapabilitiesListBasic(t *testing.T) {
    ...
    if len(resp.Capabilities) == 0 { t.Fatalf("expected >=1 capability") }
    // Basic shape assertions
    first := resp.Capabilities[0]
    if _, ok := first["id"]; !ok { t.Fatalf("missing id field") }
    if _, ok := first["version"]; !ok { t.Fatalf("missing version field") }
}
...
func TestCapabilitiesNegotiateHappy(t *testing.T) {
    ...
    if !resp.Success { t.Fatalf("success false") }
    if resp.Agreed[cid] != ver { t.Fatalf("expected agreed %s=%s got %v", cid, ver, resp.Agreed) }
    if _, ok := resp.Unsupported["unknown.cap"]; !ok { t.Fatalf("expected unknown.cap unsupported list present") }
}
...
func TestCapabilitiesNegotiateInvalidPayload(t *testing.T) {
    ...
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if resp.Success { t.Fatalf("expected success=false") }
    if resp.Code != "capabilities_negotiate_invalid_payload" { t.Fatalf("unexpected code %s", resp.Code) }
    if resp.Error != "invalid_payload" { t.Fatalf("unexpected error %s", resp.Error) }
}
