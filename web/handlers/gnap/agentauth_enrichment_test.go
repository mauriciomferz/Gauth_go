...
type mockVerifService struct {
	agentauthplus.VerificationService
...
func TestHandler_LinkAgentAuthContext_Enrichment(t *testing.T) {
	...
	report := &agentauthplus.VerificationReport{
		PoAID: "poa-123",
		PoAVerification: &agentauthplus.VerificationResult{
			IssuerID:  "issuer-001",
			GranteeID: "grantee-001",
			Scope:     []string{"read"},
		},
		ChainOfAuthority: []agentauthplus.AuthorityLink{
			{
				GranteeID: "grantee-001",
				IssuerID:  "issuer-001",
		...
		OverallValid: true,
		FiduciaryCompliance: &agentauthplus.FiduciaryComplianceCheck{
			Compliant: true,
		},
		CapabilityCheck: &agentauthplus.CapabilityVerificationCheck{
			Sufficient: true,
		},
	}
