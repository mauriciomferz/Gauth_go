...
func TestModelLimitsAttestation(t *testing.T) {
	...
	limitsFile.Write([]byte(`{"model_limits":{"demo":{"max_input_tokens":10}}}`))
	limitsFile.Close()
	...
	for i := 0; i < 2; i++ {
		...
		if w.Code != 400 { t.Fatalf("expected exceed 400 got %d", w.Code) }
	}
	...
	if w2.Code != 200 { t.Fatalf("attestation status=%d body=%s", w2.Code, w2.Body.String()) }
	var resp struct {
	...
	if err := json.Unmarshal(w2.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
	if !resp.Success || !resp.Configured { t.Fatalf("expected success+configured got %+v", resp) }
	if resp.Snapshot["hash"] == "" { t.Fatalf("empty snapshot hash") }
	if resp.Audit["head_hash"] == "" { t.Fatalf("empty audit head hash") }
	if resp.Anchor["latest_hash"] == "" { t.Fatalf("empty anchor latest hash") }
	if resp.StrictUnknown != true { t.Fatalf("strict_unknown flag false") }
}
