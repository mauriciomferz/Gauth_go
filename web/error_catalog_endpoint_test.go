...
func TestUIIndex(t *testing.T) {
    ...
    if !containsStr(w.Body.String(), "GAuth Beta Dashboard") { t.Fatalf("index missing content") }
}
...
func TestAttestationVerifyErrorEnvelope(t *testing.T) {
    ...
    if err := json.Unmarshal(w.Body.Bytes(), &env); err != nil { t.Fatalf("unmarshal error: %v body=%s", err, w.Body.String()) }
    if env.Code != "attestation_invalid_json" { t.Fatalf("unexpected code %s", env.Code) }
    if env.Message == "" { t.Fatalf("missing message") }
    if env.Details == nil || env.Details["http_path"] != "/api/v1/model/limits/verify" { t.Fatalf("details missing http_path: %#v", env.Details) }
}
...
func TestErrorCatalogCaching(t *testing.T) {
    ...
    if w2.Body.Len() != 0 { t.Fatalf("expected empty body for 304, got %s", w2.Body.String()) }
}
...
func TestRotationSummaryV2Deterministic(t *testing.T) {
    ...
    if err != nil { t.Fatalf("build1: %v", err) }
    ...
    if err != nil { t.Fatalf("build2: %v", err) }
...
func containsStr(haystack, needle string) bool { return len(haystack) >= len(needle) && (func() bool { return ecIndex(haystack, needle) >= 0 })() }
