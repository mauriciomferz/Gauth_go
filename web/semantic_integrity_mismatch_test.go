...
func TestSemanticIntegrityMismatch(t *testing.T) {
	...
	if first.Current == "" {
		t.Fatalf("expected non-empty current hash on first call")
	}
	// Tamper: overwrite persisted file with a different hash than prevHash that server will expect next time.
	if err := os.WriteFile(persistPath, []byte("sha256:tampered"), 0o600); err != nil {
	...
	if second.Integrity != "mismatch" {
		t.Fatalf("expected integrity_status mismatch got %s prev=%s cur=%s", second.Integrity, second.Prev, second.Current)
	}
}
