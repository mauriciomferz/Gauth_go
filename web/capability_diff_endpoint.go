...
func diffCapabilities(base, current []capability.Capability) (added, removed []simpleCapabilityMeta, modified []capabilityModified) {
    ...
    for _, c := range base { baseMap[c.ID] = c }
    currMap := make(map[string]capability.Capability, len(current))
    for _, c := range current { currMap[c.ID] = c }
    // Added & modified
...
func (s *BetaServer) registerCapabilityDiff() {
    s.router.GET("/api/v1/capabilities/diff", func(c *gin.Context) {
        ...
        if s.capDiffSnapshots != nil { s.capDiffSnapshots.Add(caps, currentHash) }
        baseHash := since
        ...
        if baseHash == "" || baseHash == currentHash {
            ...
            if s.capDiffSnapshots == nil {
                ...
                if span != nil { span.SetTag("outcome", "error"); span.SetTag("error_code", "snapshot_disabled"); span.End() }
                return
            }
            ...
            if !ok {
                ...
                if span != nil { span.SetTag("outcome", "error"); span.SetTag("error_code", "base_hash_unknown"); span.End() }
                return
            }
        ...
        if m, ok := s.metrics.(interface{ IncCapabilityDiffRequests(); ObserveCapabilityDiffLatency(d time.Duration) }); ok {
