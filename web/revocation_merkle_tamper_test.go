...
func TestRevocationMerkleProofTamperDetection(t *testing.T) {
    ...
    s := NewBetaServer("")
    if s.revocationChain == nil { s.revocationChain = delegation.NewRevocationChain() }
    ...
    for i := 0; i < 5; i++ {
        ...
        if _, err := s.revocationChain.Append(ev); err != nil { t.Fatalf("append revocation: %v", err) }
    ...
    if !resp.Success || resp.MerkleRoot == "" { t.Fatalf("malformed response: %+v", resp) }
    if len(resp.Proof) == 0 { t.Fatalf("expected non-empty proof for middle element") }
    // Verify original proof succeeds
    leafDigest := delegation.LeafDigestForEventHash(target.Hash)
    if !delegation.VerifyProof(leafDigest, resp.Proof, resp.MerkleRoot) { t.Fatalf("original proof verification failed") }
    ...
    if tampered[0].Sibling[0] == 'a' { tampered[0].Sibling = "b" + tampered[0].Sibling[1:] } else { tampered[0].Sibling = "a" + tampered[0].Sibling[1:] }
