...
import { gauthPlusAPI, DualControlApproval } from '../../lib/gauthplus-api'
...
export default function DualControlPanel() {
  ...
  const fetchPendingApprovals = async () => {
    try {
      setLoading(true)
      const response = await gauthPlusAPI.getPendingApprovals()
  ...
  const handleApprove = async (approvalId: string) => {
    try {
      await gauthPlusAPI.approveAction(approvalId, 'admin', 'Approved from UI')
  ...
  const handleReject = async (approvalId: string) => {
    try {
      await gauthPlusAPI.rejectAction(approvalId, 'admin', 'Rejected from UI')
      await fetchPendingApprovals()
    } catch (error) {
