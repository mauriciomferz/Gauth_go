...
func TestRotationSummary_MultiSignatureInvalidSignature(t *testing.T) {
    ...
    if err := notary.SignRotationDescriptor(rk1, rk2, r1); err != nil { t.Fatalf("sign r1: %v", err) }
    ...
    if err := notary.SignRotationDescriptor(rk2, rk3, r2); err != nil { t.Fatalf("sign r2: %v", err) }
    ...
    var apiResp struct { Summary *verification.RotationSummary `json:"summary"`; Success bool `json:"success"` }
    if err := json.Unmarshal(w.Body.Bytes(), &apiResp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if apiResp.Summary == nil || len(apiResp.Summary.Signatures) < 2 { t.Fatalf("expected multisig signatures >=2") }
