...
func TestRotationSummary_MultiSignatureThresholdUnsatisfied(t *testing.T) {
    ...
    var errResp struct { Code string `json:"code"`; RFC string `json:"rfc_ref"`; Detail map[string]any `json:"detail"` }
    if err := json.Unmarshal(w.Body.Bytes(), &errResp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if errResp.Code != "rotation_threshold_unsatisfied" { t.Fatalf("expected rotation_threshold_unsatisfied got %s", errResp.Code) }
}
