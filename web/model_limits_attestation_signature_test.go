...
func TestModelLimitsAttestationSignature(t *testing.T) {
    ...
    if w2.Code != 200 { t.Fatalf("status %d body=%s", w2.Code, w2.Body.String()) }
    ...
    var att attStruct
    if err := json.Unmarshal(w2.Body.Bytes(), &att); err != nil { t.Fatalf("unmarshal attestation: %v", err) }
    if att.Signature == "" || att.SigKid == "" || att.SigMode != sigModeEdDSA { t.Fatalf("signature fields missing or invalid: %+v", att) }
    if att.SigKid != kid { t.Fatalf("kid mismatch expected %s got %s", kid, att.SigKid) }
    ...
    if err != nil { t.Fatalf("marshal unsigned: %v", err) }
    if len(pub) != ed25519.PublicKeySize { t.Fatalf("public key size bad") }
    if !ed25519.Verify(pub, raw, sigBytes) { t.Fatalf("signature verify failed") }
    // Tamper check
    unsigned.Snapshot.Hash = unsigned.Snapshot.Hash + "tamper"
    tamperedRaw, _ := json.Marshal(unsigned)
    if ed25519.Verify(pub, tamperedRaw, sigBytes) { t.Fatalf("tamper should fail verification") }
}
