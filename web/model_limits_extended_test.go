...
func TestModelExtendedLimits(t *testing.T) {
	...
	if resp.Code != 200 { t.Fatalf("expected 200 within limits got %d body=%s", resp.Code, resp.Body.String()) }
	// 2. Output tokens exceed
	resp = doModelReq(bs, map[string]any{"model_id":"demo-model","input_tokens":50,"output_tokens":200})
	if resp.Code != 400 || !bytes.Contains(resp.Body.Bytes(), []byte("model_output_limit_exceeded")) { t.Fatalf("expected output exceed 400 got %d body=%s", resp.Code, resp.Body.String()) }
	...
	if resp.Code != 400 || !bytes.Contains(resp.Body.Bytes(), []byte("model_limit_exceeded")) { t.Fatalf("expected input exceed 400 got %d body=%s", resp.Code, resp.Body.String()) }
	...
	bs.modelRateMu.Lock(); bs.modelRateLimits["demo-model"] = 3; bs.modelRateMu.Unlock()
	for i:=0; i<3; i++ {
	...
	if st, ok := bs.modelRateState["demo-model"]; ok { st.WindowStart = time.Now().Add(-time.Minute); st.Count = 0; bs.modelRateState["demo-model"] = st }
	...
	if resp.Code == 429 { t.Fatalf("unexpected immediate rate limit after short wait body=%s", resp.Body.String()) }
}
