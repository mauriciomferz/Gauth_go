...
func TestTokenCreateClientNonceReplay(t *testing.T) {
    ...
    if w1.Code != 200 && w1.Code != 201 { t.Fatalf("expected 200/201 first create got %d body=%s", w1.Code, w1.Body.String()) }
    req2 := httptest.NewRequest("POST", "/api/v1/token/create", bytes.NewReader(body1))
    w2 := httptest.NewRecorder()
    s.router.ServeHTTP(w2, req2)
    if w2.Code != 409 { t.Fatalf("expected 409 replay detection got %d body=%s", w2.Code, w2.Body.String()) }
    if !bytes.Contains(w2.Body.Bytes(), []byte("nonce_reused")) { t.Fatalf("expected nonce_reused code in body: %s", w2.Body.String()) }
}
