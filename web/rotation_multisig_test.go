...
func TestRotationSummary_MultiSignature(t *testing.T) {
    ...
    if _, err := m.Rotate(); err != nil { t.Fatalf("manager rotate: %v", err) }
    ...
    var resp struct {
        ...
        Summary struct {
            ...
            Signatures      []struct { Kid string `json:"kid"`; Signature string `json:"signature"`; Mode string `json:"mode"` } `json:"signatures"`
        } `json:"summary"`
    }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if !resp.Success { t.Fatalf("success false") }
    if resp.Summary.Threshold != 2 { t.Fatalf("expected threshold 2 got %d", resp.Summary.Threshold) }
    if resp.Summary.SatisfiedWeight < resp.Summary.Threshold { t.Fatalf("satisfied_weight < threshold: %+v", resp.Summary) }
    if len(resp.Summary.Signatures) < 2 { t.Fatalf("expected >=2 signatures in multisig mode got %d", len(resp.Summary.Signatures)) }
}
