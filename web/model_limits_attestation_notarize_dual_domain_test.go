...
func TestModelLimitsAttestationNotarizeDualDomain(t *testing.T) {
    ...
    if att.Signature == "" || att.DomainSignature == "" || att.DomainPrefix != "EXTRA_ATTEST:" { t.Fatalf("missing signature fields: %+v", att) }
    if att.Notarization == nil || !att.Notarization.Success { t.Fatalf("expected notarization receipt present: %+v", att.Notarization) }
    ...
    if !ed25519.Verify(active.Public, primaryMsg, primarySig) { t.Fatalf("primary signature invalid") }
    domainMsg := append([]byte(att.DomainPrefix), raw...)
    if !ed25519.Verify(active.Public, domainMsg, domainSig) { t.Fatalf("domain signature invalid") }
}
