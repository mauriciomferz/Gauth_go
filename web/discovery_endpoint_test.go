...
func TestRB3DiscoveryBasic(t *testing.T) {
	...
	if v, ok := payload["schema_version"].(float64); !ok || int(v) != 1 {
		t.Fatalf("schema_version unexpected %#v", payload["schema_version"])
	}
	if dom, ok := payload["active_digest_domain"].(string); !ok || dom == "" {
		t.Fatalf("active_digest_domain invalid %#v", payload["active_digest_domain"])
	}
	// Caching headers
	cc := w.Header().Get("Cache-Control")
	if cc != "max-age=30" {
		t.Fatalf("Cache-Control mismatch %s", cc)
	}
	...
	if w2.Body.Len() != 0 {
		t.Fatalf("expected empty body on 304 got %s", w2.Body.String())
	}
}
