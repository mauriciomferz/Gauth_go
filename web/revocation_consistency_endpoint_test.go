...
func TestRevocationConsistencySizesTrivial(t *testing.T) {
    ...
    if err := json.Unmarshal(w.Body.Bytes(), &body); err != nil { t.Fatalf("unmarshal: %v body=%s", err, w.Body.String()) }
    proof, ok := body["proof"].(map[string]any)
    if !ok { t.Fatalf("missing proof object body=%v", body) }
    if proof["trivial"] != true { t.Fatalf("expected trivial=true proof=%v", proof) }
    if path, ok := proof["path"].([]any); ok && len(path) != 0 { t.Fatalf("expected empty path proof=%v", proof) }
}
...
func TestRevocationConsistencySizesUnavailable(t *testing.T) {
    ...
    if body["code"] != "consistency_proof_unavailable" { t.Fatalf("expected code consistency_proof_unavailable body=%v", body) }
}
