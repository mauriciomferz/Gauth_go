...
func TestRotationV2SignatureInvalid(t *testing.T) {
  ...
  if err != nil { t.Fatalf("initial build error: %v", err) }
  if len(failuresOriginal) != 0 || verifiedOriginal == 0 { t.Fatalf("unexpected initial state failures=%v verified=%d", failuresOriginal, verifiedOriginal) }
  // Corrupt first signature by flipping bits.
  if len(art.Signers) == 0 || art.Signers[0].Signature == "" { t.Fatalf("no signer signature to corrupt") }
  raw, dErr := base64.RawURLEncoding.DecodeString(art.Signers[0].Signature)
  if dErr != nil { t.Fatalf("decode sig: %v", dErr) }
  if len(raw) != ed25519.SignatureSize { t.Fatalf("unexpected sig size %d", len(raw)) }
  raw[0] ^= 0xFF
  ...
  if verifiedAfter >= verifiedOriginal { t.Fatalf("expected lower verified weight after corruption; got %d vs original %d", verifiedAfter, verifiedOriginal) }
  if len(failuresAfter) == 0 { t.Fatalf("expected at least one failure after corruption") }
  // Ensure failure reasons contain either signature_invalid or public_key_not_found.
  found := false
  for _, f := range failuresAfter { if f == "signature_invalid" || f == "public_key_not_found" { found = true; break } }
  if !found { t.Fatalf("expected failure reason signature_invalid or public_key_not_found; got %v", failuresAfter) }
  _ = perAlg // allow lint
}
