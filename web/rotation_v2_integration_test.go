...
func TestRotationV2SigningIntegration(t *testing.T) {
  ...
  if err != nil { t.Fatalf("read config source (%s): %v", cfgSrc, err) }
  tmpDir := t.TempDir()
  ...
  if err != nil { t.Fatalf("buildAndOptionallySignRotationV2 error: %v", err) }
  if len(failures) != 0 { t.Fatalf("unexpected failures: %v", failures) }
  if verified < art.ThresholdWeight { t.Fatalf("verified weight %d below threshold %d", verified, art.ThresholdWeight) }
  if len(perAlg) == 0 || perAlg["ED25519"] == 0 { t.Fatalf("perAlg map missing ED25519 weight: %+v", perAlg) }
  // Ensure every configured signer acquired signature.
  missing := []string{}
  for _, s := range art.Signers { if s.Signature == "" { missing = append(missing, s.ID) } }
  if len(missing) > 0 { t.Fatalf("signatures missing for signers: %v", missing) }
  // Marshal to ensure JSON tags intact (no panic).
  if _, mErr := json.Marshal(art); mErr != nil { t.Fatalf("marshal artifact failed: %v", mErr) }
}
