...
func TestRotationV2ExplicitKeysImport(t *testing.T) {
  ...
  total := 0
  for _, s := range art.Signers { total += s.Weight }
  if verified != total { t.Fatalf("expected verified=%d got %d", total, verified) }
  if perAlg["ED25519"] != total { t.Fatalf("perAlg mismatch: %v", perAlg) }
  for _, s := range art.Signers { if s.Signature == "" { t.Fatalf("missing signature for %s", s.ID) } }
