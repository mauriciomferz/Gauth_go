...
func TestModelUserLimits(t *testing.T) {
	...
	if resp.Code != 200 { t.Fatalf("alice within limits expected 200 got %d body=%s", resp.Code, resp.Body.String()) }
	...
	if resp.Code != 400 || !bytes.Contains(resp.Body.Bytes(), []byte("model_user_input_limit_exceeded")) { t.Fatalf("expected alice input exceed 400 got %d body=%s", resp.Code, resp.Body.String()) }
	...
	if resp.Code != 400 || !bytes.Contains(resp.Body.Bytes(), []byte("model_user_output_limit_exceeded")) { t.Fatalf("expected alice output exceed 400 got %d body=%s", resp.Code, resp.Body.String()) }
	...
	for i:=0;i<3;i++ {
		...
		if i < 2 && resp.Code != 200 { t.Fatalf("alice rate allow #%d failed code=%d body=%s", i, resp.Code, resp.Body.String()) }
		if i == 2 && resp.Code != 429 { t.Fatalf("alice expected rate limited on 3rd request code=%d body=%s", resp.Code, resp.Body.String()) }
	}
	...
	if resp.Code != 400 || !bytes.Contains(resp.Body.Bytes(), []byte("model_user_input_limit_exceeded")) { t.Fatalf("expected bob input exceed 400 got %d body=%s", resp.Code, resp.Body.String()) }
	...
	if resp.Code != 400 || !bytes.Contains(resp.Body.Bytes(), []byte("model_limit_exceeded")) { t.Fatalf("expected global input exceed for charlie 400 got %d body=%s", resp.Code, resp.Body.String()) }
	...
	if mem, ok := bs.metrics.(*imetrics.Memory); ok {
		...
		if mem.ModelUserOutputLimitExceeded() < 1 { t.Fatalf("expected user output exceed counter >=1 got %d", mem.ModelUserOutputLimitExceeded()) }
		if mem.ModelUserRateLimitExceeded() < 1 { t.Fatalf("expected user rate exceed counter >=1 got %d", mem.ModelUserRateLimitExceeded()) }
	}
}
