...
func TestBLSAggregateIssueAndVerify(t *testing.T) {
    ...
    if err != nil { t.Fatalf("marshal issue body: %v", err) }
    w := httptest.NewRecorder()
    req := httptest.NewRequest("POST", "/api/v1/crypto/bls/aggregate", bytes.NewReader(ib))
    srv.router.ServeHTTP(w, req)
    if w.Code != 200 { t.Fatalf("issue expected 200 got %d body=%s", w.Code, w.Body.String()) }
    ...
    if err := json.Unmarshal(w.Body.Bytes(), &issueResp); err != nil { t.Fatalf("parse issue resp: %v", err) }
    if !issueResp.Success || issueResp.Mode != "issue" { t.Fatalf("unexpected issue response: %+v", issueResp) }
    if issueResp.ParticipantCount != 5 || len(issueResp.PublicKeysB64) != 5 { t.Fatalf("expected 5 participants, got %d/%d", issueResp.ParticipantCount, len(issueResp.PublicKeysB64)) }
    if issueResp.AggregatedSignatureB64 == "" { t.Fatalf("empty aggregated signature") }

    snapAfterIssue := mem.SnapshotEx()
    if snapAfterIssue.MultiSignatureBatchSizeCount == 0 { t.Fatalf("batch size count not recorded on issue") }
    if snapAfterIssue.MultiSignatureBatchSizeMax < 5 { t.Fatalf("batch size max < 5 got %d", snapAfterIssue.MultiSignatureBatchSizeMax) }
    if snapAfterIssue.MultiSignatureAggregateLatencyCount == 0 { t.Fatalf("aggregate latency count not recorded") }
    ...
    if err != nil { t.Fatalf("marshal verify body: %v", err) }
    wv := httptest.NewRecorder()
    reqv := httptest.NewRequest("POST", "/api/v1/crypto/bls/aggregate", bytes.NewReader(vb))
    srv.router.ServeHTTP(wv, reqv)
    if wv.Code != 200 { t.Fatalf("verify expected 200 got %d body=%s", wv.Code, wv.Body.String()) }
    ...
    if err := json.Unmarshal(wv.Body.Bytes(), &verifyResp); err != nil { t.Fatalf("parse verify resp: %v", err) }
    if !verifyResp.Success || verifyResp.Mode != "verify" || !verifyResp.Valid { t.Fatalf("unexpected verify response: %+v", verifyResp) }
    if verifyResp.ParticipantCount != 5 { t.Fatalf("expected verify participant_count=5 got %d", verifyResp.ParticipantCount) }

    snapAfterVerify := mem.SnapshotEx()
    if snapAfterVerify.MultiSignatureVerifications != 1 { t.Fatalf("expected MultiSignatureVerifications=1 got %d", snapAfterVerify.MultiSignatureVerifications) }
    if snapAfterVerify.MultiSignatureVerificationFailures != 0 { t.Fatalf("unexpected verification failures: %d", snapAfterVerify.MultiSignatureVerificationFailures) }
    if snapAfterVerify.MultiSignatureAggregateLatencyCount < 2 { t.Fatalf("expected aggregate latency count >=2 got %d", snapAfterVerify.MultiSignatureAggregateLatencyCount) }
}
...
func TestBLSAggregateVerifyTamper(t *testing.T) {
    ...
    if w.Code != 200 { t.Fatalf("issue expected 200 got %d", w.Code) }
    var ir struct { AggregatedSignatureB64 string `json:"aggregated_signature_b64"`; PublicKeysB64 []string `json:"public_keys_b64"` }
    if err := json.Unmarshal(w.Body.Bytes(), &ir); err != nil { t.Fatalf("parse issue resp: %v", err) }
    raw, err := base64.StdEncoding.DecodeString(ir.AggregatedSignatureB64)
    if err != nil { t.Fatalf("decode agg sig: %v" , err) }
    if len(raw) == 0 { t.Fatalf("empty agg sig bytes") }
    raw[0] ^= 0xFF // mutate
    ...
    wv := httptest.NewRecorder(); reqv := httptest.NewRequest("POST","/api/v1/crypto/bls/aggregate", bytes.NewReader(vb)); srv.router.ServeHTTP(wv, reqv)
    if wv.Code == 200 {
        var vr struct { Valid bool `json:"valid"` }
        if err := json.Unmarshal(wv.Body.Bytes(), &vr); err != nil { t.Fatalf("parse verify resp: %v", err) }
        if vr.Valid { t.Fatalf("expected invalid after tamper") }
        snap := mem.SnapshotEx()
        if snap.MultiSignatureVerificationFailures == 0 { t.Fatalf("expected verification failure counter increment") }
    } else if wv.Code == 400 {
...
func TestBLSAggregateBadMode(t *testing.T) {
    ...
    srv.router.ServeHTTP(w, req)
    if w.Code != 400 { t.Fatalf("expected 400 invalid mode got %d", w.Code) }
}
...
func TestBLSAggregateMissingMessage(t *testing.T) {
    ...
    req := httptest.NewRequest("POST", "/api/v1/crypto/bls/aggregate", bytes.NewReader(b))
    srv.router.ServeHTTP(w, req)
    if w.Code != 400 { t.Fatalf("expected 400 missing_message got %d", w.Code) }
}
...
func TestBLSAggregateLatencyAdvances(t *testing.T) {
    ...
    wv := httptest.NewRecorder(); reqv := httptest.NewRequest("POST","/api/v1/crypto/bls/aggregate", bytes.NewReader(vb)); start := time.Now(); srv.router.ServeHTTP(wv, reqv)
    if wv.Code != 200 { t.Fatalf("verify expected 200 got %d", wv.Code) }
    _ = start // placeholder in case future latency assertions compare wall-clock
    snap := mem.SnapshotEx()
    if snap.MultiSignatureAggregateLatencyCount < 2 { t.Fatalf("expected >=2 aggregate latency samples got %d", snap.MultiSignatureAggregateLatencyCount) }
    if snap.MultiSignatureAggregateLatencyTotalNS == 0 { t.Fatalf("expected non-zero aggregate latency total") }
}
...
func TestBLSAggregatePublicKeyDecodeFailure(t *testing.T) {
    ...
    wv := httptest.NewRecorder(); reqv := httptest.NewRequest("POST","/api/v1/crypto/bls/aggregate", bytes.NewReader(vb)); srv.router.ServeHTTP(wv, reqv)
    if wv.Code != 400 { t.Fatalf("expected 400 for public_key_decode_failed got %d body=%s", wv.Code, wv.Body.String()) }
    if !strings.Contains(wv.Body.String(), "public_key_decode_failed") { t.Fatalf("expected public_key_decode_failed marker, body=%s", wv.Body.String()) }
}
...
func TestBLSAggregateAggregatedSignatureDecodeFailure(t *testing.T) {
    ...
    w := httptest.NewRecorder(); req := httptest.NewRequest("POST","/api/v1/crypto/bls/aggregate", bytes.NewReader(ib)); srv.router.ServeHTTP(w, req)
    if w.Code != 200 { t.Fatalf("issue expected 200 got %d", w.Code) }
    var ir struct { PublicKeysB64 []string `json:"public_keys_b64"` }
    if err := json.Unmarshal(w.Body.Bytes(), &ir); err != nil { t.Fatalf("parse issue resp: %v", err) }
    ...
    wv := httptest.NewRecorder(); reqv := httptest.NewRequest("POST","/api/v1/crypto/bls/aggregate", bytes.NewReader(vb)); srv.router.ServeHTTP(wv, reqv)
    if wv.Code != 400 { t.Fatalf("expected 400 aggregated_signature_decode_failed got %d body=%s", wv.Code, wv.Body.String()) }
    if !strings.Contains(wv.Body.String(), "aggregated_signature_decode_failed") { t.Fatalf("expected aggregated_signature_decode_failed marker, body=%s", wv.Body.String()) }
}
