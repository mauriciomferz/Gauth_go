...
func TestSemanticReactiveThrottle(t *testing.T) {
    ...
    for i := 0; i < 6; i++ {
        ...
        s.semanticHistory = append(s.semanticHistory, struct{ At time.Time; Snapshot map[string]uint64 }{At: base.Add(time.Duration(i) * 5 * time.Second), Snapshot: snap})
        s.semanticHistMu.Unlock()
        w := httptest.NewRecorder()
        req, _ := http.NewRequest("GET", "/api/v1/diagnostics/semantic", http.NoBody)
        s.router.ServeHTTP(w, req)
        if w.Code != http.StatusOK { t.Fatalf("diag status %d", w.Code) }
    ...
    if wAct.Code != 429 { t.Fatalf("expected 429 got %d body=%s", wAct.Code, wAct.Body.String()) }
    var errResp struct { Code string `json:"code"`; RFC string `json:"rfc_ref"` }
    if err := json.Unmarshal(wAct.Body.Bytes(), &errResp); err != nil { t.Fatalf("unmarshal act: %v", err) }
    if errResp.Code != "semantic_throttle_active" { t.Fatalf("expected semantic_throttle_active code got %s", errResp.Code) }
    if errResp.RFC != "rfc115:reactive_controls" { t.Fatalf("expected rfc115:reactive_controls got %s", errResp.RFC) }
}
