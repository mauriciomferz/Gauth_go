...
func TestModelLimitsAttestationNotarization(t *testing.T) {
    ...
    var att attStruct
    if err := json.Unmarshal(w2.Body.Bytes(), &att); err != nil { t.Fatalf("unmarshal: %v", err) }
    if att.Signature == "" || att.SigKid == "" || att.SigMode != sigModeEdDSA { t.Fatalf("signature fields missing") }
    if att.Notarization == nil || !att.Notarization.Success || att.Notarization.Provider == "" { t.Fatalf("notarization missing or invalid") }
    // Surge stats are opportunistic; assert structure if present, otherwise accept absence.
    if att.Surge != nil {
        if !att.Surge.Triggered || att.Surge.ModelID == "" || att.Surge.Last10Sec == 0 { t.Fatalf("surge stats invalid: %+v", att.Surge) }
    }
    ...
    if !ed25519.Verify(active.Public, raw, sigBytes) { t.Fatalf("signature verify failed") }
}
