...
func TestRotationV2NoSigning(t *testing.T) {
	...
	if verified != 0 {
		t.Fatalf("expected verified 0 got %d", verified)
	}
	if len(perAlg) != 0 {
		t.Fatalf("expected empty perAlg got %v", perAlg)
	}
	if art.ThresholdWeight <= 0 {
		t.Fatalf("threshold invalid %d", art.ThresholdWeight)
	}
	if verified >= art.ThresholdWeight {
		t.Fatalf("unexpected threshold_met when signatures disabled")
	}
	if len(failures) != 0 {
		t.Fatalf("no failures expected, got %v", failures)
	}
}
