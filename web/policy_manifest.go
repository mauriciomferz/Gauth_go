...
func (s *BetaServer) registerPolicyManifest() {
	...
	s.router.GET("/api/v1/policy/manifest", func(c *gin.Context) {
		...
		if err != nil {
			respondError(c, 500, "manifest_build_failed", "build_failed",
				"policy manifest build failed", "AAP001:policy_manifest", err.Error())
			return
		}
		...
		if os.Getenv("AGENTAUTH_TOKEN_SIG_MODE") != sigModeEdDSA || kp == nil {
			respondError(c, 500, "signing_unavailable", "signing_unavailable",
				"active eddsa key unavailable", "AAP001:policy_manifest", nil)
			return
		}
		...
		if sErr != nil {
			respondError(c, 500, "signing_unavailable", "signing_unavailable",
				"active eddsa key unavailable", "AAP001:policy_manifest", sErr.Error())
			return
		}
