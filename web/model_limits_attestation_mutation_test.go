...
func TestModelLimitsAttestationMutation(t *testing.T) {
    ...
    if err := json.Unmarshal(w.Body.Bytes(), &att); err != nil { t.Fatalf("unmarshal: %v", err) }
    sig, _ := att["signature"].(string)
    kid, _ := att["sig_kid"].(string)
    if sig == "" || kid == "" { t.Fatalf("missing signature fields: %+v", att) }
    originalNonce, _ := att["nonce"].(string)
    if originalNonce == "" { t.Skip("nonce not present (replay protection disabled)") }
    ...
    if err := json.Unmarshal(w2.Body.Bytes(), &v); err != nil { t.Fatalf("verify decode: %v", err) }
    if suc, _ := v["success"].(bool); !suc { t.Fatalf("expected success true got %v", v) }
    if valid, _ := v["valid"].(bool); valid { t.Fatalf("expected valid false got %v", v) }
    if errStr, _ := v["error"].(string); errStr != "signature_invalid" { t.Fatalf("expected signature_invalid got %s full=%v", errStr, v) }
}
