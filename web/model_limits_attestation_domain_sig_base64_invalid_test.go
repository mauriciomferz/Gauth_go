...
// TestModelLimitsAttestationDomainSignatureBase64Invalid ensures invalid base64 domain_signature triggers soft invalid.
func TestModelLimitsAttestationDomainSignatureBase64Invalid(t *testing.T) {
    ...
    if att["domain_signature"] == nil { t.Skip("domain signature absent") }
    // Set invalid base64 value
    att["domain_signature"] = "***invalid***"
    ...
    if w2.Code != 200 { t.Fatalf("expected 200 got %d body=%s", w2.Code, w2.Body.String()) }
    var v map[string]any
    if err := json.Unmarshal(w2.Body.Bytes(), &v); err != nil { t.Fatalf("unmarshal verify: %v", err) }
    if v["error"] != "domain_signature_base64_invalid" { t.Fatalf("expected domain_signature_base64_invalid got %+v", v) }
