...
func (r Report) ToMarkdown() string {
	...
	fmt.Fprintf(
		&b,
		"<!-- conformance-meta generated=%s mapped_clauses=%d "+
			"found_clauses=%d required_symbols=%d symbols_found=%d "+
			"coverage=%.2f gap_impl=%d gap_partial=%d gap_missing=%d gap_total=%d -->\n",
		r.GeneratedAt,
		r.Summary.MappedClauses,
		r.Summary.MappedClausesFound,
		r.Summary.RequiredSymbols,
		r.Summary.SymbolsFound,
		r.Summary.CoveragePercent,
		r.Summary.GapImplemented,
		r.Summary.GapPartial,
		r.Summary.GapMissing,
		r.Summary.GapTotal,
	)
	...
	fmt.Fprintf(
		&b,
		"Missing: clauses=%d symbols=%d tests=%d\n\n",
		r.Summary.MissingClauses,
		r.Summary.MissingSymbols,
		r.Summary.MissingTests,
	)
	fmt.Fprintf(
		&b,
		"GAP Matrix: implemented=%d partial=%d missing=%d total=%d\n\n",
		r.Summary.GapImplemented,
		r.Summary.GapPartial,
		r.Summary.GapMissing,
		r.Summary.GapTotal,
	...
	if len(r.GapMatrix.Sections) > 0 {
		...
		fmt.Fprintf(
			&b,
			"_GAP status distribution: implemented=%d partial=%d missing=%d total=%d_\n\n",
			r.Summary.GapImplemented,
			r.Summary.GapPartial,
			r.Summary.GapMissing,
			r.Summary.GapTotal,
...
// Each symbol lists count and collapsible details (simple text fallback).
// Consumers can post-process for actual HTML collapsible if desired.
