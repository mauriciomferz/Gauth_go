...
func main() {
	...
	// Channel for controlling load monitoring and health reporting
...
func monitorHealth(serviceMesh *mesh.ServiceMesh, done chan struct{}) {
	// Periodically print health report for all services
...
func runSimulationPhases(ctx context.Context, serviceMesh *mesh.ServiceMesh) {
	// Define simulation phases with increasing load factors
	phases := []struct {
		loadFactor float64
	...
	for phaseNum, phase := range phases {
		// Set load factors and simulate traffic for each phase
...
func simulateTraffic(ctx context.Context, serviceMesh *mesh.ServiceMesh, numClients int) {
	...
	// Simulate concurrent client requests to OrderService
