// Cascading Failure Simulation Example
// Demonstrates resilience patterns in microservices architectures using a service mesh.
// Shows how failures can cascade through dependencies and how patterns mitigate issues.
...
// ServiceMesh coordinates all services and dependencies
...
func (mesh *ServiceMesh) addService(sType ServiceType, name string, baseLatency time.Duration, baseErrorRate float64) {
	...
	svc.Resilience = resilience.NewPatterns(name,
		resilience.WithCircuitBreaker(
			...
			func(name string, from, to interface{}) {
				fromState, _ := from.(resilience.CircuitState)
				toState, _ := to.(resilience.CircuitState)
...
// processRequest simulates a service request, checks dependencies, and applies load/failure logic
...
// call executes a request with resilience patterns (circuit breaker, rate limit, bulkhead, retry)
...
// recordSuccess and recordFailure update health metrics
...
// increaseLoad sets the service load factor
