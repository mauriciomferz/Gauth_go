...
import (
	...
	"github.com/Gimel-Foundation/GiFo-RFC-0150-Go-Implementation-of-GAuth-1.0/internal/rate"
)
...
func demoBasicUsage(limiter *rate.Limiter) {
	...
	fmt.Println("\nBasic Usage Demo:")
	for i := 1; i <= 3; i++ {
		if err := limiter.AllowClient(ctx, id); err != nil {
...
func main() {
	...
	config := rate.Config{
		...
		WindowSize:        2 * time.Second, // Over a 2-second window
