...
func main() {
	...
	// Simulate client 1: Burst of requests (burst test)
	wg.Add(1)
	go func() {
		defer wg.Done()
		clientID := "client1"
	...
	// Simulate client 2: Steady rate (steady rate test)
	wg.Add(1)
	go func() {
		defer wg.Done()
		clientID := "client2"
	...
	// Simulate client 3: Reset and retry (reset test)
	wg.Add(1)
	go func() {
		defer wg.Done()
		clientID := "client3"
	...
	// Simulate client 4: Remove and retry (remove test)
	wg.Add(1)
	go func() {
		defer wg.Done()
		clientID := "client4"
	...
	// Demonstrate window sliding (window sliding test)
	fmt.Printf("\nWindow sliding test:\n")
	clientID := "client5"
