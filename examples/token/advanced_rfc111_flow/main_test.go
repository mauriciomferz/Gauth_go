...
func TestAdvancedRFC111FlowOutput(t *testing.T) {
	...
	if !strings.Contains(output, "Owner proof for user-100: email-verification") {
		t.Errorf("Expected output to contain 'Owner proof for user-100: email-verification', got: %s", output)
	}
	if !strings.Contains(output, "Token granted.") {
		t.Errorf("Expected output to contain 'Token granted.', got: %s", output)
	}
	if !strings.Contains(output, "Token delegated to user-200.") {
		t.Errorf("Expected output to contain 'Token delegated to user-200.', got: %s", output)
	}
