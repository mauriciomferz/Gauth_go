...
- **Metadata**: Device, app, labels, tags, and attributes are type-safe.
- **Token Rotation**: Demonstrates secure rotation of tokens.

## How to Run
```bash
cd examples/token/type_safe_usage
go run main.go
```

## Code Review Notes
- Code is idiomatic Go, modular, and demonstrates type-safe token patterns.
- All operations are clearly separated and easy to follow.
- Inline comments added for Beta readability.

## Educational Comments
- See `main.go` for type-safe token operations and metadata usage.
- All major token lifecycle operations are demonstrated and tested.
