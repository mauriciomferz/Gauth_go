...
func startJWKS(keys map[string]*rsa.PrivateKey) *httptest.Server {
    type jwk struct { Kid string `json:"kid"`; Kty string `json:"kty"`; Alg string `json:"alg"`; Use string `json:"use"`; N string `json:"n"`; E string `json:"e"` }
    handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request){
        var out struct{ Keys []jwk `json:"keys"` }
...
func TestAuthMiddleware_RS256_JWKSFetchError(t *testing.T) {
    ...
    if body["reason"] != "jwks_fetch_error" { t.Fatalf("expected reason jwks_fetch_error got %v body=%s", body["reason"], w.Body.String()) }
}
...
func TestAuthMiddleware_RS256_NegativeKidCaching(t *testing.T) {
    ...
    jwksSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request){
        ...
        e := base64.RawURLEncoding.EncodeToString(eBytes)
        out := struct{ Keys []jwk `json:"keys"` }{Keys: []jwk{{Kid:"validkid",Kty:"RSA",Alg:"RS256",Use:"sig",N:n,E:e}}}
        _ = json.NewEncoder(w).Encode(out)
    }))
    defer jwksSrv.Close()
    ...
    defer func(){ os.Unsetenv("GAUTH_AI_DEMO_JWKS_URL"); os.Unsetenv("GAUTH_AI_DEMO_JWT_EXPECT_ALG"); os.Unsetenv("GAUTH_AI_DEMO_JWKS_NEGATIVE_TTL_SECONDS") }()
    // Reset caches
    jwksCache = nil
    jwksNegative = nil
    jwksExpiry = time.Now().Add(-1 * time.Second)
    ...
    r := gin.New(); r.Use(authMiddleware()); r.GET("/p", func(c *gin.Context){ c.String(200, "ok") })
    // First request (should trigger JWKS fetch + kid_not_found)
    req,_ := http.NewRequest("GET", "/p", nil); req.Header.Set("Authorization", "Bearer "+token1)
    w := httptest.NewRecorder(); r.ServeHTTP(w, req)
    if w.Code != 401 { t.Fatalf("expected 401 first miss got %d", w.Code) }
    // Second request (negative cache hit; should NOT trigger second JWKS fetch)
    req2,_ := http.NewRequest("GET", "/p", nil); req2.Header.Set("Authorization", "Bearer "+token2)
    w2 := httptest.NewRecorder(); r.ServeHTTP(w2, req2)
    if w2.Code != 401 { t.Fatalf("expected 401 second miss got %d", w2.Code) }
    if reqCount != 1 { t.Fatalf("expected single JWKS HTTP fetch, got %d", reqCount) }
}
...
func TestJWKSBackgroundRefresh(t *testing.T) {
    ...
    jwksSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request){
        ...
        eBytes := big.NewInt(int64(priv.PublicKey.E)).Bytes()
        e := base64.RawURLEncoding.EncodeToString(eBytes)
        out := struct{ Keys []jwk `json:"keys"` }{Keys: []jwk{{Kid:"bkid",Kty:"RSA",Alg:"RS256",Use:"sig",N:n,E:e}}}
        _ = json.NewEncoder(w).Encode(out)
    }))
    defer jwksSrv.Close()
    os.Setenv("GAUTH_AI_DEMO_JWKS_URL", jwksSrv.URL)
    ...
    if fetches < 1 { t.Fatalf("expected initial JWKS fetch, got %d", fetches) }
    ...
    time.Sleep(sleepFor)
    if fetches < 2 { t.Fatalf("expected background refresh second fetch, still %d", fetches) }
}
...
func TestAuthMiddleware_RS256_NegativeKidEviction(t *testing.T) {
    ...
    defer func(){
        os.Unsetenv("GAUTH_AI_DEMO_JWKS_URL"); os.Unsetenv("GAUTH_AI_DEMO_JWT_EXPECT_ALG")
        os.Unsetenv("GAUTH_AI_DEMO_JWKS_NEGATIVE_TTL_SECONDS"); os.Unsetenv("GAUTH_AI_DEMO_JWKS_NEGATIVE_MAX_ENTRIES")
    ...
    jwksCache = nil; jwksNegative = nil; jwksExpiry = time.Now().Add(-1*time.Second)
    ...
    r := gin.New(); r.Use(authMiddleware())
    // add /metrics endpoint to scrape counters
    r.GET("/metrics", gin.WrapH(promhttp.Handler()))
    r.GET("/p", func(c *gin.Context){ c.String(200, "ok") })
    // First request (kidA) -> adds kidA negative entry
    reqA,_ := http.NewRequest("GET", "/p", nil); reqA.Header.Set("Authorization", "Bearer "+tokenA)
    wA := httptest.NewRecorder(); r.ServeHTTP(wA, reqA)
    if wA.Code != 401 { t.Fatalf("expected 401 for first missing kid, got %d", wA.Code) }
    // Second request (kidB) -> triggers eviction (kidA removed, kidB inserted)
    reqB,_ := http.NewRequest("GET", "/p", nil); reqB.Header.Set("Authorization", "Bearer "+tokenB)
    wB := httptest.NewRecorder(); r.ServeHTTP(wB, reqB)
    if wB.Code != 401 { t.Fatalf("expected 401 for second missing kid, got %d", wB.Code) }
    ...
    for _, ln := range lines {
        if strings.HasPrefix(ln, "ai_demo_jwks_negative_evictions_total") {
            ...
            if len(parts) == 2 { if v, err := strconv.Atoi(parts[1]); err == nil { evictionVal = v } }
        }
    }
    if evictionVal != 1 { t.Fatalf("expected eviction counter=1 got %d full metrics=%s", evictionVal, metricsBody) }
