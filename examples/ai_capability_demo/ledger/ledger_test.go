...
func TestLedgerAppendAndRoot(t *testing.T) {
	l := New()
	if l.LatestRoot() != "" {
		t.Fatalf("expected empty root initially")
	}
	...
	if root == "" {
		t.Fatalf("expected non-empty root after first append")
	}
	id2, root2 := l.Append("decision", "payload2", "")
	if id2 != 1 {
		t.Fatalf("expected id 1 got %d", id2)
	}
	if root2 == root {
		t.Fatalf("expected root change after second append")
	}
}
...
func TestLedgerProofIntegrity(t *testing.T) {
	l := New()
	for i := 0; i < 5; i++ {
		l.Append("decision", "p"+string(rune('a'+i)), "")
	}
	proof, err := l.Proof(2)
	if err != nil {
	...
	if proof.EntryID != 2 {
		t.Fatalf("expected entry id 2")
	}
	if len(proof.Siblings) == 0 {
		t.Fatalf("expected non-empty siblings path")
	}
	if proof.Root != l.LatestRoot() {
		t.Fatalf("proof root mismatch latest")
	}
	if len(proof.Orientations) != len(proof.Siblings) {
		t.Fatalf("orientations length mismatch siblings")
	}
	...
	if fullRoot != proof.Root {
		t.Fatalf("recomputed full root mismatch: got %s want %s", fullRoot, proof.Root)
	}
}
...
func TestLedgerProofOrientationRebuild(t *testing.T) {
	l := New()
	for i := 0; i < 6; i++ {
		l.Append("decision", fmt.Sprintf("entry-%d", i), "")
	}
	proof, err := l.Proof(5)
	if err != nil {
	...
	if acc != proof.Root {
		t.Fatalf("orientation rebuild root mismatch got %s want %s", acc, proof.Root)
	}
}
