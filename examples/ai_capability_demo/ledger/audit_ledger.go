...
func NewAuditLedger(path string) (*AuditLedger, error) {
	...
	if err := db.Update(func(tx *bolt.Tx) error { _, e := tx.CreateBucketIfNotExists(auditBucket); return e }); err != nil {
		db.Close()
		return nil, err
	}
...
func (al *AuditLedger) VerifyChain() (int, error) {
	...
	err := al.db.View(func(tx *bolt.Tx) error {
		...
		for k, v := c.First(); k != nil; k, v = c.Next() {
			...
			if calc != ae.EntryHash {
				idxMismatch = ae.ID
				return errors.New("entry_hash_mismatch")
			}
			prev = ae.EntryHash
		}
		return nil
