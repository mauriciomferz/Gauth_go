...
func (bl *BoltLedger) Append(entryType, payload, digest string) (int, string, error) {
	bl.mu.Lock(); defer bl.mu.Unlock()
	var id int
	if digest == "" { ph := sha256.Sum256([]byte(payload)); digest = hex.EncodeToString(ph[:]) }
	err := bl.db.Update(func(tx *bolt.Tx) error {
		b := tx.Bucket(boltBucket)
		if b == nil { return errors.New("bucket_missing") }
		id = b.Stats().KeyN
...
func (bl *BoltLedger) Get(id int) (Entry, bool) {
	bl.mu.RLock(); defer bl.mu.RUnlock()
	var out Entry
...
func (bl *BoltLedger) Size() int {
	bl.mu.RLock(); defer bl.mu.RUnlock()
	var n int
	bl.db.View(func(tx *bolt.Tx) error { b := tx.Bucket(boltBucket); if b != nil { n = b.Stats().KeyN }; return nil })
	return n
}
...
func (bl *BoltLedger) Proof(id int) (Proof, error) {
	...
	if id < 0 || id >= len(entries) { return Proof{}, fmt.Errorf("entry_not_found") }
	layer := make([]string, len(entries))
	for i, e := range entries { layer[i] = leafHash(e.Digest) }
	path := []string{}
	orient := []string{}
	idx := id
	work := layer
	for len(work) > 1 {
		...
		for i := 0; i < len(work); i += 2 {
			if i+1 == len(work) { next = append(next, hash(work[i], work[i])) } else { next = append(next, hash(work[i], work[i+1])) }
		}
		idx = idx / 2
		work = next
	}
...
func (bl *BoltLedger) HistoricalRoots() []string { bl.mu.RLock(); defer bl.mu.RUnlock(); out := make([]string, len(bl.roots)); copy(out, bl.roots); return out }
...
func (bl *BoltLedger) recomputeRootsLocked() {
	entries, err := bl.loadAllLocked(); if err != nil { return }
	bl.roots = []string{}
	for i := range entries { // incremental emission semantics
		partial := entries[:i+1]
		layer := make([]string, len(partial))
		for j, e := range partial { layer[j] = leafHash(e.Digest) }
		for len(layer) > 1 {
			var next []string
			for k := 0; k < len(layer); k += 2 {
				if k+1 == len(layer) { next = append(next, hash(layer[k], layer[k])) } else { next = append(next, hash(layer[k], layer[k+1])) }
			}
			layer = next
		}
...
func (bl *BoltLedger) computeRootLocked() string {
	entries, err := bl.loadAllLocked(); if err != nil || len(entries) == 0 { return "" }
	layer := make([]string, len(entries))
	for i, e := range entries { layer[i] = leafHash(e.Digest) }
	for len(layer) > 1 {
		var next []string
