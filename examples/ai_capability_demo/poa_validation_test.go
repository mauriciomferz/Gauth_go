...
func setupTestRouter(t *testing.T) *gin.Engine {
    ...
    if os.Getenv("GAUTH_AI_DEMO_TEST_DISABLE_ENFORCEMENT") == "1" {
    ...
    r.POST("/demo/enforce", func(c *gin.Context) {
        ...
        if req.POAID != "" { // replicate PoA validation logic
            result := "not_found"
            poaRepoMu.RLock(); poa, ok := poaRepo[req.POAID]; poaRepoMu.RUnlock()
            if ok {
                now := time.Now().UTC()
                if poa.Status == rfc0111.POAStatusRevoked { result = "revoked"; allowed = false } else if now.After(poa.ValidUntil) || now.Before(poa.ValidFrom) { result = "expired"; allowed = false } else {
                    match := false
                    for _, s := range poa.Scope { if s == req.Action { match = true; break } }
                    if !match { result = "scope_mismatch"; allowed = false } else { result = "success" }
                }
                meta["poa_id"] = poa.ID
            } else {
                allowed = false
    ...
    r.POST("/demo/poa/:id/sign", func(c *gin.Context) {
        ...
        if err := c.ShouldBindJSON(&req); err != nil { c.JSON(400, gin.H{"error": err.Error()}); return }
        poaRepoMu.Lock(); poa, ok := poaRepo[id];
        ...
        // signer validation
        known := false
        for _, s := range poa.Signers { if s == req.Signer { known = true; break } }
        if !known { poaRepoMu.Unlock(); c.JSON(400, gin.H{"error": "unknown_signer"}); return }
        ...
        poa.MultiSignatures[req.Signer] = &rfc0111.POASignature{Algorithm: "ed25519", KeyID: req.Signer + "-kid", DigestHex: digest, SigBase64: dummySig, Canonical: canon}
    ...
    r.GET("/demo/poa/:id/status", func(c *gin.Context) {
        id := c.Param("id")
        poaRepoMu.RLock(); poa, ok := poaRepo[id]; poaRepoMu.RUnlock()
...
func issueTestPOA(id string, status rfc0111.POAStatus, scope []string, durSeconds int) *rfc0111.PowerOfAttorney {
    now := time.Now().UTC()
    ...
    poaRepoMu.Lock(); poaRepo[poa.ID] = poa; poaRepoMu.Unlock()
    return poa
}
...
func signHS256(claims map[string]any, secret string) string {
    ...
    mac := hmac.New(sha256.New, []byte(secret)); mac.Write([]byte(header + "." + payload)); sig := base64.RawURLEncoding.EncodeToString(mac.Sum(nil))
    return header + "." + payload + "." + sig
}

func TestPOAValidationSuccess(t *testing.T) {
    os.Setenv("GAUTH_AI_DEMO_JWT_SECRET", "secret123")
    ...
    poa := issueTestPOA("poa-success", rfc0111.POAStatusActive, []string{"transaction:read"}, 300)
    ...
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal error: %v", err) }
    res := resp["result"].(map[string]any)
    if allowed, _ := res["allowed"].(bool); !allowed { t.Fatalf("expected allowed true") }
    os.Unsetenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH")
    os.Unsetenv("GAUTH_AI_DEMO_TEST_DISABLE_ENFORCEMENT")
}

func TestPOAValidationRevoked(t *testing.T) {
    os.Setenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH", "1")
    router := setupTestRouter(t)
    poa := issueTestPOA("poa-revoked", rfc0111.POAStatusRevoked, []string{"transaction:read"}, 300)
    body := `{"action":"transaction:read","claims":{},"poa_id":"` + poa.ID + `"}`
    w := httptest.NewRecorder(); req := httptest.NewRequest("POST", "/demo/enforce", http.NoBody); req.Body = httptestBody(body)
    router.ServeHTTP(w, req)
    var resp map[string]any; _ = json.Unmarshal(w.Body.Bytes(), &resp)
    res := resp["result"].(map[string]any)
    if allowed, _ := res["allowed"].(bool); allowed { t.Fatalf("expected revoked PoA to disallow") }
}
...
func TestPOAValidationExpired(t *testing.T) {
    ...
    // expires immediately
    now := time.Now().UTC()
    poa := &rfc0111.PowerOfAttorney{ID: "poa-expired", Version: 1, Grantor: "g", Grantee: "gr", Scope: []string{"transaction:read"}, Restrictions: map[string]string{}, ValidFrom: now.Add(-time.Hour), ValidUntil: now.Add(-time.Minute), Status: rfc0111.POAStatusActive, CreatedAt: now, UpdatedAt: now}
    poaRepoMu.Lock(); poaRepo[poa.ID] = poa; poaRepoMu.Unlock()
    ...
    var resp map[string]any; _ = json.Unmarshal(w.Body.Bytes(), &resp)
    res := resp["result"].(map[string]any)
    if allowed, _ := res["allowed"].(bool); allowed { t.Fatalf("expected expired PoA to disallow") }
}

func TestPOAValidationScopeMismatch(t *testing.T) {
    os.Setenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH", "1")
    router := setupTestRouter(t)
    poa := issueTestPOA("poa-scope", rfc0111.POAStatusActive, []string{"transaction:read"}, 300)
    body := `{"action":"transaction:execute","claims":{},"poa_id":"` + poa.ID + `"}`
    w := httptest.NewRecorder(); req := httptest.NewRequest("POST", "/demo/enforce", http.NoBody); req.Body = httptestBody(body)
    router.ServeHTTP(w, req)
    var resp map[string]any; _ = json.Unmarshal(w.Body.Bytes(), &resp)
    res := resp["result"].(map[string]any)
    if allowed, _ := res["allowed"].(bool); allowed { t.Fatalf("expected scope mismatch to disallow") }
}
...
func TestPOAIntegrityDigestMismatch(t *testing.T) {
    ...
    r := setupTestRouter(t)
    poa := issueTestPOA("poa-digest", rfc0111.POAStatusActive, []string{"transaction:read"}, 300)
    now := time.Now().UTC()
    ...
    w := httptest.NewRecorder(); req := httptest.NewRequest("POST", "/demo/enforce", httptestBody(`{"action":"transaction:read","claims":{},"poa_id":"`+poa.ID+`"}`))
    req.Header.Set("Authorization", "Bearer "+token)
    r.ServeHTTP(w, req)
    if w.Code != http.StatusUnauthorized { t.Fatalf("expected 401 got %d", w.Code) }
    var resp map[string]any; _ = json.Unmarshal(w.Body.Bytes(), &resp)
    if resp["reason"] != "poa_digest_mismatch" { t.Fatalf("expected poa_digest_mismatch got %v", resp["reason"]) }
    os.Unsetenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH")
}
...
func TestPOAIntegrityVersionMismatch(t *testing.T) {
    ...
    os.Setenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH", "0")
    r := setupTestRouter(t)
    poa := issueTestPOA("poa-version", rfc0111.POAStatusActive, []string{"transaction:read"}, 300)
    now := time.Now().UTC()
    digest, _, _ := rfc0111.CanonicalPOADigest(poa)
    ...
    if w.Code != http.StatusUnauthorized { t.Fatalf("expected 401 got %d", w.Code) }
    var resp map[string]any; _ = json.Unmarshal(w.Body.Bytes(), &resp)
    if resp["reason"] != "poa_version_mismatch" { t.Fatalf("expected poa_version_mismatch got %v", resp["reason"]) }
    os.Unsetenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH")
}
...
func TestPOAIntegritySuccess(t *testing.T) {
    ...
    os.Setenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH", "0")
    r := setupTestRouter(t)
    poa := issueTestPOA("poa-success-int", rfc0111.POAStatusActive, []string{"transaction:read"}, 300)
    now := time.Now().UTC()
    digest, _, _ := rfc0111.CanonicalPOADigest(poa)
...
func TestPOAIntegrityMetricsIncrement(t *testing.T) {
    ...
    os.Setenv("GAUTH_AI_DEMO_JWT_SECRET", secret)
    os.Setenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH", "0")
    // Initialize metrics counter manually if not already (simulate GAUTH_AI_DEMO_METRICS=1 block)
    ...
    r := setupTestRouter(t)
    poa := issueTestPOA("poa-metrics", rfc0111.POAStatusActive, []string{"transaction:read"}, 300)
    now := time.Now().UTC()
    digest, _, _ := rfc0111.CanonicalPOADigest(poa)
    ...
    if after != before+1 { t.Fatalf("expected digest_mismatch counter to increment by 1 (before=%f after=%f)", before, after) }
    os.Unsetenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH")
}
...
func TestMultisigQuorumActivation(t *testing.T) {
    gin.SetMode(gin.TestMode)
    os.Setenv("GAUTH_AI_DEMO_JWT_SECRET", "multisigsecret")
    os.Setenv("GAUTH_AI_DEMO_TEST_BYPASS_AUTH", "1")
    router := setupTestRouter(t)
    // Prepare draft via direct in-memory creation (simpler than hitting prepare endpoint for unit scope)
    now := time.Now().UTC()
    poa := &rfc0111.PowerOfAttorney{ID: "poa-multi", Version:1, Grantor:"g1", Grantee:"gr1", Scope: []string{"transaction:read"}, ValidFrom: now, ValidUntil: now.Add(time.Hour), Status: rfc0111.POAStatusDraft, CreatedAt: now, UpdatedAt: now, Signers: []string{"alice","bob"}, Threshold:2, MultiSignatures: map[string]*rfc0111.POASignature{}}
    poaRepoMu.Lock(); poaRepo[poa.ID] = poa; poaRepoMu.Unlock()
    // Simulate signatures hitting the sign endpoint for realism
    for _, signer := range []string{"alice","bob"} {
        body := `{"signer":"` + signer + `"}`
        w := httptest.NewRecorder(); req := httptest.NewRequest("POST", "/demo/poa/"+poa.ID+"/sign", http.NoBody); req.Body = httptestBody(body)
        router.ServeHTTP(w, req)
        if w.Code != 200 { t.Fatalf("signature submission failed %d body=%s", w.Code, w.Body.String()) }
    }
    // Finalize
    wFin := httptest.NewRecorder(); reqFin := httptest.NewRequest("POST", "/demo/poa/"+poa.ID+"/finalize", http.NoBody)
    router.ServeHTTP(wFin, reqFin)
    if wFin.Code != 200 { t.Fatalf("finalize failed: %d body=%s", wFin.Code, wFin.Body.String()) }
    // Validate status changed
    poaRepoMu.RLock(); updated := poaRepo[poa.ID]; poaRepoMu.RUnlock()
    if updated.Status != rfc0111.POAStatusActive { t.Fatalf("expected status active got %s", updated.Status) }
}
...
func TestMultisigDuplicateSignature(t *testing.T) {
    ...
    router := setupTestRouter(t)
    now := time.Now().UTC()
    poa := &rfc0111.PowerOfAttorney{ID: "poa-multi-dup", Version:1, Grantor:"g1", Grantee:"gr1", Scope: []string{"transaction:read"}, ValidFrom: now, ValidUntil: now.Add(time.Hour), Status: rfc0111.POAStatusDraft, CreatedAt: now, UpdatedAt: now, Signers: []string{"alice","bob"}, Threshold:2, MultiSignatures: map[string]*rfc0111.POASignature{}}
    poaRepoMu.Lock(); poaRepo[poa.ID] = poa; poaRepoMu.Unlock()
    // First signature
    w1 := httptest.NewRecorder(); req1 := httptest.NewRequest("POST", "/demo/poa/"+poa.ID+"/sign", http.NoBody); req1.Body = httptestBody(`{"signer":"alice"}`)
    router.ServeHTTP(w1, req1); if w1.Code != 200 { t.Fatalf("first signature failed: %d", w1.Code) }
    // Duplicate
    w2 := httptest.NewRecorder(); req2 := httptest.NewRequest("POST", "/demo/poa/"+poa.ID+"/sign", http.NoBody); req2.Body = httptestBody(`{"signer":"alice"}`)
    router.ServeHTTP(w2, req2); if w2.Code != 409 { t.Fatalf("expected 409 duplicate got %d", w2.Code) }
}
...
func TestMultisigUnknownSigner(t *testing.T) {
    ...
    router := setupTestRouter(t)
    now := time.Now().UTC()
    poa := &rfc0111.PowerOfAttorney{ID: "poa-multi-unknown", Version:1, Grantor:"g1", Grantee:"gr1", Scope: []string{"transaction:read"}, ValidFrom: now, ValidUntil: now.Add(time.Hour), Status: rfc0111.POAStatusDraft, CreatedAt: now, UpdatedAt: now, Signers: []string{"alice","bob"}, Threshold:2, MultiSignatures: map[string]*rfc0111.POASignature{}}
    poaRepoMu.Lock(); poaRepo[poa.ID] = poa; poaRepoMu.Unlock()
    w := httptest.NewRecorder(); req := httptest.NewRequest("POST", "/demo/poa/"+poa.ID+"/sign", http.NoBody); req.Body = httptestBody(`{"signer":"charlie"}`)
    router.ServeHTTP(w, req)
    if w.Code != 400 { t.Fatalf("expected 400 unknown signer got %d", w.Code) }
}
...
type bytesBuffer struct { b []byte; i int }
func bytesNewBufferString(s string) *bytesBuffer { return &bytesBuffer{b: []byte(s)} }
func (bb *bytesBuffer) Read(p []byte) (int, error) { if bb.i >= len(bb.b) { return 0, ioEOF }; n := copy(p, bb.b[bb.i:]); bb.i += n; return n, nil }
