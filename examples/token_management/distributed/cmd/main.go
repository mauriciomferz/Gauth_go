...
type ClusterNode struct {
	...
	jwtSigner   token.JWTSigner       // interface, not *token.JWTSigner
...
func NewClusterNode(id string, sharedChan chan TokenEvent) *ClusterNode {
	...
	jwtSigner := token.NewJWTSigner([]byte("dummy-key"), token.HS256)

	node := &ClusterNode{
		id:          id,
		store:       store,
		blacklist:   blacklist, // assign as interface
		jwtSigner:   jwtSigner, // assign as interface
...
func (n *ClusterNode) RevokeToken(ctx context.Context, tokenID string) error {
	...
	if err := n.blacklist.Add(ctx, t.ID, "cluster revocation"); err != nil {
		return err
	}
	n.tokenEvents <- TokenEvent{
		Type:      "revoked",
...
func main() {
	...
	go func() {
		...
		if verified, err := node2.jwtSigner.Verify(signed); err != nil {
			log.Printf("Node 2 failed to verify token: %v", err)
		} else {
			fmt.Printf("Node 2 verified token for subject: %s\n", verified.Subject)
		}

		if verified, err := node3.jwtSigner.Verify(signed); err != nil {
