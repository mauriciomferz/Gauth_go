...
func main() {
	...
	if validationErr, ok := err.(*token.ValidationError); ok {
		fmt.Printf("Token validation failed as expected with code: %s\n", validationErr.Code)
	}
	...
	// Issue refresh token
	issuedRefresh, err := tokenService.Issue(ctx, refreshToken)
	if err != nil {
		log.Fatalf("Failed to issue refresh token: %v", err)
	}

	// Exchange refresh token for new access token
	newAccess, err := tokenService.Refresh(ctx, issuedRefresh)
	if err != nil {
		log.Fatalf("Failed to refresh token: %v", err)
	}
	fmt.Printf("New access token issued with ID: %s\n", newAccess.ID)
}
