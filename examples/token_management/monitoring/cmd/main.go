...
func main() {
	...
	// Manually filter tokens expiring within 1 hour
	allTokens, err := monitor.querier.Query(ctx, token.Filter{})
	if err != nil {
		...
		for _, t := range allTokens {
			...
			if t.ExpiresAt.Sub(now) < time.Hour {
				if err := blacklist.Add(ctx, t.ID, "monitoring test"); err != nil {
