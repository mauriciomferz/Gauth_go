...
func NewAPIGateway() *APIGateway {
	...
	gateway.globalLimit = ratelimit.NewLimiter(&ratelimit.Config{
...
func (g *APIGateway) AddRoute(config *RouteConfig) {
	...
	if svc != nil {
		...
		svc.limiter = ratelimit.NewLimiter(config.RateLimit)
...
func (g *APIGateway) HandleRequest(ctx context.Context, req *RequestContext) error {
	...
	if err := g.globalLimit.Allow(ctx, req.ClientID); err != nil {
	...
	return svc.bulkhead.Execute(ctx, func() error {
		...
		return svc.retry.Execute(ctx, func() error {
			return svc.breaker.Execute(ctx, func() error {
