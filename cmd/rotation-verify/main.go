...
type resolver struct { m map[string]notary.PublicKeyRecord }
func (r *resolver) FindByID(id string) *notary.PublicKeyRecord { rec, ok := r.m[id]; if !ok { return nil }; return &rec }
...
func parsePubSpec(spec string) (id string, rec notary.PublicKeyRecord, err error) {
  ...
  if len(parts) < 3 { return "", rec, errors.New("invalid pub spec; want id:ALG:base64url") }
  id = parts[0]
  alg := strings.ToUpper(parts[1])
  rawB64 := parts[2]
  b, err := base64.RawURLEncoding.DecodeString(rawB64)
  if err != nil { return "", rec, fmt.Errorf("decode: %w", err) }
  switch alg {
  case "ED25519":
    if len(b) != ed25519.PublicKeySize { return "", rec, fmt.Errorf("ed25519 pub key wrong size %d", len(b)) }
...
func loadArtifact(path string) (*notary.WeightedRotationArtifact, error) {
  ...
  if env.Artifact != nil { return env.Artifact, nil }
...
func main(){
  ...
  flag.Parse()
  if file == "" { fmt.Fprintln(os.Stderr, "--file required"); os.Exit(2) }
  art, err := loadArtifact(file)
  if err != nil { fmt.Fprintf(os.Stderr, "error: %v\n", err); os.Exit(1) }
  if expectDigest != "" && art.CanonicalDigest != expectDigest { fmt.Fprintf(os.Stderr, "error: digest mismatch (have %s)\n", art.CanonicalDigest); os.Exit(1) }
  ...
  for _, s := range art.Signers {
    emb := ""
    if rec := r.FindByID(s.ID); rec != nil && rec.Ed25519 != nil { emb = "pub" }
    fmt.Printf("  - %s (%s) weight=%d sig=%v %s\n", s.ID, s.Alg, s.Weight, s.Signature != "", emb)
  }
  if len(failures) > 0 { fmt.Printf("Failures: %v\n", failures) }
  for alg, w := range byAlg { fmt.Printf("Alg %s verified weight: %d\n", alg, w) }
  if !thresholdMet { os.Exit(3) }
}
