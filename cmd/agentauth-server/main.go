...
func main() {
	...
	fmt.Println("======================")
	enableProm := config.Get("AGENTAUTH_METRICS", "") == "prometheus"
	if enableProm {
		fmt.Println("[metrics] Prometheus metrics enabled (endpoint /metrics)")
	} else {
		fmt.Println("[metrics] Prometheus metrics disabled (set AGENTAUTH_METRICS=prometheus to enable)")
	}

	// Configuration via environment (educational – NOT production secure)
	authURL := config.Get("AGENTAUTH_AUTH_SERVER_URL", "https://auth.example.com")
	clientID := config.Get("AGENTAUTH_CLIENT_ID", "demo-client")
	clientSecret, generatedClient, warnClient := config.EphemeralSecret("AGENTAUTH_CLIENT_SECRET", 24)
	if warnClient != "" {
	...
	gconfig := agentauth.Config{
		...
		RateLimit:         agentauth.Config{}.RateLimit, // Placeholder
	}
	// NOTE: Current agentauth.New does not accept instrumentation options; Prometheus adapter
	// is initialized solely to expose process metrics (no internal hook here yet).
	if enableProm {
		_ = imetrics.NewPrometheusMetrics(imetrics.PrometheusAdapterOptions{Namespace: "agentauth", Subsystem: "core"})
	}
	authService, err := agentauth.New(gconfig)
	if err != nil {
	...
	transaction := agentauth.TransactionDetails{
		ID:          "tx-12345",
		Type:        agentauth.PaymentTransaction,
		Status:      agentauth.TransactionPending,
		ClientID:    "demo-client",
	...
	resourceServer := agentauth.NewResourceServer("demo-resource", authService)
	...
	if enableProm {
		...
		go func() {
			addr := os.Getenv("AGENTAUTH_METRICS_ADDR")
			if addr == "" {
				addr = ":9095"
			}
