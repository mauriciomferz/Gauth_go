...
func TestMultiSigBenchPercentiles(t *testing.T) {
    ...
    if len(lines) != 1 { t.Fatalf("expected 1 JSON line; got %d", len(lines)) }
    recs := parseRecords(t, lines)
    r := recs[0]
    // Presence checks
    for _, k := range []string{"p50_sign_ns", "p95_sign_ns", "p99_sign_ns", "p50_verify_ns", "p95_verify_ns", "p99_verify_ns"} {
        if _, ok := r[k]; !ok { t.Fatalf("missing percentile field %s", k) }
    }
    p50s := r["p50_sign_ns"].(float64)
    p95s := r["p95_sign_ns"].(float64)
    p99s := r["p99_sign_ns"].(float64)
    if !(p50s <= p95s && p95s <= p99s) { t.Fatalf("sign percentile ordering invalid: p50=%f p95=%f p99=%f", p50s, p95s, p99s) }
    p50v := r["p50_verify_ns"].(float64)
    p95v := r["p95_verify_ns"].(float64)
    p99v := r["p99_verify_ns"].(float64)
    if !(p50v <= p95v && p95v <= p99v) { t.Fatalf("verify percentile ordering invalid: p50=%f p95=%f p99=%f", p50v, p95v, p99v) }
}
