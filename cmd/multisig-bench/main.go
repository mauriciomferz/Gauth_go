...
func parseSignerList(raw string) ([]int, error) {
    if strings.TrimSpace(raw) == "" { return nil, fmt.Errorf("empty signers list") }
    ...
    for _, p := range parts {
        ...
        if p == "" { continue }
        var v int
        for _, ch := range p { if ch < '0' || ch > '9' { return nil, fmt.Errorf("invalid signer count: %s", p) } }
        _, err := fmt.Sscanf(p, "%d", &v)
        if err != nil || v <= 0 { return nil, fmt.Errorf("invalid signer count: %s", p) }
    ...
    for i := 0; i < len(out); i++ {
        for j := i+1; j < len(out); j++ { if out[j] < out[i] { out[i], out[j] = out[j], out[i] } }
    }
    return out, nil
}
...
func main() {
    ...
    msg := make([]byte, 256)
    if _, err := rng.Read(msg); err != nil { fmt.Fprintf(os.Stderr, "rng read error: %v\n", err); os.Exit(1) }

    // Metrics collector (optional)
    var m imetrics.Metrics = imetrics.Noop
    mem, useMem := (&imetrics.Memory{}), false
    if emitMetrics { mem = imetrics.NewMemory(); m = mem; useMem = true }
    ...
    for _, signers := range signerCounts {
        ...
        for i := 0; i < signers; i++ {
            ...
            if kErr != nil { fmt.Fprintf(os.Stderr, "key gen error: %v\n", kErr); os.Exit(1) }
            keys[i] = priv
            pubs[i] = pub
        }
    ...
    verifySamples := make([]int64, 0, iterations)
        ...
        for iter := 0; iter < iterations; iter++ {
            ...
            if mode == "verify" || mode == "both" {
                ...
                if useMem { m.ObserveMultiSignatureVerificationLatency(time.Duration(durV) * time.Nanosecond) }
            }
        }
        ...
        if threshold > 0 { rec.Threshold = threshold }
    ...
    if summaryPath != "" {
        s := summary{SignerGroups: len(signerCounts), TotalRecords: len(records), TotalSigners: func() int { tot:=0; for _, v := range signerCounts { tot+=v }; return tot }(), Mode: mode, Iterations: iterations, Timestamp: time.Now().UTC().Format(time.RFC3339Nano)}
        sb, _ := json.MarshalIndent(&s, "", "  ")
        if err := os.WriteFile(summaryPath, sb, 0644); err != nil { fmt.Fprintf(os.Stderr, "write summary error: %v\n", err) }
    }
...
func computePercentiles(samples []int64) (p50, p95, p99 int64) {
    ...
    for i := 1; i < n; i++ {
        j := i
        for j > 0 && cp[j] < cp[j-1] { cp[j], cp[j-1] = cp[j-1], cp[j]; j-- }
    }
