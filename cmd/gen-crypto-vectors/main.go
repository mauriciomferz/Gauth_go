...
// - ECDSA: deterministic private key & deterministic nonce source (pseudo-random HMAC-SHA256 expansion).
// - BLS: uses CSPRNG (library lacks documented deterministic setter here) -> non-deterministic (acceptable for fixture snapshot; rerun will differ).
//
// Usage:
...
func deterministicECDSAPriv(seed string) *ecdsa.PrivateKey {
    ...
    n := curve.Params().N
    d.Mod(d, n)
    if d.Sign() == 0 { d.Add(d, big.NewInt(1)) }
...
func (r *deterministicReader) refill() {
    ...
    for i := 0; i < 8; i++ { ctrBytes[i] = byte(r.ctr >> (56 - 8*i)) }
    mac.Write(ctrBytes)
    r.buf = mac.Sum(nil)
    r.ctr++
}
...
// encodeDERSignature replicates internal canonical encoding (SEQUENCE of r,s) with leading zero for negative avoidance.
func encodeDERSignature(r, s *big.Int) []byte {
    rb := r.Bytes(); sb := s.Bytes()
    if len(rb) > 0 && rb[0]&0x80 != 0 { rb = append([]byte{0x00}, rb...) }
    if len(sb) > 0 && sb[0]&0x80 != 0 { sb = append([]byte{0x00}, sb...) }
...
func normalizeLowS(s, n *big.Int) *big.Int { half := new(big.Int).Rsh(n,1); if s.Cmp(half)==1 { return new(big.Int).Sub(n,s) }; return s }
...
func main() {
    ...
    badEd := append([]byte(nil), edSig...); if len(badEd)>0 { badEd[0]^=0xFF }
    ...
    if err != nil { fmt.Fprintf(os.Stderr, "ecdsa sign error: %v\n", err); os.Exit(1) }
    ...
    vectors = append(vectors, vector{Alg: "ECDSA-P256", MessageHex: hexOf(eMsg), PublicHex: hexOf(pubBytes), SignatureHex: hexOf(derHigh), Valid: false, Note: "high-S malleable variant"})
    // Truncated variant
    trunc := der[:len(der)/2]
    ...
    if err := bls.Init(bls.BLS12_381); err == nil {
        var sk bls.SecretKey; sk.SetByCSPRNG(); pk := sk.GetPublicKey()
        ...
        bad := append([]byte(nil), sig.Serialize()...); if len(bad)>0 { bad[0]^=0xAA }
        vectors = append(vectors, vector{Alg: "BLS12-381", MessageHex: hexOf(bMsg), PublicHex: hexOf(pk.Serialize()), SignatureHex: hexOf(bad), Valid: false, Note: "corrupted first byte"})
    } else {
