...
func main() {
	...
	case "rotation-v2-file":
		if *rotV2File == "" { err = errors.New("rotation-v2-file required"); break }
		res.Detail, err = auditRotationV2File(*rotV2File, *rotV2Prev)
	case "poa-verify":
		if *poaID == "" { err = errors.New("poa-id required") ; break }
		res.Detail, err = auditRemotePOA(*baseURL, *poaID)
	case "poa-file":
		if *poaFile == "" { err = errors.New("poa-file required") ; break }
		res.Detail, err = auditLocalPOA(*poaFile)
	case "attestation-file":
		if *attFile == "" { err = errors.New("attestation-file required") ; break }
	...
	if !res.Success { os.Exit(1) }
	if *verbose { fmt.Fprintf(os.Stderr, "completed in %dms\n", res.LatencyMs) }
}
...
func auditRotation(base string) (interface{}, error) {
	...
	if err := json.Unmarshal(body, &sum); err != nil { return nil, fmt.Errorf("decode_rotation_summary: %w", err) }
	reg := cryptoReg.GlobalEdDSARegistry
	if reg == nil { return nil, errors.New("eddsa_registry_unavailable") }
	// Verify all signatures collected.
	validCount := 0
	reasons := []string{}
	payload, err := canonicalRotationSummaryPayload(&sum)
	if err != nil { return nil, fmt.Errorf("canonical_payload_error: %w", err) }
	msg := append([]byte("GAUTH_ROTATION_SUMMARY:"), payload...)
	for _, sig := range sum.Signatures {
		k := reg.FindByID(sig.Kid)
		if k == nil { reasons = append(reasons, "kid_not_found:"+sig.Kid); continue }
		bsig, err := base64.RawURLEncoding.DecodeString(sig.Signature)
		if err != nil { reasons = append(reasons, "signature_decode:"+sig.Kid); continue }
		if ed25519.Verify(k.Public, msg, bsig) { validCount++ } else { reasons = append(reasons, "signature_invalid:"+sig.Kid) }
	}
...
func auditLocalAttestation(path string) (interface{}, error) {
	...
	if err != nil { return nil, err }
	var att Attestation
	if err := json.Unmarshal(data, &att); err != nil { return nil, fmt.Errorf("decode_attestation: %w", err) }
...
func auditRemoteAttestation(base, id string) (interface{}, error) {
	...
	if err != nil { return nil, err }
	var att Attestation
	if err := json.Unmarshal(body, &att); err != nil { return nil, fmt.Errorf("decode_attestation: %w", err) }
	return verifyAttestation(&att), nil
}
...
func verifyAttestation(att *Attestation) map[string]interface{} {
	if att == nil { return map[string]interface{}{"valid": false, "reason": "nil_attestation"} }
	...
	reg := cryptoReg.GlobalEdDSARegistry
	if reg == nil { result["valid"] = false; result["reason"] = "registry_unavailable"; return result }
	k := reg.FindByID(att.SigKid)
	if k == nil { result["valid"] = false; result["reason"] = "kid_not_found"; return result }
	// unsigned view
	...
	if err != nil { result["valid"] = false; result["reason"] = "signature_decode"; return result }
	valid := ed25519.Verify(k.Public, msg, sigBytes)
	result["signature_valid"] = valid
	// Local replay detection
	if att.Nonce != "" {
		if _, seen := attestationSeen.Load(att.Nonce); seen { result["replay"] = true } else { attestationSeen.Store(att.Nonce, struct{}{}) }
	}
...
func auditRevocation(base string) (interface{}, error) {
	...
	if err != nil { return nil, fmt.Errorf("revocation_head_fetch: %w", err) }
	var headResp struct {
	...
	if err := json.Unmarshal(headBody, &headResp); err != nil { return nil, fmt.Errorf("revocation_head_decode: %w", err) }
	...
	if headResp.Head == "" { result["error"] = "empty_head" }
...
func auditRevocationProof(base, id string, index int, hash string) (interface{}, error) {
	...
	if err != nil { return nil, fmt.Errorf("revocation_proof_fetch: %w", err) }
	var pr struct {
		Success    bool                         `json:"success"`
	...
	if err := json.Unmarshal(body, &pr); err != nil { return nil, fmt.Errorf("revocation_proof_decode: %w", err) }
	if !pr.Success { return nil, fmt.Errorf("revocation_proof_unsuccessful") }
	eventHash := hash
	if eventHash == "" { // need to fetch events listing to resolve ID or index to hash
		...
		if lerr != nil { return nil, fmt.Errorf("revocation_events_fetch: %w", lerr) }
		var evResp struct {
			Success bool `json:"success"`
			Events  []struct {
				ID    string `json:"id"`
		...
		if err := json.Unmarshal(lbody, &evResp); err != nil { return nil, fmt.Errorf("revocation_events_decode: %w", err) }
		found := false
		for _, e := range evResp.Events {
			if id != "" && e.ID == id { eventHash = e.Hash; found = true; break }
			if id == "" && index >= 0 && e.Index == index { eventHash = e.Hash; found = true; break }
		}
		if !found { return nil, fmt.Errorf("revocation_event_not_found") }
	}
...
func auditRevocationConsistency(base string, older, newer int) (interface{}, error) {
	if older < 0 || newer < 0 || older > newer { return nil, errors.New("invalid sizes (older/newer)") }
	...
	if err := json.Unmarshal(body, &resp); err != nil { return nil, fmt.Errorf("decode_consistency_sizes: %w", err) }
...
func auditRemotePOA(base, id string) (interface{}, error) {
	...
	if err := json.Unmarshal(body, &doc); err != nil { return nil, fmt.Errorf("decode_poa: %w", err) }
...
func auditLocalPOA(path string) (interface{}, error) {
	...
	if err != nil { return nil, err }
	var doc poa.ProofOfAuthorization
	if err := json.Unmarshal(data, &doc); err != nil { return nil, fmt.Errorf("decode_poa: %w", err) }
	return verifyPOA(&doc), nil
}
...
func httpGet(url string) ([]byte, error) {
	...
	if resp.StatusCode >= 300 { b, _ := io.ReadAll(resp.Body); return nil, fmt.Errorf("http_%d: %s", resp.StatusCode, string(b)) }
...
func canonicalRotationSummaryPayload(sum *notary.RotationSummary) ([]byte, error) {
	if sum == nil { return nil, errors.New("nil_summary") }
	payload := struct {
...
func auditRotationV2ArtifactJSON(data []byte, expectedPrev string) (interface{}, error) {
	...
	type signerEntry struct { Signer string `json:"signer"`; Alg string `json:"alg"`; Weight int `json:"weight"`; Signature string `json:"signature"`; Public string `json:"public,omitempty"` }
	var outer struct {
		Success bool `json:"success"`
		Artifact struct {
	...
	if err := json.Unmarshal(data, &outer); err != nil { return nil, fmt.Errorf("decode_rotation_v2: %w", err) }
	art := outer.Artifact
	...
	validSigners := []string{}
	for _, si := range s {
		if si.Signature == "" { continue }
		algUpper := strings.ToUpper(si.Alg)
		sigBytes, err := base64.RawURLEncoding.DecodeString(si.Signature)
		if err != nil { failures = append(failures, "signature_decode:"+si.Signer); continue }
		switch algUpper {
		case "ED25519":
			if si.Public == "" { failures = append(failures, "public_missing:"+si.Signer); continue }
			pubBytes, err := base64.RawURLEncoding.DecodeString(si.Public)
			if err != nil || len(pubBytes) != ed25519.PublicKeySize { failures = append(failures, "public_decode:"+si.Signer); continue }
		...
		case "ECDSA-P256", "ECDSA_P256", "ECDSA-P256-SHA256":
			if si.Public == "" { failures = append(failures, "public_missing:"+si.Signer); continue }
			pubBytes, err := base64.RawURLEncoding.DecodeString(si.Public)
			if err != nil || len(pubBytes) != 65 || pubBytes[0] != 0x04 { failures = append(failures, "public_decode:"+si.Signer); continue }
			x := new(big.Int).SetBytes(pubBytes[1:33])
			y := new(big.Int).SetBytes(pubBytes[33:65])
			curve := elliptic.P256()
			if !curve.IsOnCurve(x, y) { failures = append(failures, "public_invalid_point:"+si.Signer); continue }
			var rs struct{ R, S *big.Int }
			if _, err := asn1.Unmarshal(sigBytes, &rs); err != nil || rs.R == nil || rs.S == nil { failures = append(failures, "signature_decode:"+si.Signer); continue }
			h := sha256.Sum256(preimage)
			pk := ecdsa.PublicKey{Curve: curve, X: x, Y: y}
			if !ecdsa.Verify(&pk, h[:], rs.R, rs.S) { failures = append(failures, "signature_invalid:"+si.Signer); continue }
			verifiedWeight += si.Weight
			validSigners = append(validSigners, si.Signer)
		default:
			if si.Public != "" { failures = append(failures, "alg_unsupported:"+algUpper+":"+si.Signer) }
		}
	}
