...
func TestRotationV2EmbeddedPublicKeyVerification(t *testing.T) {
    ...
    if err := notary.AttachEd25519Signature(&art, privA, "sA", "ED25519", 3); err != nil { t.Fatalf("attach A: %v", err) }
    if err := notary.AttachEd25519Signature(&art, privB, "sB", "ED25519", 3); err != nil { t.Fatalf("attach B: %v", err) }
    ...
    for i := range art.Signers {
        ...
        case "sA": art.Signers[i].Public = base64.RawURLEncoding.EncodeToString(pubA)
        case "sB": art.Signers[i].Public = base64.RawURLEncoding.EncodeToString(pubB)
        }
    }
    ...
    if originalDigest != recomputed { t.Fatalf("digest changed after embedding public keys: orig=%s new=%s", originalDigest, recomputed) }

    // Auditor-style verification.
    verifiedWeight, failures, thresholdMet := auditorStyleVerify(&art)
    if verifiedWeight != 6 { t.Fatalf("expected verified weight 6 got %d failures=%v", verifiedWeight, failures) }
    if !thresholdMet { t.Fatalf("expected threshold met") }
...
func auditorStyleVerify(art *notary.WeightedRotationArtifact) (int, []string, bool) {
    if art == nil { return 0, []string{"artifact_nil"}, false }
    ...
    for _, s := range art.Signers {
        ...
        if err != nil || len(pubBytes) != ed25519.PublicKeySize { failures = append(failures, "public_decode:"+s.ID); continue }
        sigBytes, err := base64.RawURLEncoding.DecodeString(s.Signature)
        if err != nil || len(sigBytes) != ed25519.SignatureSize { failures = append(failures, "signature_decode:"+s.ID); continue }
        if !ed25519.Verify(ed25519.PublicKey(pubBytes), preimage, sigBytes) { failures = append(failures, "signature_invalid:"+s.ID); continue }
        verified += s.Weight
    }
