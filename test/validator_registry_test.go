...
func TestValidatorRegistrySuccess(t *testing.T) {
	...
	metrics := vr.Snapshot()
	if len(metrics) != 1 || metrics[0].Invocations == 0 { t.Fatalf("expected invocation metrics, got %#v", metrics) }
	if metrics[0].Failures != 0 { t.Fatalf("expected zero failures") }
}
...
func TestValidatorRegistryFailure(t *testing.T) {
	...
	_ = vr.Register("deny_region", func(req authz.Request, policy authz.Policy) error {
		if req.Context["region"] == "blocked" { return errors.New("region blocked") }
		return nil
	}, authz.WithTimeout(100*time.Millisecond))
	ma := authz.NewMemoryAuthorizer(); ma.SetValidatorRegistry(vr)
	...
	if !dec1.Allow { t.Fatalf("expected allow when region not blocked") }
	...
	if dec2.Allow { t.Fatalf("expected deny when validator fails") }
	m := vr.Snapshot()[0]
	if m.Failures == 0 { t.Fatalf("expected failure count >0") }
}
...
func TestValidatorRegistryTimeout(t *testing.T) {
	...
	if dec.Allow { t.Fatalf("expected deny due to timeout fail-closed") }
	m := vr.Snapshot()[0]
	if m.Failures == 0 { t.Fatalf("expected failure metric for timeout") }
}
