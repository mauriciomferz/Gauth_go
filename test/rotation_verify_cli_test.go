...
func TestRotationVerifyCLI(t *testing.T) {
	...
	if err != nil {
		...
		if tm, ok := parsed["threshold_met"].(bool); ok && !tm {
			t.Skip("skipping: threshold not met")
			return
		}
		if vw, ok := parsed["verified_weight"].(float64); ok && vw == 0 {
			t.Skip("skipping: no verified weight")
			return
		}
