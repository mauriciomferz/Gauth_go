...
func TestDelegationDepthLimitExceeded(t *testing.T) {
	...
	for i := 1; i <= 2; i++ {
		...
		if err != nil { t.Fatalf("unexpected error appending depth %d: %v", i, err) }
	}
	...
	if err == nil { t.Fatalf("expected depth exceeded error") }
	rfcErr, ok := err.(rfc.RFCError)
	if !ok { t.Fatalf("expected RFCError got %T", err) }
	if rfcErr.Code != rfc.ErrDelegationDepthExceeded { t.Fatalf("expected code %s got %s", rfc.ErrDelegationDepthExceeded, rfcErr.Code) }
...
func TestDelegationDepthLimitDisabled(t *testing.T) {
	...
	for i := 1; i <= 5; i++ {
		...
		if err != nil { t.Fatalf("unexpected error with unlimited depth: %v", err) }
	}
}
