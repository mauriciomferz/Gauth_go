...
func TestRevocationProofVerification(t *testing.T) {
    ...
    for _, evh := range events { tree.AppendLeaf(evh) }
    // Generate proof for index 2
    proof, root, err := tree.GenerateProof(2)
    if err != nil { t.Fatalf("generate proof: %v", err) }
    res := auditor.VerifyRevocationProof(events[2], proof, root)
    if !res.Included || res.Reason != "" { t.Fatalf("expected inclusion true with empty reason; got %+v", res) }
    if res.LeafDigest == "" { t.Fatalf("leaf digest empty") }
    ...
    if resBad.Included || resBad.Reason == "" { t.Fatalf("expected failure; got %+v", resBad) }
}
