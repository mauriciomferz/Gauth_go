...
func TestScopePatternsPrefixWildcard(t *testing.T) {
	scope := []string{"audit.*"}
	if !rfc0111.ScopeContains(scope, "audit.read.log") {
		t.Fatalf("expected prefix wildcard audit.* to match audit.read.log")
	}
	if rfc0111.ScopeContains(scope, "auditz.read.log") { // prefix mismatch
...
func TestScopePatternsRegexGate(t *testing.T) {
	...
	if rfc0111.ScopeContains(scope, "payment.create") {
	...
	if rfc0111.ScopeContains(scope, "payment.delete") {
...
func TestScopePatternsNumericRange(t *testing.T) {
	...
	if !rfc0111.ScopeContains(scope, "rate7") {
		t.Fatalf("expected rate7 within range [5-10]")
	}
	if rfc0111.ScopeContains(scope, "rate4") {
		t.Fatalf("did not expect rate4 below range [5-10]")
	}
	if rfc0111.ScopeContains(scope, "rate11") {
