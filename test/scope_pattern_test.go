...
func TestScopePatternsExactAndWildcard(t *testing.T) {
	...
	if !gauth_rfc_001.ScopeContains(scope, "any.action") {
		// Global wildcard should match everything
		t.Fatalf("expected global wildcard to match arbitrary action")
	}
	if !gauth_rfc_001.ScopeContains(scope, "read.user") {
		t.Fatalf("expected exact match for read.user")
	}
	if gauth_rfc_001.ScopeContains([]string{"read.user"}, "write.user") {
...
func TestScopePatternsPrefixWildcard(t *testing.T) {
	scope := []string{"audit.*"}
	if !gauth_rfc_001.ScopeContains(scope, "audit.read.log") {
		t.Fatalf("expected prefix wildcard audit.* to match audit.read.log")
	}
	if gauth_rfc_001.ScopeContains(scope, "auditz.read.log") { // prefix mismatch
...
func TestScopePatternsRegexGate(t *testing.T) {
	...
	if gauth_rfc_001.ScopeContains(scope, "payment.create") {
	...
	if gauth_rfc_001.ScopeContains(scope, "payment.delete") {
...
func TestScopePatternsNumericRange(t *testing.T) {
	...
	if !gauth_rfc_001.ScopeContains(scope, "rate7") {
		t.Fatalf("expected rate7 within range [5-10]")
	}
	if gauth_rfc_001.ScopeContains(scope, "rate4") {
		t.Fatalf("did not expect rate4 below range [5-10]")
	}
	if gauth_rfc_001.ScopeContains(scope, "rate11") {
