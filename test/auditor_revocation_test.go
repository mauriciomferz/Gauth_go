...
func TestAuditorRevocationHeadParsing(t *testing.T) {
	...
	if err := json.Unmarshal(data, &headResp); err != nil { t.Fatalf("decode failed: %v", err) }
	if headResp.Head != "abc123" || headResp.Length != 5 || !headResp.Verified { t.Fatalf("unexpected values: %+v", headResp) }
}
