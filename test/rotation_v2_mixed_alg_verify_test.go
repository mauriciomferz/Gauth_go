...
func (r *resolverBoth) FindByID(id string) *notary.PublicKeyRecord {
	switch id {
	case "ed1":
		return &notary.PublicKeyRecord{Ed25519: r.ed}
	case "ec1":
		return &notary.PublicKeyRecord{ECDSA: r.ec}
	default:
		return nil
	}
}
...
func (r *resolverEd) FindByID(id string) *notary.PublicKeyRecord {
	if id == "ed1" {
		return &notary.PublicKeyRecord{Ed25519: r.ed}
	}
	return nil
...
func TestRotationV2MixedAlgVerification(t *testing.T) {
	...
	// Attach signatures
	if err := notary.AttachEd25519Signature(&art, privEd, "ed1", "ED25519", 30); err != nil {
		t.Fatalf("attach ed25519: %v", err)
	}
	if err := notary.AttachECDSASignature(&art, privEc, "ec1", "ECDSA-P256", 40); err != nil {
		t.Fatalf("attach ecdsa: %v", err)
	}
	...
	if verified != 70 {
		t.Fatalf("expected verified weight 70 got %d failures=%v", verified, failures)
	}
	if len(failures) != 0 {
...
func TestRotationV2MixedAlgFailure(t *testing.T) {
	...
	for i := range art.Signers {
		if art.Signers[i].ID == "ec1" {
			art.Signers[i].Signature = base64.RawURLEncoding.EncodeToString(fake)
		}
	}
	rs := &resolverEd{ed: pubEd}
	verified, _, failures := notary.VerifyArtifactSignatures(&art, rs)
	if verified != 30 {
		t.Fatalf("expected verified weight 30 got %d", verified)
	}
	...
	if !foundEc {
		t.Fatalf("missing ec1 signer post verification")
	}
}
