...
func TestPolicyExpressionBasic(t *testing.T) {
	...
	if !dec.Allow {
		t.Fatalf("expected allow, got deny: %v", dec)
	}
	...
	if dec2.Allow {
		t.Fatalf("expected deny due to expression mismatch")
	}
}
...
func TestPolicyExpressionInList(t *testing.T) {
	...
	if dec, _ := ma.Authorize(context.Background(), req); !dec.Allow {
		t.Fatalf("expected allow for tier gold")
	}
	req2 := authz.Request{Subject: "bob", Resource: "data:55", Action: "write", Context: map[string]string{"tier": "bronze"}}
	if dec, _ := ma.Authorize(context.Background(), req2); dec.Allow {
		t.Fatalf("expected deny for tier bronze")
	}
}
...
func TestPolicyExpressionParseError(t *testing.T) {
	...
	if dec, _ := ma.Authorize(context.Background(), req); dec.Allow {
		t.Fatalf("expected deny due to parse error fail-closed")
	}
}
...
func TestPolicyExpressionLimits(t *testing.T) {
	...
	if err == nil || ok {
		t.Fatalf("expected error & false due to token limit, got ok=%v err=%v", ok, err)
	}
}
...
func TestPolicyExpressionDebug(t *testing.T) {
	...
	if !ok {
		t.Fatalf("expected ok=true for expression, got false")
	}
}
...
func TestPolicyExpressionTokenDump(t *testing.T) {
	...
	if len(toks) < 8 {
		t.Fatalf("unexpected small token slice: %d", len(toks))
	}
	for i, tk := range toks {
		t.Logf("%d: typ=%d lit=%s", i, tk.Type, tk.Literal)
	}
}
