...
func TestRawPOAStreamBasic(t *testing.T) {
	...
	if len(chain.Items) != 2 { t.Fatalf("expected 2 items got %d", len(chain.Items)) }
	if len(chain.ChainHash) == 0 { t.Fatalf("missing chain hash") }
}
...
func TestRawPOAStreamLimits(t *testing.T) {
	...
	if err == nil { t.Fatalf("expected size limit error") }
}
...
func TestRawPOAStreamUnexpectedKey(t *testing.T) {
	...
	if idx == -1 { t.Skip("encoding changed – skip") }
	enc[idx+1] = 'z'
	_, err := poa.DecodeRawPOAStream(bytes.NewReader(enc), poa.DefaultStreamLimits)
	if err == nil { t.Fatalf("expected decode error for unexpected key") }
}
...
func TestRawPOAStreamPrevHashContinuity(t *testing.T) {
	...
	if err != nil { t.Fatalf("decode with continuity verify: %v", err) }
	if len(chain.Items) != 2 { t.Fatalf("expected 2 items") }
}
...
func TestRawPOAStreamPrevHashMismatch(t *testing.T) {
	...
	if err == nil { t.Fatalf("expected prev_hash mismatch error") }
}
...
func TestRawPOAStreamIndefiniteArray(t *testing.T) {
	...
	if err != nil { t.Fatalf("indefinite decode error: %v", err) }
	if len(chain.Items) != 2 { t.Fatalf("expected 2 items got %d", len(chain.Items)) }
	if chain.HashAlgo.String() != "sha256" { t.Fatalf("unexpected algo %s", chain.HashAlgo.String()) }
}
