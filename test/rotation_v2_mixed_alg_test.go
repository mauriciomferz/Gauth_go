...
func (r *rType) FindByID(id string) *notary.PublicKeyRecord { if id == "s1" { return &notary.PublicKeyRecord{Ed25519: r.key} }; return nil }
...
func TestRotationV2MixedAlgorithmsConfigDefaultAlg(t *testing.T) {
    ...
    if err != nil { t.Fatalf("build artifact: %v", err) }
    for _, s := range art.Signers { if s.Alg == "" { t.Fatalf("signer %s alg empty after build", s.ID) } }
    ...
    if err := notary.AttachEd25519Signature(&art, priv, "s1", "ED25519", 30); err != nil { t.Fatalf("attach sig: %v", err) }
    // Minimal resolver instance
    r := &rType{key: pub}
    verified, _, failures := notary.VerifyArtifactSignatures(&art, r)
    if verified != 30 { t.Fatalf("expected verified weight 30 got %d failures=%v", verified, failures) }
}
