...
func (r *singleEdResolver) FindByID(id string) *notary.PublicKeyRecord { if id == "s1" { return &notary.PublicKeyRecord{Ed25519: r.key} }; return nil }
...
func TestRotationV2ContinuityUpdatesPreviousHash(t *testing.T) {
    ...
    if err != nil { t.Fatalf("artifact1: %v", err) }
    if err := notary.AttachEd25519Signature(&art1, priv, "s1", "ED25519", 10); err != nil { t.Fatalf("attach1: %v", err) }
    verified1, _, _ := notary.VerifyArtifactSignatures(&art1, &singleEdResolver{key: pub})
    if verified1 != 10 { t.Fatalf("verified1 mismatch") }
    ...
    if err != nil { t.Fatalf("artifact2: %v", err) }
    if art2.PreviousArtifactHash == "" { t.Fatalf("expected previous hash present") }
    if art2.PreviousArtifactHash != art1.CanonicalDigest { t.Fatalf("continuity mismatch prev=%s expected=%s", art2.PreviousArtifactHash, art1.CanonicalDigest) }
}
