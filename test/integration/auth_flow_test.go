...
func setupTestAuth(t *testing.T) *agentauth.Service {
...
func TestCompleteAuthFlow(t *testing.T) {
	...
	server := agentauth.NewResourceServer("test-resource", auth)
...
func TestConcurrentTransactions(t *testing.T) {
	...
	// Get a valid token
	grant, _ := auth.InitiateAuthorization(agentauth.AuthorizationRequest{
		ClientID: "test-client",
		Scopes:   []string{"read", "write", "transaction:execute"},
	})

	tokenResp, _ := auth.RequestToken(agentauth.TokenRequest{
		GrantID: grant.GrantID,
		Scope:   grant.Scope,
	})
...
func TestResourceServerFailover(t *testing.T) {
	...
	servers := []*agentauth.ResourceServer{
		agentauth.NewResourceServer("server-1", auth),
		agentauth.NewResourceServer("server-2", auth),
		agentauth.NewResourceServer("server-3", auth),
	}
