...
func (s *Ed25519Signer) Sign(msg []byte) ([]byte, error) {
    if len(s.priv) != ed25519.PrivateKeySize { return nil, errors.New("ed25519_signer_no_private") }
...
func (s *ECDSASigner) Sign(msg []byte) ([]byte, error) {
    if s.priv == nil { return nil, errors.New("ecdsa_signer_no_private") }
    h := sha256.Sum256(msg)
...
func NewBLSSigner(priv *BLSKey) *BLSSigner {
    if priv == nil { return &BLSSigner{} }
    return &BLSSigner{priv: priv}
}

func (s *BLSSigner) Algo() string { return "BLS12-381" }
func (s *BLSSigner) Public() []byte {
    if s.priv != nil { return s.priv.Public.Serialize() }
    if s.pub != nil { return s.pub.Public.Serialize() }
    return nil
}
func (s *BLSSigner) Sign(msg []byte) ([]byte, error) {
    if s.priv == nil { return nil, errors.New("bls_signer_no_private") }
    return BLSSign(s.priv, msg)
}
func (s *BLSSigner) Verify(msg, sig []byte) bool {
    var k *BLSKey
    if s.priv != nil { k = s.priv } else { k = s.pub }
...
func decodeDERSignature(der []byte) (*big.Int, *big.Int, error) {
    if len(der) < 8 || der[0] != 0x30 { return nil, nil, errors.New("invalid_der_prefix") }
    // Basic length check
    seqLen := int(der[1])
    if seqLen+2 != len(der) { return nil, nil, errors.New("invalid_der_length") }
    // Expect INTEGER r
    if der[2] != 0x02 { return nil, nil, errors.New("missing_r_integer") }
    rLen := int(der[3])
    if 4+rLen+2 >= len(der) { return nil, nil, errors.New("r_length_out_of_bounds") }
    rStart := 4; rEnd := rStart + rLen
    // INTEGER s
    if der[rEnd] != 0x02 { return nil, nil, errors.New("missing_s_integer") }
    sLen := int(der[rEnd+1])
    sStart := rEnd + 2; sEnd := sStart + sLen
    if sEnd != len(der) { return nil, nil, errors.New("s_length_out_of_bounds") }
...
func normalizeLowS(s, n *big.Int) *big.Int {
    ...
    if s.Cmp(half) == 1 { return new(big.Int).Sub(n, s) }
    return s
}
