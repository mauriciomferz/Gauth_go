...
func TestBLSSimpleAggregatorMetrics(t *testing.T) {
    ...
    for i := 0; i < 4; i++ {
        ...
        if err := agg.Add(k.Public.Serialize(), sig); err != nil { t.Fatalf("add: %v", err) }
        pubKeys = append(pubKeys, k.Public.Serialize())
    }
    aSig, err := agg.Aggregate()
    if err != nil { t.Fatalf("aggregate: %v", err) }
    if !agg.Verify(msg, aSig, pubKeys) { t.Fatalf("verify failed") }
    // Basic assertions
    snap := m.SnapshotEx()
    if snap.MultiSignatureAggregateLatencyCount == 0 { t.Fatalf("expected aggregate latency count >0") }
    if snap.MultiSignatureBatchSizeMax < 4 { t.Fatalf("expected batch size max >=4 got %d", snap.MultiSignatureBatchSizeMax) }
    if snap.MultiSignatureVerifications == 0 { t.Fatalf("expected multi-signature verifications >0") }
    if snap.MultiSignatureVerificationFailures != 0 { t.Fatalf("unexpected failures") }
}
...
func TestBatchVerifyInstrumentedEd25519(t *testing.T) {
    ...
    if !BatchVerifyEd25519Instrumented(m, pubs, msgs, sigs) { t.Fatalf("batch verify ed25519 failed") }
    snap := m.SnapshotEx()
    if snap.MultiSignatureVerifications == 0 { t.Fatalf("expected verifications >0") }
    if snap.MultiSignatureBatchSizeMax < 3 { t.Fatalf("expected batch size max >=3") }
}
