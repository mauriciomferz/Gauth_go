...
func gfMulRaw(a, b byte) byte {
    ...
    for i := 0; i < 8; i++ {
        ...
        hi := a & 0x80
        a <<= 1
        if hi != 0 { a ^= 0x1b } // AES polynomial (x^8 + x^4 + x^3 + x + 1)
        b >>= 1
    }
    return res
}
...
func SplitSecret(secret []byte, n, t int, rnd io.Reader) ([]Share, error) {
    ...
    for i, sb := range secret {
        ...
        row[0] = sb
        if t > 1 {
            if _, err := io.ReadFull(rnd, row[1:]); err != nil { return nil, err }
        }
        coeffs[i] = row
    }
...
func Reconstruct(shares []Share, t int) ([]byte, error) {
    if len(shares) < t || t < 2 { return nil, ErrInsufficient }
    used := map[int]struct{}{}
    valueLen := len(shares[0].Value)
    for _, s := range shares {
        if s.Index < 1 || s.Index > 255 { return nil, ErrIndexOutOfRange }
        if len(s.Value) != valueLen { return nil, ErrReconstruction }
        if _, exists := used[s.Index]; exists { return nil, ErrDuplicateIndex }
        used[s.Index] = struct{}{}
    }
    subset := shares[:t]
    ...
    for bi := 0; bi < valueLen; bi++ {
        ...
        for i, si := range subset {
            ...
            invDen, ok := gfInv(den)
            if !ok { return nil, ErrReconstruction }
...
func ThresholdSignEd25519(shares []Share, t int, public ed25519.PublicKey, msg []byte) ([]byte, error) {
    ...
    if len(public) != ed25519.PublicKeySize { return nil, errors.New("invalid_public_key") }
    if len(seed) != ed25519.SeedSize { return nil, errors.New("invalid_seed_size") }
    priv := ed25519.NewKeyFromSeed(seed)
    // Ensure derived public matches provided (defense against corrupted shares)
    derived := priv.Public().(ed25519.PublicKey)
    if !ed25519Equal(derived, public) { return nil, errors.New("public_mismatch_after_reconstruct") }
