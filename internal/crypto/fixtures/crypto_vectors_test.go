	...
	for i, v := range vecs {
		...
		case "Ed25519":
			if len(pubBytes) != ed25519.PublicKeySize {
				t.Fatalf("[%d] bad ed25519 pub size", i)
			}
		...
		case "ECDSA-P256":
			if len(pubBytes) == 0 || pubBytes[0] != 0x04 {
				t.Fatalf("[%d] ecdsa pub must be uncompressed SEC1", i)
			}
			curve := elliptic.P256()
			...
			if !okDer {
				if v.Valid {
					t.Errorf("[%d] expected valid DER but parse failed", i)
				}
				continue
			}
		...
		case "BLS12-381":
			...
			if err := pk.Deserialize(pubBytes); err != nil {
				if v.Valid {
					t.Errorf("[%d] bls pub deserialize: %v", i, err)
				}
				continue
			}
			var sig bls.Sign
			if err := sig.Deserialize(sigBytes); err != nil {
				if v.Valid {
					t.Errorf("[%d] bls sig deserialize: %v", i, err)
				}
				continue
			}
