...
func BatchVerifyEd25519(publicKeys []interface{}, messages [][]byte, signatures [][]byte) bool {
	n := len(publicKeys)
	if n == 0 || len(messages) != n || len(signatures) != n { return false }
	for i := 0; i < n; i++ {
		pk, ok := publicKeys[i].(ed25519.PublicKey)
		if !ok || len(pk) != ed25519.PublicKeySize { return false }
		if !ed25519.Verify(pk, messages[i], signatures[i]) { return false }
	}
	return true
}
...
func BatchVerifyBLS(publicKeys []interface{}, messages [][]byte, signatures [][]byte) bool {
	...
	for i := 0; i < n; i++ {
		...
		if !ok { return false }
		if !BLSVerify(&BLSKey{Public: pk}, messages[i], signatures[i]) { return false }
	}
	return true
}
