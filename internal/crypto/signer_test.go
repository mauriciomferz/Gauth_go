...
func TestSignerEd25519RoundTrip(t *testing.T) {
    ...
    if !s.Verify(msg, sig) { t.Fatalf("verify failed") }
    // Negative: mutate one byte
    sig[0] ^= 0xFF
    if s.Verify(msg, sig) { t.Fatalf("verify should fail on mutated signature") }
}

func TestSignerECDSARoundTrip(t *testing.T) {
    priv, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
    if err != nil { t.Fatalf("gen ecdsa: %v", err) }
    ...
    if derr != nil { t.Fatalf("decode der: %v", derr) }
    sig2 := encodeDERSignature(r, vS)
    if !s.Verify(msg, sig2) { t.Fatalf("verify failed after re-encode") }
    // Negative: truncate
    bad := sig[:len(sig)/2]
    if s.Verify(msg, bad) { t.Fatalf("verify should fail on truncated sig") }
}
...
func TestSignerECDSAHighSRejection(t *testing.T) {
    ...
    sig, err := s.Sign(msg)
    if err != nil { t.Fatalf("sign: %v", err) }
    if !s.Verify(msg, sig) { t.Fatalf("verify failed for canonical low-S signature") }
    ...
    if highS.Cmp(half) != 1 { t.Fatalf("constructed highS not greater than n/2") }
    highSig := encodeDERSignature(r, highS)
    if s.Verify(msg, highSig) { t.Fatalf("verify should reject high-S variant") }
}
...
func TestSignerBLSRoundTrip(t *testing.T) {
    ...
    if err != nil { t.Fatalf("sign: %v", err) }
    if !signer.Verify(msg, sig) { t.Fatalf("verify failed") }
    // Negative: mutate first byte
    sig[0] ^= 0xAA
    if signer.Verify(msg, sig) { t.Fatalf("verify should fail after corruption") }
}
