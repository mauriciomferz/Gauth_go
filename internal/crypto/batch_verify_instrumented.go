...
func BatchVerifyEd25519Instrumented(m imetrics.Metrics, publicKeys []interface{}, messages [][]byte, signatures [][]byte) bool {
	...
	if n == 0 || len(messages) != n || len(signatures) != n {
		if m != nil {
			m.IncMultiSignatureVerificationFailures()
		}
		return false
	}
	for i := 0; i < n; i++ {
		...
		if !ok || len(pk) != ed25519.PublicKeySize {
			if m != nil {
				m.IncMultiSignaturePublicKeyMissing()
			}
			return false
		}
		if !ed25519.Verify(pk, messages[i], signatures[i]) {
			if m != nil {
				m.IncMultiSignatureInvalidSignatureFailures()
				m.IncMultiSignatureVerificationFailures()
			}
			return false
		}
	...
	if m != nil {
		m.ObserveMultiSignatureVerificationLatency(latency)
		m.ObserveMultiSignatureBatchSize(n)
		m.IncMultiSignatureVerifications()
	}
	return true
}
