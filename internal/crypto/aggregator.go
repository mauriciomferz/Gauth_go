...
func (a *BLSSimpleAggregator) Add(pubKey, sig []byte) error {
    if len(pubKey) == 0 || len(sig) == 0 { return errors.New("empty_inputs") }
    // Deserialize public key
    var pk bls.PublicKey
    if err := pk.Deserialize(pubKey); err != nil { return errors.New("invalid_pubkey") }
    // Verify individual signature before inclusion
    var s bls.Sign
    if err := s.Deserialize(sig); err != nil { return errors.New("invalid_signature") }
    if !s.VerifyByte(&pk, a.message) { return errors.New("sig_verify_fail") }
...
func (a *BLSSimpleAggregator) Aggregate() ([]byte, error) {
    if len(a.signatures) == 0 { return nil, errors.New("no_signatures") }
    start := time.Now()
    ...
    if err != nil {
        if a.m != nil { a.m.IncMultiSignatureVerificationFailures() }
        return nil, err
    }
    return agg, nil
}
...
func (a *BLSSimpleAggregator) Verify(msg []byte, aggSig []byte, pubKeys [][]byte) bool {
    if len(pubKeys) == 0 || len(aggSig) == 0 { return false }
    ...
    for _, raw := range pubKeys {
        ...
        if err := pk.Deserialize(raw); err != nil {
            if a.m != nil { a.m.IncMultiSignaturePublicKeyMissing() }
            return false
        }
        pks = append(pks, pk)
    }
    ...
    if a.m != nil {
        ...
        if ok { a.m.IncMultiSignatureVerifications() } else { a.m.IncMultiSignatureVerificationFailures() }
    }
    return ok
}
