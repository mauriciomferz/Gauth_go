...
func NewFilesystemProvider(root string, masterKey []byte) (*FilesystemProvider, error) {
	...
	if masterKey == nil {
		if b, err := os.ReadFile(keyPath); err == nil {
			...
			mk := make([]byte, 32)
			if _, err := rand.Read(mk); err != nil { return nil, err }
			if err := os.WriteFile(keyPath, []byte(hex.EncodeToString(mk)), 0o600); err != nil { return nil, err }
			masterKey = mk
		}
	}
...
func (p *FilesystemProvider) Backend() string { return "filesystem" }
...
func (p *FilesystemProvider) Store(key string, value []byte) error {
	if len(value) == 0 { return errors.New("empty secret value") }
	nonce := make([]byte, 12)
...
func (p *FilesystemProvider) Get(key string) ([]byte, error) {
	...
	if len(parts) != 2 { return nil, errors.New("corrupt secret file") }
...
func (p *FilesystemProvider) Delete(key string) error {
	if err := os.Remove(p.pathFor(key)); err != nil { return err }
	return nil
}
...
func (p *FilesystemProvider) Rotate(newMasterKey []byte) error {
	if len(newMasterKey) != 32 { return errors.New("new master key must be 32 bytes") }
	entries, err := os.ReadDir(p.root)
	if err != nil { return err }
	oldKey := p.masterKey
	for _, e := range entries {
		if !strings.HasSuffix(e.Name(), ".secret") { continue }
		path := filepath.Join(p.root, e.Name())
		// read & decrypt with old key
		raw, err := os.ReadFile(path)
		if err != nil { return err }
		parts := strings.SplitN(string(raw), ":", 2)
		if len(parts) != 2 { return errors.New("corrupt secret file during rotation") }
		...
		if _, err := io.ReadFull(rand.Reader, nonceNew); err != nil { return err }
		...
		if err := os.WriteFile(path, []byte(enc), 0o600); err != nil { return err }
	}
	// persist new master key
	if err := os.WriteFile(filepath.Join(p.root, "master.key"), []byte(hex.EncodeToString(newMasterKey)), 0o600); err != nil { return err }
