...
func EncodeECDSAP256Uncompressed(pk *ecdsa.PublicKey) string {
    if pk == nil || pk.Curve != elliptic.P256() || pk.X == nil || pk.Y == nil { return "" }
    xb := pk.X.Bytes(); yb := pk.Y.Bytes()
    if len(xb) > 32 || len(yb) > 32 { return "" }
    xpad := make([]byte, 32); copy(xpad[32-len(xb):], xb)
    ypad := make([]byte, 32); copy(ypad[32-len(yb):], yb)
...
func AttachEd25519Signature(art *WeightedRotationArtifact, priv ed25519.PrivateKey, signerID string, alg string, weight int) error {
    if art == nil { return fmt.Errorf("artifact nil") }
    if len(priv) != ed25519.PrivateKeySize { return fmt.Errorf("invalid ed25519 private key size") }
    // Ensure signer entry presence (weight checked earlier); add if missing.
    found := false
    for i := range art.Signers {
        if art.Signers[i].ID == signerID {
            found = true
            if art.Signers[i].Weight != weight { return fmt.Errorf("weight mismatch for existing signer") }
            if art.Signers[i].Alg != alg { return fmt.Errorf("alg mismatch for existing signer") }
...
func AttachECDSASignature(art *WeightedRotationArtifact, priv *ecdsa.PrivateKey, signerID string, alg string, weight int) error {
    if art == nil { return fmt.Errorf("artifact nil") }
    if priv == nil || priv.Curve != elliptic.P256() { return fmt.Errorf("invalid ecdsa private key") }
    found := false
    for i := range art.Signers {
        if art.Signers[i].ID == signerID {
            found = true
            if art.Signers[i].Weight != weight { return fmt.Errorf("weight mismatch for existing signer") }
            if !strings.EqualFold(art.Signers[i].Alg, alg) { return fmt.Errorf("alg mismatch for existing signer") }
            break
        }
    }
    if !found {
...
func MarshalCanonicalJSON(art *WeightedRotationArtifact) ([]byte, error) {
    if art == nil { return nil, fmt.Errorf("artifact nil") }
...
func LoadWeightsConfig(path string) (*WeightsConfig, error) {
    ...
    if cfg.SchemaVersion != 1 { return nil, fmt.Errorf("unsupported schema_version %d", cfg.SchemaVersion) }
    if cfg.ThresholdWeight <= 0 { return nil, errors.New("threshold_weight must be > 0") }
    if len(cfg.Signers) == 0 { return nil, errors.New("signers empty") }
    // Validate uniqueness & weights
    seen := map[string]struct{}{}
    total := 0
    for i, s := range cfg.Signers {
        if s.ID == "" { return nil, errors.New("signer id empty") }
        if s.Weight <= 0 { return nil, fmt.Errorf("signer %s weight must be >0", s.ID) }
        if _, ok := seen[s.ID]; ok { return nil, fmt.Errorf("duplicate signer id %s", s.ID) }
        seen[s.ID] = struct{}{}
        if s.Alg == "" { cfg.Signers[i].Alg = "ED25519" }
        total += s.Weight
    }
    if cfg.ThresholdWeight > total { return nil, fmt.Errorf("threshold_weight %d exceeds total signer weight %d", cfg.ThresholdWeight, total) }
    return &cfg, nil
}
...
func BuildArtifactFromConfig(cfg *WeightsConfig, prevHash string, now time.Time) (WeightedRotationArtifact, error) {
    if cfg == nil { return WeightedRotationArtifact{}, errors.New("config nil") }
            ...
            case "ECDSA-P256", "ECDSA_P256", "ECDSA-P256-SHA256":
                ...
                if rawMap != "" {
                    ...
                    for _, e := range entries {
                        ...
                        if len(parts) != 2 || parts[0] != s.ID { continue }
                        b, err := base64.RawURLEncoding.DecodeString(parts[1])
                        if err != nil || len(b) != 65 || b[0] != 0x04 { continue }
                        // Basic curve point validation
                        x := new(big.Int).SetBytes(b[1:33])
                        y := new(big.Int).SetBytes(b[33:65])
                        if !elliptic.P256().IsOnCurve(x, y) { continue }
    ...
    if embedFlag { rotationV2PublicKeysEmbedded.Inc() }
...
func VerifyArtifactSignatures(art *WeightedRotationArtifact, resolver PublicKeyResolver) (int, map[string]int, []string) {
    if art == nil { return 0, map[string]int{}, []string{"artifact_nil"} }
    var failures []string
    verified := 0
    ...
    for _, s := range art.Signers {
        ...
        if resolver == nil { failures = append(failures, "resolver_nil"); rotationV2SignatureFailures.WithLabelValues("resolver_nil").Inc(); continue }
        rec := resolver.FindByID(s.ID)
        algUpper := strings.ToUpper(s.Alg)
        sigBytes, err := base64.RawURLEncoding.DecodeString(s.Signature)
        if err != nil { failures = append(failures, "signature_decode"); rotationV2SignatureFailures.WithLabelValues("signature_decode").Inc(); rotationV2SignatureFailuresByAlg.WithLabelValues(algUpper, "signature_decode").Inc(); continue }
        switch algUpper {
        case "ED25519":
            if rec == nil || len(rec.Ed25519) != ed25519.PublicKeySize { failures = append(failures, "public_key_not_found"); rotationV2SignatureFailures.WithLabelValues("public_key_not_found").Inc(); rotationV2SignatureFailuresByAlg.WithLabelValues(algUpper, "public_key_not_found").Inc(); continue }
            if len(sigBytes) != ed25519.SignatureSize || !ed25519.Verify(rec.Ed25519, preimage, sigBytes) {
                failures = append(failures, "signature_invalid"); rotationV2SignatureFailures.WithLabelValues("signature_invalid").Inc(); rotationV2SignatureFailuresByAlg.WithLabelValues(algUpper, "signature_invalid").Inc(); continue }
            verified += s.Weight
            byAlgWeight[algUpper] += s.Weight
        case "ECDSA-P256", "ECDSA_P256", "ECDSA-P256-SHA256":
            if rec == nil || rec.ECDSA == nil || rec.ECDSA.Curve != elliptic.P256() { failures = append(failures, "public_key_not_found"); rotationV2SignatureFailures.WithLabelValues("public_key_not_found").Inc(); rotationV2SignatureFailuresByAlg.WithLabelValues(algUpper, "public_key_not_found").Inc(); continue }
            h := sha256.Sum256(preimage)
            var rs struct{ R, S *big.Int }
            if _, err := asn1.Unmarshal(sigBytes, &rs); err != nil || rs.R == nil || rs.S == nil { failures = append(failures, "signature_decode"); rotationV2SignatureFailures.WithLabelValues("signature_decode").Inc(); rotationV2SignatureFailuresByAlg.WithLabelValues(algUpper, "signature_decode").Inc(); continue }
            if !ecdsa.Verify(rec.ECDSA, h[:], rs.R, rs.S) { failures = append(failures, "signature_invalid"); rotationV2SignatureFailures.WithLabelValues("signature_invalid").Inc(); rotationV2SignatureFailuresByAlg.WithLabelValues(algUpper, "signature_invalid").Inc(); continue }
            verified += s.Weight
            byAlgWeight[algUpper] += s.Weight
        default:
    ...
    for alg, w := range byAlgWeight { rotationV2VerifiedWeightByAlg.WithLabelValues(alg).Set(float64(w)) }
