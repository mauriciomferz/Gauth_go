...
func TestLoadWeightsConfigValid(t *testing.T) {
    ...
    if _, err := tmp.WriteString(content); err != nil { t.Fatalf("write: %v", err) }
    tmp.Close()
    cfg, err := LoadWeightsConfig(tmp.Name())
    if err != nil { t.Fatalf("load: %v", err) }
    if cfg.ThresholdWeight != 3 { t.Fatalf("threshold mismatch") }
    ...
    if len(art.Signers) != 2 || art.Signers[0].ID != "a" || art.Signers[1].ID != "b" { t.Fatalf("unexpected signer order: %#v", art.Signers) }
    if art.CanonicalDigest == "" { t.Fatalf("digest empty") }
}
