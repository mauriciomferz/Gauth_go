...
func TestRotationLedgerEntrySigning(t *testing.T) {
    ...
    path := f.Name(); f.Close(); os.Remove(path); defer os.Remove(path)
    ...
    entries := led.Entries()
    if entries[0].Signature == "" || entries[1].Signature == "" { t.Fatalf("expected signatures present") }
    mismatches, invalid := VerifyRotationLedger(entries, false, func(kid string) ed25519.PublicKey {
        // single kid mapping
        return pubKey
    })
    if mismatches != 0 { t.Fatalf("unexpected mismatches=%d", mismatches) }
    if invalid != 0 { t.Fatalf("unexpected invalid sigs=%d", invalid) }
}
...
func TestRotationLedgerTamperDetect(t *testing.T) {
    ...
    if mismatches == 0 { t.Fatalf("expected mismatches after tamper") }
}