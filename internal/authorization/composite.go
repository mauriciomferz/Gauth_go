...
func CanonicalHash(a *CompositeAuthorizationArtifact) (string, error) {
    ...
    if a.DecisionAuthority != nil && len(a.DecisionAuthority.DecisionMatrix) > 0 {
        ...
        for k := range a.DecisionAuthority.DecisionMatrix { keys = append(keys, k) }
        sort.Strings(keys)
        ordered := make([][2]string, 0, len(keys))
        for _, k := range keys { ordered = append(ordered, [2]string{k, a.DecisionAuthority.DecisionMatrix[k]}) }
