...
func TestActivateSuccess(t *testing.T) {
    ...
    if state.CanonicalHash == "" { t.Fatalf("canonical hash empty") }
    if state.PreviousArtifactHash != "" { t.Fatalf("expected empty previous hash on first activation") }
}

func TestActivateConflict(t *testing.T) {
    m := &Manager{}
    first := minimalArtifact()
    if _, err := m.Activate(first); err != nil { t.Fatalf("first activation failed: %v", err) }
