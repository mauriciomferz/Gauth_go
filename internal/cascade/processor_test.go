...
func TestCascadeProcessor(t *testing.T) {
	createTestPOA := func(id, parentID string, depth int) *gauth_aap_001.PowerOfAttorney {
		now := time.Now().UTC()
	...
	setupHierarchy := func() (*memoryRepo, []*gauth_aap_001.PowerOfAttorney) {
		...
		poas := []*gauth_aap_001.PowerOfAttorney{root, child1, child2, grandchild1}
	...
	t.Run("suspend mode suspends descendants", func(t *testing.T) {
		...
		child1, _ := repo.Get("child1")
		if child1.Status != gauth_aap_001.POAStatusSuspended {
	...
	t.Run("depth limit works correctly", func(t *testing.T) {
		...
		grandchild1, _ := repo.Get("grandchild1")
		if grandchild1.Status != gauth_aap_001.POAStatusActive {
...
type memoryRepo struct {
	store map[string]*gauth_aap_001.PowerOfAttorney
}

func (m *memoryRepo) Create(p *gauth_aap_001.PowerOfAttorney) error {
	m.store[p.ID] = p
	return nil
}

func (m *memoryRepo) Get(id string) (*gauth_aap_001.PowerOfAttorney, bool) {
	p, ok := m.store[id]
	return p, ok
}

func (m *memoryRepo) ListByPrincipal(principal string) []*gauth_aap_001.PowerOfAttorney {
	var result []*gauth_aap_001.PowerOfAttorney
...
func (m *memoryRepo) Update(p *gauth_aap_001.PowerOfAttorney) error {
...
func (m *memoryRepo) ListDescendants(parentPoaID string, maxDepth int) ([]*gauth_aap_001.PowerOfAttorney, error) {
	if parentPoaID == "" {
		return []*gauth_aap_001.PowerOfAttorney{}, nil
	}
