...
func boolToStr(b bool) string { if b { return "1" }; return "0" }
...
func (s *SnapshotStore) Add(list []Capability, hash string) Snapshot {
    if s == nil { return Snapshot{Hash: hash, Timestamp: time.Now().UTC(), Capabilities: list} }
    s.mu.Lock()
    defer s.mu.Unlock()
    ...
    if len(s.ring) > s.capacity {
        ...
        for i, sn := range s.ring { s.index[sn.Hash] = i }
    }
    return snap
}
...
func (s *SnapshotStore) Get(hash string) (Snapshot, bool) {
    if s == nil { return Snapshot{}, false }
    s.mu.RLock()
    defer s.mu.RUnlock()
    idx, ok := s.index[hash]
    if !ok { return Snapshot{}, false }
...
func (s *SnapshotStore) Latest() (Snapshot, bool) {
    ...
    s.mu.RLock()
    defer s.mu.RUnlock()
    if len(s.ring) == 0 { return Snapshot{}, false }
...
func (s *SnapshotStore) ListHashes() []string {
    ...
    for _, sn := range s.ring { out = append(out, sn.Hash) }
    return out
}
