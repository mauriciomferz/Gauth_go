...
func ParseClaims(data []byte) (map[string]any, error) {
    ...
    if !ok || delim != '{' { return nil, errors.New("claims must be a JSON object") }
    ...
    for dec.More() {
        ...
        if err != nil { return nil, err }
        key, ok := tKey.(string)
        if !ok { return nil, errors.New("non-string key in claims object") }
        if _, exists := seen[key]; exists { return nil, &DuplicateKeyError{Key: key} }
        var v any
        if err := dec.Decode(&v); err != nil { return nil, err }
        if err := checkDepth(v, 0, 10); err != nil { return nil, fmt.Errorf("depth validation for key %s: %w", key, err) }
        claims[key] = v
        seen[key] = struct{}{}
    }
    // consume closing '}'
    if _, err := dec.Token(); err != nil { return nil, err }
    // Ensure no trailing non-whitespace content
    if dec.More() { return nil, errors.New("unexpected trailing content after claims object") }
    return claims, nil
}

// checkDepth ensures nesting depth does not exceed maxDepth.
func checkDepth(v any, current, maxDepth int) error {
    if current > maxDepth { return fmt.Errorf("max depth %d exceeded", maxDepth) }
    switch tv := v.(type) {
    case map[string]any:
        for _, inner := range tv {
            if err := checkDepth(inner, current+1, maxDepth); err != nil { return err }
        }
    case []any:
        for _, inner := range tv {
            if err := checkDepth(inner, current+1, maxDepth); err != nil { return err }
