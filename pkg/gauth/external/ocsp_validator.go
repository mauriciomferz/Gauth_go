...
func (v *OCSPValidator) ValidateCertificate(ctx context.Context, req *OCSPValidationRequest) (*OCSPValidationResult, error) {
	...
	if v.config.CacheEnabled {
		if cached := v.getFromCache(cacheKey); cached != nil {
			...
			v.populateResultFromResponse(result, cached, req.RequestID)
	...
	if ocspServer == "" {
		ocspServer = v.getOCSPServer(req.Certificate, req.IssuerCertificate)
	}
	...
	// Send OCSP request with retries
	...
	for attempt := 0; attempt <= v.config.MaxRetries; attempt++ {
		...
		ocspResp, lastErr = v.sendOCSPRequest(ctx, ocspServer, ocspReq)
		if lastErr == nil {
			break
		}
	}
	...
	if lastErr != nil {
		result.Errors = append(result.Errors, fmt.Sprintf("OCSP request failed: %v", lastErr))
		
		// Try CRL fallback
		if v.config.EnableCRLFallback {
			crlResult, err := v.validateWithCRL(ctx, req)
			if err == nil {
				crlResult.CRLFallback = true
				return crlResult, nil
			}
			result.Warnings = append(result.Warnings, fmt.Sprintf("CRL fallback also failed: %v", err))
		}
