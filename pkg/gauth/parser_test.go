...
func TestParseClaimsBasic(t *testing.T) {
    ...
    if claims["sub"] != "alice" { t.Fatalf("sub mismatch") }
    if m, ok := claims["meta"].(map[string]any); !ok || m["tier"] != "gold" { t.Fatalf("meta tier mismatch") }
}

func TestParseClaimsDuplicate(t *testing.T) {
    js := []byte(`{"a":1,"a":2}`)
    _, err := ParseClaims(js)
    if err == nil { t.Fatalf("expected duplicate key error") }
}
...
func TestParseClaimsDepthExceeded(t *testing.T) {
    ...
    if err == nil { t.Fatalf("expected depth error") }
}
