// Package gauth - JWE Key Registry
...
// Supports GAUTH_JWE_KEY_DIR for multi-key setup or single key pair
func (r *KeyRegistry) LoadKeysFromEnvironment() error {
	// Check for key directory first
	if keyDir := os.Getenv("GAUTH_JWE_KEY_DIR"); keyDir != "" {
	...
	if publicKeyPath == "" || privateKeyPath == "" {
		return fmt.Errorf("GAUTH_JWE_PUBLIC_KEY and GAUTH_JWE_PRIVATE_KEY must be set")
	}
...
// Assumes key IDs follow a sortable naming convention (e.g., gauth-prod-2025-11)
