...
func WithDescription(desc string) func(*ValidatorEntry) { return func(v *ValidatorEntry) { v.Description = desc } }
// WithVersion option.
func WithVersion(ver string) func(*ValidatorEntry) { return func(v *ValidatorEntry) { v.Version = ver } }
// WithTags option.
func WithTags(tags ...string) func(*ValidatorEntry) { return func(v *ValidatorEntry) { v.Tags = tags } }
// WithTimeout option.
func WithTimeout(d time.Duration) func(*ValidatorEntry) { return func(v *ValidatorEntry) { v.Timeout = d } }
...
func (vr *ValidatorRegistry) Invoke(id string, req Request, policy Policy) error {
	...
	for i, ub := range ve.latencyBuckets {
		if lat <= ub { atomic.AddUint64(&ve.bucketCounts[i], 1); break }
	}
	if err != nil { atomic.AddUint64(&ve.failures, 1) }
	return err
}
...
func (vr *ValidatorRegistry) Snapshot() []ValidatorMetrics {
	...
	for _, ve := range vr.entries {
		...
		for i, ub := range ve.latencyBuckets {
			...
			if cnt > 0 { vm.LatencyHistogram[ub] = cnt }
		}
		res = append(res, vm)
	}
	vr.mu.RUnlock()
