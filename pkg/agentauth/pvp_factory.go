...
func (f *PVPFactory) CreatePVP() (PowerVerificationPoint, error) {
	provider := strings.ToLower(os.Getenv("AGENTAUTH_PVP_PROVIDER"))
	...
	if f.productionMode && (provider == "" || provider == "mock") {
		return nil, fmt.Errorf(
			"production mode requires real identity verification provider. " +
				"Set AGENTAUTH_PVP_PROVIDER to one of: stripe, veriff, idemia, onfido, jumio")
	}
...
func NewMockPVPWithWarning() *MockPVPWithWarning {
	...
	fmt.Fprintf(os.Stderr, "║                                                                 ║\n")
	fmt.Fprintf(os.Stderr, "║ Set AGENTAUTH_PVP_PROVIDER to: stripe, veriff, idemia, onfido,    ║\n")
