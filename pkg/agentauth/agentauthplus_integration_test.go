...
func TestAgentAuthPlusIntegration_DelegationDepth(t *testing.T) {
	...
	t.Run("CreateDelegationChain", func(t *testing.T) {
		for i := 0; i < len(agentIDs)-1; i++ {
			delegation := &agentauthplus.AIDelegation{
				SourcePOAID:     poaID,
...
func TestAgentAuthPlusIntegration_CapabilityEnforcement(t *testing.T) {
	...
	t.Run("InsufficientCapabilityShouldFail", func(t *testing.T) {
		assessment := &agentauthplus.AICapabilityAssessment{
			AgentID:        agentID,
...
func TestAgentAuthPlusIntegration_FiduciaryViolations(t *testing.T) {
	...
	t.Run("MinorViolationShouldWarn", func(t *testing.T) {
		violation := &agentauthplus.FiduciaryDutyViolation{
			POAID:                poaID,
			AgentID:              agentID,
			DutyType:             "disclosure",
...
func TestAgentAuthPlusIntegration_ComplianceValidator(t *testing.T) {
	...
	t.Run("RequestValidationWithAgentAuthPlus", func(t *testing.T) {
		...
		request := &agentauth.ExtendedAuthorizationRequest{
			AuthorizationRequest: &agentauth.AuthorizationRequest{
...
func createTestAgentAuthPlusValidator(db *database.DB) *agentauth.AgentAuthPlusValidator {
	successorService := agentauthplus.NewPostgreSQLSuccessorService(db)
	delegationService := agentauthplus.NewPostgreSQLDelegationService(db)
	dualControlService := agentauthplus.NewPostgreSQLDualControlService(db)
	fiduciaryService := agentauthplus.NewPostgreSQLFiduciaryDutyService(db)
	...
	return agentauth.NewAgentAuthPlusValidator(
		successorService,
		delegationService,
		dualControlService,
		fiduciaryService,
...
func createMockChainValidator() *agentauth.AuthorizationChainValidator {
	// Create a mock chain validator for testing
	// In real tests, this would use proper mocks
	return agentauth.NewAuthorizationChainValidator(nil, nil, nil)
}
