...
// This test requires a running Postgres instance and AGENTAUTH_POSTGRES_DSN env var
func TestPostgresExtendedTokenStore_Cleanup(t *testing.T) {
	dsn := os.Getenv("AGENTAUTH_POSTGRES_DSN")
	if dsn == "" {
		t.Skip("Skipping Postgres cleanup test: AGENTAUTH_POSTGRES_DSN not set")
	}
