...
func TestAuditReplayDeterminism(t *testing.T) {
    ...
    if err := ml.Log(context.TODO(), ev); err != nil { t.Fatalf("log: %v", err) }
    if err := ml.VerifyChain(); err != nil { t.Fatalf("verify chain: %v", err) }
    metaReq, ok := ev.Metadata["request"].(map[string]interface{})
    if !ok { t.Fatalf("request metadata missing") }
    metaDec, ok := ev.Metadata["decision"].(map[string]interface{})
    if !ok { t.Fatalf("decision metadata missing") }
