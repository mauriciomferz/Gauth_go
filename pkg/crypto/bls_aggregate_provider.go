...
func newMultiBLSKeyProvider() *multiBLSKeyProvider { return &multiBLSKeyProvider{publics: make(map[string][]byte)} }

func (m *multiBLSKeyProvider) ActiveSigner() (Signer, error) { return nil, errors.New("no active signer for aggregate provider") }
...
func (m *multiBLSKeyProvider) VerifyWith(msg, sig []byte, keyID string) error { return errors.New("single verify unsupported for aggregate provider") }
...
func (m *multiBLSKeyProvider) GenerateBLSKeys(n int) (keyIDs []string, privs []bls.SecretKey, err error) {
    if n <= 0 { return nil, nil, errors.New("n must be >0") }
    if err := bls.Init(bls.BLS12_381); err != nil { return nil, nil, err }
    for i := 0; i < n; i++ {
...
func aggregateVerify(pubKeys []bls.PublicKey, message, aggSig []byte) bool {
    ...
    for i, pk := range pubKeys { if i == 0 { aggPK = pk } else { aggPK.Add(&pk) } }
    var sig bls.Sign
...
func init() {
    ...
    RegisterAlgorithm(Algorithm{Name: AlgoBLS12381Agg, Verify: func(canonical []byte, sigBase64 string, keyID string, kp KeyProvider) error {
        ...
        // Structural checks
        if len(messages) != 1 { return fmt.Errorf("%s: expected exactly 1 message", AlgoBLS12381Agg) }
        if len(signatures) != 1 { return fmt.Errorf("%s: expected exactly 1 aggregated signature", AlgoBLS12381Agg) }
        if len(keyIDs) == 0 { return fmt.Errorf("%s: no key IDs provided", AlgoBLS12381Agg) }
        if kp == nil { return fmt.Errorf("%s: missing key provider", AlgoBLS12381Agg) }
        msg := messages[0]
        aggSigBytes, err := base64.StdEncoding.DecodeString(signatures[0])
        if err != nil { return fmt.Errorf("%s: base64 decode aggregated signature: %w", AlgoBLS12381Agg, err) }
        // Collect and deserialize public keys
        var pubKeys []bls.PublicKey
        for _, kid := range keyIDs {
            pkRaw, algo, err := kp.PublicKey(kid)
            if err != nil { return fmt.Errorf("%s: public key lookup failed for %s: %w", AlgoBLS12381Agg, kid, err) }
            if algo != AlgoBLS12381 { return fmt.Errorf("%s: key %s algorithm mismatch: %s", AlgoBLS12381Agg, kid, algo) }
            var pk bls.PublicKey
            if err := pk.Deserialize(pkRaw); err != nil { return fmt.Errorf("%s: deserialize public key %s failed", AlgoBLS12381Agg, kid) }
