...
func (s *ecdsaSigner) Sign(msg []byte) ([]byte, error) {
    if s.priv == nil { return nil, errors.New("ecdsa: no private key") }
...
func NewInMemoryECDSAProvider() (*InMemoryECDSAProvider, error) {
    ...
    if err != nil { return nil, fmt.Errorf("generate ecdsa p256: %w", err) }
...
func (p *InMemoryECDSAProvider) ActiveSigner() (Signer, error) {
    if p.active == nil { return nil, errors.New("no active signer") }
    return p.active, nil
}
...
func (p *InMemoryECDSAProvider) VerifyWith(msg, sig []byte, keyID string) error {
    ...
    if isHighS(sv, pk.Params().N) { return errors.New("ecdsa: high-s signature rejected") }
    if !ecdsa.Verify(pk, h[:], r, sv) { return errors.New("ecdsa: invalid signature") }
    return nil
}
...
func (p *InMemoryECDSAProvider) Rotate() (string, error) {
    priv, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
    if err != nil { return "", fmt.Errorf("rotate generate ecdsa: %w", err) }
    keyID := deriveECDSAKeyID(&priv.PublicKey)
...
func init() {
    RegisterAlgorithm(Algorithm{Name: AlgoECDSAP256, Verify: func(canonical []byte, sigBase64 string, keyID string, kp KeyProvider) error {
        if kp == nil { return errors.New("ecdsa-p256: missing key provider") }
