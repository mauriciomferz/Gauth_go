...
func TestBLSSimpleAggregator(t *testing.T) {
	...
	for i := range pubKeys {
		if err := agg.Add(pubKeys[i], sigs[i]); err != nil {
	...
	if !agg.Verify(msg, aggSig, pubKeys) {
		t.Fatalf("aggregate verify failed")
	}
	// Negative: verify with missing pub key
	if agg.Verify(msg, aggSig, pubKeys[:2]) {
		t.Fatalf("verify should fail with subset pubkeys")
	}
}
