...
func TestKMSMetricsEmission(t *testing.T) {
    ...
    kms, err := NewMockKMS()
    if err != nil { t.Fatalf("new mock kms: %v", err) }

    // Exercise operations
    if _, err := kms.ActiveSigner(); err != nil { t.Fatalf("active signer: %v", err) }
    if _, _, err := kms.PublicKey(kms.active.keyID); err != nil { t.Fatalf("public key: %v", err) }
    if _, err := kms.Rotate(); err != nil { t.Fatalf("rotate: %v", err) }
    if _, err := kms.ListKeys(); err != nil { t.Fatalf("list keys: %v", err) }
