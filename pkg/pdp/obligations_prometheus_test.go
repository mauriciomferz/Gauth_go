...
func TestPrometheusObligationMetricsIntegration(t *testing.T) {
    ...
    mfs, err := reg.Gather()
    if err != nil { t.Fatalf("failed to gather prometheus metrics: %v", err) }

    // Helpers to locate metric families
    findMF := func(name string) *dto.MetricFamily {
        for _, mf := range mfs { if mf.GetName() == name { return mf } }
        return nil
    }

    // Mandatory failure counter
    mandMF := findMF("gauth_rfc0111_mandatory_obligation_failures_total")
    ...
    failMF := findMF("gauth_rfc0111_obligations_failed_total")
    ...
    execMF := findMF("gauth_rfc0111_obligations_executed_total")
    ...
    latMF := findMF("gauth_rfc0111_obligation_latency_seconds")
    if latMF == nil { t.Fatalf("missing obligation_latency_seconds metric family") }
    var foundCount bool
    for _, m := range latMF.Metric {
        if h := m.GetHistogram(); h != nil { if h.GetSampleCount() >= 2 { foundCount = true; break } }
    }
    if !foundCount { t.Fatalf("expected histogram sample count >=2 (two obligations) in obligation_latency_seconds") }
}
