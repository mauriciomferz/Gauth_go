...
func TestObligationsExecutionSuccessAndAudit(t *testing.T) {
    ...
    if !dec.Allow { t.Fatalf("expected allow decision") }
    if len(dec.Obligations) != 2 { t.Fatalf("expected 2 obligations, got %d", len(dec.Obligations)) }
    // metrics: 2 obligations executed
    if m.ObligationsExecuted() != 2 { t.Fatalf("expected obligations executed = 2, got %d", m.ObligationsExecuted()) }
    if m.ObligationsFailed() != 0 { t.Fatalf("expected obligations failed = 0, got %d", m.ObligationsFailed()) }
    // audit file lines
    f, err := os.Open(auditPath)
    if err != nil { t.Fatalf("failed reading audit file: %v", err) }
    ...
    for scanner.Scan() {
        ...
        if rec.Duration < 0 { f.Close(); t.Fatalf("negative duration_ms") }
    }
    f.Close()
    if count != 2 { t.Fatalf("expected 2 audit lines, got %d", count) }
    _ = os.Remove(auditPath)
}
...
func TestObligationsExecutionFailure(t *testing.T) {
    ...
    if err != nil { t.Fatalf("unexpected error: %v", err) }
    if !dec.Allow { t.Fatalf("expected allow decision") }
    if m.ObligationsExecuted() != 1 { t.Fatalf("expected executed=1 (only ok), got %d", m.ObligationsExecuted()) }
    if m.ObligationsFailed() != 1 { t.Fatalf("expected failed=1 (fail_me), got %d", m.ObligationsFailed()) }
    f, err := os.Open(auditPath)
    if err != nil { t.Fatalf("failed reading audit file: %v", err) }
    scanner := bufio.NewScanner(f)
    count := 0
    ...
    for scanner.Scan() {
        count++
        line := scanner.Text()
        if !strings.Contains(line, "duration_ms") { f.Close(); t.Fatalf("missing duration_ms: %s", line) }
        if strings.Contains(line, "fail_me") && strings.Contains(line, "\"success\":false") { failureSeen = true }
    }
    f.Close()
    if count != 2 { t.Fatalf("expected 2 audit lines, got %d", count) }
    if !failureSeen { t.Fatalf("expected failure audit entry for fail_me") }
    _ = os.Remove(auditPath)
}
...
func TestMandatoryFailureDeniesWhenConfigured(t *testing.T) {
    ...
    if err != nil { t.Fatalf("unexpected error: %v", err) }
    if dec.Allow { t.Fatalf("expected deny due to mandatory failure; got allow with reason %s", dec.Reason) }
    if !strings.Contains(dec.Reason, "mandatory obligation") { t.Fatalf("reason should mention mandatory failure: %s", dec.Reason) }
    if dec.Metadata["mandatory_obligation_failures"] != "critical_log" { t.Fatalf("metadata missing failures list: %+v", dec.Metadata) }
    _ = os.Remove(auditPath)
}
...
func TestMandatoryFailureDoesNotDenyWhenDisabled(t *testing.T) {
    ...
    if err != nil { t.Fatalf("unexpected error: %v", err) }
    if !dec.Allow { t.Fatalf("expected allow; engine not configured to deny on mandatory failure") }
    if _, ok := dec.Metadata["mandatory_obligation_failures"]; ok { t.Fatalf("metadata should not include mandatory failures when outcome not flipped") }
    _ = os.Remove(auditPath)
}
...
func TestObligationLatencyAndMandatoryMetrics(t *testing.T) {
    ...
    if dec.Allow { t.Fatalf("expected deny due to mandatory failure") }
    // Metrics: mandatory failure counter should be >=1
    if m.MandatoryObligationFailures() < 1 { t.Fatalf("expected mandatory obligation failures >=1, got %d", m.MandatoryObligationFailures()) }
    // Latency metrics: at least 2 observations
    snap := m.SnapshotEx()
    if snap.ObligationLatencyCount < 2 { t.Fatalf("expected at least 2 obligation latency observations, got %d", snap.ObligationLatencyCount) }
    if snap.ObligationLatencyTotalNS == 0 { t.Fatalf("expected non-zero total latency") }
    _ = os.Remove(auditPath)
}
...
func TestObligationsContextCancellation(t *testing.T) {
    ...
    if m.ObligationsFailed() == 0 { t.Fatalf("expected at least one failed due to cancellation") }
    if m.ObligationsExecuted()+m.ObligationsFailed() < 1 { t.Fatalf("expected some obligations processed") }
    f, err := os.Open(auditPath)
    if err != nil { t.Fatalf("failed reading audit file: %v", err) }
    scanner := bufio.NewScanner(f)
    found := false
    for scanner.Scan() {
        line := scanner.Text()
        if strings.Contains(line, "duration_ms") { found = true; break }
    }
    f.Close()
    if !found { t.Fatalf("expected at least one line with duration_ms") }
    _ = os.Remove(auditPath)
}
