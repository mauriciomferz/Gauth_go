...
//  2. Attacker authenticates to AgentAuth Server as User B (attacker's own identity)
...
func (s *Service) EnforceAgentSessionBinding(ctx context.Context, poa *PowerOfAttorney, sessionUser string) error {
	...
	// This is the core fix for CVE-2025-AGENTAUTH-001 (Impersonation Attack)
...
ctx context.Context, poa *PowerOfAttorney, requestedAction string, requestedAmount *float64,
	...
	for _, scopeItem := range poa.Scope {
		...
		// Feature-gated by AGENTAUTH_HIERARCHICAL_SCOPE=1 for backward compatibility
		if ctx.Value("AGENTAUTH_HIERARCHICAL_SCOPE") != nil {
