...
func (s *Service) VerifyAttestationProof(ctx context.Context, proof *attest.AttestationProof) error {
	...
	// Optional trust anchor enforcement (strict issuer binding). Enabled when AGENTAUTH_ATTEST_REQUIRE_TRUST_ANCHOR=1.
	requireAnchor := parseBoolEnv("AGENTAUTH_ATTEST_REQUIRE_TRUST_ANCHOR")
...
func lookupRawPOAHashAlgo() string {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AGENTAUTH_RAW_POA_CHAIN_HASH_ALGO")))
