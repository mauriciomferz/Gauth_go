...
// - AGENTAUTH_BATCH_VERIFY_WORKERS: Override parallel worker count
// - AGENTAUTH_BATCH_VERIFY_BLS: Enable BLS batch optimization (default 0)
func (s *Service) BatchVerifyTokens(req BatchVerifyTokensRequest) ([]BatchVerificationResult, error) {
	...
	if os.Getenv("AGENTAUTH_BATCH_VERIFY_PARALLEL") == "0" {
		parallel = false
	}
	useBLS := req.UseBLS || os.Getenv("AGENTAUTH_BATCH_VERIFY_BLS") == "1"

	maxWorkers := req.MaxWorkers
	if maxWorkers <= 0 {
		maxWorkers = 4 // Conservative default
		if envWorkers := os.Getenv("AGENTAUTH_BATCH_VERIFY_WORKERS"); envWorkers != "" {
