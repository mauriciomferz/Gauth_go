...
func (s *simpleFakeRegistry) FindByID(id string) *internalCrypto.Key {
	return &internalCrypto.Key{Public: s.pub, ID: "fuzz"}
}
...
func (f *fuzzReplayStrategy) Record(nonce string) {
	if len(f.seen) > 2048 {
		for k := range f.seen {
			delete(f.seen, k)
			break
...
func FuzzVerifyModelLimitsAttestation(f *testing.F) {
	...
	base.Snapshot.Hash = "sha256:deadbeef"
	base.Snapshot.GeneratedAt = time.Now().UTC().Format(time.RFC3339Nano)
	unsignedStruct := struct {
	...
	}{Success: base.Success, Configured: base.Configured, Snapshot: base.Snapshot}
	...
	noNonce := base
	noNonce.Nonce = ""
	missingNonceSeed, _ := json.Marshal(noNonce)
	f.Add(missingNonceSeed)
	// Invalid primary signature base64 seed
	badSig := base
	badSig.Signature = "@@not_base64@@"
	badSigB, _ := json.Marshal(badSig)
	f.Add(badSigB)
	// Inconsistent notarization (Success=false)
	inc := base
	inc.Notarization = &struct {
		Provider       string  `json:"provider"`
		Timestamp      string  `json:"timestamp"`
		LatencySeconds float64 `json:"latency_seconds"`
		Success        bool    `json:"success"`
	}{Provider: "mem", Timestamp: time.Now().UTC().Format(time.RFC3339Nano), LatencySeconds: 0.001, Success: false}
	incSeed, _ := json.Marshal(inc)
	f.Add(incSeed)
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if len(att.DomainPrefix) > 256 {
			att.DomainPrefix = att.DomainPrefix[:256]
		}
