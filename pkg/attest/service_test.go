...
func TestBuildUnsignedGeneratesNonce(t *testing.T) {
    ...
    if nonce1 == "" || nonce2 == "" { t.Fatalf("nonce empty") }
    if nonce1 == nonce2 { t.Fatalf("expected distinct nonces got same: %s", nonce1) }
    // Provided nonce preserved
    _, given := svc.BuildUnsigned(raw, "fixednonce")
    if given != "fixednonce" { t.Fatalf("expected provided nonce preserved got %s", given) }
}
...
func TestSignDomainSeparatedAndVerify(t *testing.T) {
    ...
    if kid == "" { t.Fatalf("kid empty") }
    active := internalCrypto.GlobalEdDSARegistry.Active()
    if active == nil || len(active.Public) != ed25519.PublicKeySize { t.Fatalf("active public invalid") }
    msg := append([]byte(prefix), raw...)
    if !ed25519.Verify(active.Public, msg, sig) { t.Fatalf("signature failed verification") }
}
...
func TestSignDomainSeparatedB64(t *testing.T) {
    ...
    if err != nil { t.Fatalf("sign b64 err: %v", err) }
    if b64 == "" { t.Fatalf("empty b64 signature") }
    sigBytes, decErr := base64.RawStdEncoding.DecodeString(b64)
    if decErr != nil { t.Fatalf("decode err: %v", decErr) }
    active := internalCrypto.GlobalEdDSARegistry.Active()
    msg := append([]byte(prefix), raw...)
    if !ed25519.Verify(active.Public, msg, sigBytes) { t.Fatalf("b64 signature fails verify") }
    if kid == "" { t.Fatalf("kid empty") }
}
...
func TestNotarizeAndSignModelLimits_WithNotarizationAndSigning(t *testing.T) {
    ...
    if err != nil { t.Fatalf("notarize+sign err: %v", err) }
    if res.Notarization == nil { t.Fatalf("expected notarization present") }
    if res.Notarization.Provider != "memory" { t.Fatalf("provider mismatch: %s", res.Notarization.Provider) }
    if res.Signature == "" || res.SigKid == "" { t.Fatalf("signature fields empty") }
    sigBytes, decErr := base64.RawStdEncoding.DecodeString(res.Signature)
    if decErr != nil { t.Fatalf("sig decode: %v", decErr) }
    active := internalCrypto.GlobalEdDSARegistry.Active()
    if active == nil { t.Fatalf("active nil") }
    prefixed := append([]byte(AttestationDomainPrefix), unsigned...)
    if !ed25519.Verify(active.Public, prefixed, sigBytes) { t.Fatalf("primary signature fails verification over domain-prefixed payload") }
}
...
func TestNotarizeAndSignModelLimits_NoKey(t *testing.T) {
    ...
    if err == nil { t.Fatalf("expected error no_active_key") }
    if res.Signature != "" { t.Fatalf("signature should be empty on error") }
}
...
func TestNotarizeAndSignModelLimits_NoPrivateMaterial(t *testing.T) {
    ...
    if err == nil { t.Fatalf("expected error no_private_material") }
}
...
func TestNotarizeAndSignModelLimits_DomainPrefixDualSignature(t *testing.T) {
    ...
    if err != nil { t.Fatalf("sign err: %v", err) }
    if res.Signature == "" { t.Fatalf("raw signature missing") }
    if res.DomainSignature == "" { t.Fatalf("domain signature missing") }
    if res.DomainPrefix != "GAUTH_ATTEST:" { t.Fatalf("domain prefix mismatch: %s", res.DomainPrefix) }
    ...
    active := internalCrypto.GlobalEdDSARegistry.Active()
    prefixed := append([]byte(AttestationDomainPrefix), unsigned...)
    if !ed25519.Verify(active.Public, prefixed, rawBytes) { t.Fatalf("primary signature verify fail (domain prefix)") }
    msg := append([]byte(res.DomainPrefix), unsigned...)
    if !ed25519.Verify(active.Public, msg, domBytes) { t.Fatalf("domain verify fail") }
}
...
func TestCanonicalizeModelLimitsUnsigned_StripsSignatureFields(t *testing.T) {
    ...
    if err != nil { t.Fatalf("canonicalize: %v", err) }
    var out map[string]any
    if json.Unmarshal(canon, &out) != nil { t.Fatalf("unmarshal canon") }
    for _, k := range []string{"signature","sig_kid","sig_mode","domain_signature","domain_prefix"} {
        if _, ok := out[k]; ok { t.Fatalf("expected %s removed", k) }
    }
    if out["success"] != true { t.Fatalf("success field lost") }
    if _, ok := out["snapshot"].(map[string]any); !ok { t.Fatalf("snapshot missing") }
}
