...
func TestSubDelegationDepth(t *testing.T) {
	...
	if rootResp.POA.ParentPOAID != "" || rootResp.POA.Depth != 0 { t.Fatalf("expected root depth=0 no parent, got depth=%d parent=%s", rootResp.POA.Depth, rootResp.POA.ParentPOAID) }
	...
	if err != nil { t.Fatalf("child1 create failed: %v", err) }
	if child1.POA.Depth != 1 || child1.POA.ParentPOAID != rootResp.POA.ID { t.Fatalf("expected child1 depth=1 parent=%s got depth=%d parent=%s", rootResp.POA.ID, child1.POA.Depth, child1.POA.ParentPOAID) }
	...
	if err != nil { t.Fatalf("child2 create failed: %v", err) }
	if child2.POA.Depth != 2 { t.Fatalf("expected child2 depth=2 got %d", child2.POA.Depth) }
	...
	if err != nil { t.Fatalf("child3 create failed: %v", err) }
	if child3.POA.Depth != 3 { t.Fatalf("expected child3 depth=3 got %d", child3.POA.Depth) }
	...
	if err == nil { t.Fatalf("expected depth exceed error creating child4") }
}
