...
func newMemorySigReplayStore() *memorySigReplayStore { return &memorySigReplayStore{seen: make(map[string]time.Time)} }
func (m *memorySigReplayStore) SeenSignature(k string) (bool, error) { if _, ok := m.seen[k]; ok { return true, nil }; return false, nil }
func (m *memorySigReplayStore) RecordSignature(k string, at time.Time) error { if _, ok := m.seen[k]; !ok { m.seen[k] = at }; return nil }
