...
func TestHierarchicalDigestRootAndChild(t *testing.T) {
	...
	if rootResp.POA.Version != 4 {
		...
		if rootResp.POA.Version != 4 { t.Fatalf("expected root Version=4 got %d", rootResp.POA.Version) }
	}
	...
	if childResp.POA.Version != 4 { t.Fatalf("expected child Version=4 got %d", childResp.POA.Version) }
	if childResp.POA.ParentDigest == "" { t.Fatalf("expected parent digest captured") }
	...
	if derr != nil { t.Fatalf("canonical root err: %v", derr) }
	childDig, _, derr2 := CanonicalPOADigest(&childResp.POA)
	if derr2 != nil { t.Fatalf("canonical child err: %v", derr2) }
	if rootDig == childDig { t.Fatalf("expected distinct digests root vs child") }
}
...
func TestHierDigestTamperParent(t *testing.T) {
	...
	if origSig == nil { t.Fatalf("child signature missing") }
	childResp.POA.ParentDigest = "deadbeef" // invalid digest
	newDig, _, derr := CanonicalPOADigest(&childResp.POA)
	if derr != nil { t.Fatalf("canonical after tamper err: %v", derr) }
	if newDig == origSig.DigestHex { t.Fatalf("expected digest change after tamper") }
}
