...
func TestPersistenceDurability(t *testing.T) {
	...
	if br, ok := svc1.repo.(*BoltRepository); ok {
		if err := br.Close(); err != nil { t.Fatalf("close repo failed: %v", err) }
	}
	...
	if !ok || poa2 == nil { t.Fatalf("poa not found after restart") }
	if poa2.Grantor != "alice" || poa2.Grantee != "bob" { t.Fatalf("poa data mismatch after restart: %+v", poa2) }
	...
	if err != nil { t.Fatalf("second create failed: %v", err) }
	if resp2.POA.ID == id { t.Fatalf("expected distinct ID for second POA") }

	// Reopen again to verify both entries remain
	if br2, ok := svc2.repo.(*BoltRepository); ok {
		if err := br2.Close(); err != nil { t.Fatalf("close repo2 failed: %v", err) }
	}
	...
	if !ok1 || !ok2 || found1 == nil || found2 == nil { t.Fatalf("expected both POAs present after second restart") }
}
