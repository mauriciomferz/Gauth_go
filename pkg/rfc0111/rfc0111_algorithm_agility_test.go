...
func TestAlgorithmAgility_Ed25519AndECDSA(t *testing.T) {
    ...
    if err != nil { t.Fatalf("ed25519 issuance failed: %v", err) }
    respEc, err := svcEc.CreateDelegationCtx(context.Background(), req)
    if err != nil { t.Fatalf("ecdsa issuance failed: %v", err) }

    if respEd.POA.Signature == nil || respEc.POA.Signature == nil { t.Fatalf("missing signatures") }
    if respEd.POA.Signature.Algorithm != cr.AlgoEd25519 { t.Fatalf("expected ed25519 algorithm recorded") }
    if respEc.POA.Signature.Algorithm != cr.AlgoECDSAP256 { t.Fatalf("expected ecdsa-p256 algorithm recorded") }
    ...
    if derrEd != nil || digEd != respEd.POA.Signature.DigestHex { t.Fatalf("ed25519 canonical digest mismatch") }
    digEc, _, derrEc := CanonicalPOADigest(&respEc.POA)
    if derrEc != nil || digEc != respEc.POA.Signature.DigestHex { t.Fatalf("ecdsa canonical digest mismatch") }

    // Verify signatures via registry dispatch using the service verify helper (internal path).
    if err := svcEd.verifyPOASignature(&respEd.POA); err != nil { t.Fatalf("ed25519 verification path failed: %v", err) }
    if err := svcEc.verifyPOASignature(&respEc.POA); err != nil { t.Fatalf("ecdsa verification path failed: %v", err) }
    ...
    if digTampered == respEc.POA.Signature.DigestHex { t.Fatalf("tampered digest unexpectedly matches original") }
    ...
    if err := svcEc.verifyPOASignature(&tampered); err == nil { t.Fatalf("expected verification failure for tampered POA") }
}
