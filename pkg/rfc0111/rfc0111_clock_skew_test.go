...
import (
	...
	rfc "github.com/Gimel-Foundation/GiFo-RFC-0150-Go-Implementation-of-GAuth-1.0/pkg/rfc"
)
...
func TestClockSkewTolerance(t *testing.T) {
    ...
    if err := repo.Create(poaFuture); err != nil { t.Fatalf("create future poa: %v", err) }
    ...
    if err := repo.Create(poaOutside); err != nil { t.Fatalf("create outside poa: %v", err) }
    ...
    if err := repo.Create(poaGrace); err != nil { t.Fatalf("create grace poa: %v", err) }
    ...
    if err := repo.Create(poaExpired); err != nil { t.Fatalf("create expired poa: %v", err) }
