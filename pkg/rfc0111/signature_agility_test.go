...
import (
	...
	signalgo "github.com/Gimel-Foundation/GiFo-RFC-0150-Go-Implementation-of-GAuth-1.0/internal/crypto/signalgo"
)
...
func TestSignatureAgilityEd25519Basic(t *testing.T) {
    ...
    msg := make([]byte, 64)
    if _, err := rand.Read(msg); err != nil { t.Fatalf("rand: %v", err) }
    sig, err := algo.Sign(priv, msg)
    if err != nil { t.Fatalf("sign failed: %v", err) }
    if len(sig) == 0 { t.Fatalf("empty signature returned") }
    if !algo.Verify(pub, msg, sig) { t.Fatalf("verification failed for valid signature") }
    // Negative: modify message
    badMsg := append([]byte{}, msg...)
    badMsg[0] ^= 0xFF
    if algo.Verify(pub, badMsg, sig) { t.Fatalf("verification succeeded for tampered message") }
}
...
func TestSignatureAgilityRegistryList(t *testing.T) {
    names := signalgo.List()
    foundEd := false
    for _, n := range names { if n == "Ed25519" { foundEd = true; break } }
    if !foundEd { t.Fatalf("Ed25519 not found in registry list: %v", names) }
}
...
func TestSignatureAgilityCanonicalDigestUnchanged(t *testing.T) {
    ...
    if err != nil { t.Fatalf("canonical digest failed: %v", err) }
    dig2, canon2, err2 := CanonicalPOADigest(poa)
    if err2 != nil { t.Fatalf("canonical digest second failed: %v", err2) }
    if dig1 != dig2 || !bytes.Equal(canon1, canon2) { t.Fatalf("canonical digest instability detected") }
}
