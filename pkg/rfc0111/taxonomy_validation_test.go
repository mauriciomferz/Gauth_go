...
func TestValidateTaxonomyPositive(t *testing.T) {
    ...
    if err := ValidateTaxonomy(p); err != nil { t.Fatalf("expected taxonomy valid: %v", err) }
    // Empty fields also valid
    p2 := &PowerOfAttorney{Version:3}
    if err := ValidateTaxonomy(p2); err != nil { t.Fatalf("empty taxonomy should be valid: %v", err) }
}
...
func TestValidateTaxonomyNegative(t *testing.T) {
    ...
    for i, c := range cases {
        if err := ValidateTaxonomy(&c); err == nil { t.Fatalf("case %d expected invalid taxonomy", i) }
    }
    // Version <3 should ignore invalid values (backward compatibility)
    legacy := &PowerOfAttorney{Version:2, AgentType:"alien"}
    if err := ValidateTaxonomy(legacy); err != nil { t.Fatalf("legacy version should ignore taxonomy validation: %v", err) }
}