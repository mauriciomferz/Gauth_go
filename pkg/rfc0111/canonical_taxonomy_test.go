...
func TestTaxonomyVersionUpgrade(t *testing.T) {
    ...
    if err != nil { t.Fatalf("digest v2 err: %v", err) }
    // Upgrade version only (no taxonomy values provided)
    base.Version = 3
    dV3, cV3, err := CanonicalPOADigest(base)
    if err != nil { t.Fatalf("digest v3 err: %v", err) }
    if dV2 == dV3 { t.Fatalf("expected digest change between V2 and V3 domain") }
    if !containsExactTax(cV3, "\"version\":\"3\"") { t.Fatalf("missing version=3 field: %s", string(cV3)) }
    if containsExactTax(cV3, "\"taxonomy\"") { t.Fatalf("unexpected taxonomy object when all fields empty: %s", string(cV3)) }
    if !containsExactTax(cV2, "\"version\":\"2\"") { t.Fatalf("missing version=2 field") }
}
...
func TestTaxonomyObjectPresence(t *testing.T) {
    ...
    if !containsExactTax(c1, "\"taxonomy\":") { t.Fatalf("expected taxonomy object present") }
    if !containsExactTax(c1, "\"agent_type\":\"human\"") { t.Fatalf("agent_type missing: %s", string(c1)) }
    if !containsExactTax(c1, "\"sector\":\"finance\"") { t.Fatalf("sector missing") }
    if containsExactTax(c1, "action_class") { t.Fatalf("unexpected action_class key when empty") }
    ...
    if err != nil { t.Fatalf("digest2 err: %v", err) }
    if d1 == d2 { t.Fatalf("digest unchanged after adding action_class") }
    if !containsExactTax(c2, "\"action_class\":\"read_ops\"") { t.Fatalf("missing action_class value") }
}
...
func TestTaxonomyMultiSigDomain(t *testing.T) {
    ...
    if err != nil { t.Fatalf("digest err: %v", err) }
    if !containsExactTax(cWithTax, "taxonomy") { t.Fatalf("expected taxonomy in canonical") }
    ...
    if err != nil { t.Fatalf("digest2 err: %v", err) }
    if dWithTax == dNoTax { t.Fatalf("digest unchanged after removing taxonomy values") }
    if containsExactTax(cNoTax, "taxonomy") { t.Fatalf("taxonomy unexpectedly present after clearing values") }
    // Downgrade to Version=2 (still multi-sig) -> digest changes again; taxonomy cannot appear.
    poa.Version = 2
    dV2, cV2, err := CanonicalPOADigest(poa)
    if err != nil { t.Fatalf("digest v2 err: %v", err) }
    if dV2 == dNoTax { t.Fatalf("expected different digest after version downgrade") }
    if containsExactTax(cV2, "taxonomy") { t.Fatalf("taxonomy should not appear for Version=2") }
}
...
func TestTaxonomyBackwardCompatibility(t *testing.T) {
    ...
    if err != nil { t.Fatalf("digest legacy err: %v", err) }
    if containsExactTax(cLegacy, "taxonomy") { t.Fatalf("taxonomy should not appear for Version<3 canonical JSON") }
    // Upgrade to Version=3 -> taxonomy now appears and digest changes.
    poa.Version = 3
    dUp, cUp, err := CanonicalPOADigest(poa)
    if err != nil { t.Fatalf("digest upgraded err: %v", err) }
    if dLegacy == dUp { t.Fatalf("digest unchanged after upgrading to V3 with taxonomy inclusion") }
    if !containsExactTax(cUp, "taxonomy") { t.Fatalf("expected taxonomy object after version upgrade") }
    ...
    if err != nil { t.Fatalf("digest ms legacy err: %v", err) }
    if containsExactTax(cMsLegacy, "taxonomy") { t.Fatalf("taxonomy should not appear in multi-sig Version=2") }
    ...
    if err != nil { t.Fatalf("digest ms upgraded err: %v", err) }
    if dMsLegacy == dMsUp { t.Fatalf("digest unchanged after multi-sig taxonomy upgrade") }
    if !containsExactTax(cMsUp, "taxonomy") { t.Fatalf("expected taxonomy in multi-sig Version=3 canonical") }
}
