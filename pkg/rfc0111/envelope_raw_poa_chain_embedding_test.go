...
func TestRawPOAChainEmbeddingEnabled(t *testing.T) {
    ...
    if cErr != nil { t.Fatalf("decode chain: %v", cErr) }
    if len(chain.Items) != 1 { t.Fatalf("expected 1 item, got %d", len(chain.Items)) }
    if chain.Items[0].Issuer != "g1" || chain.Items[0].Subject != "u1" { t.Fatalf("unexpected item fields") }
}
...
func TestRawPOAChainEmbeddingSizeLimit(t *testing.T) {
    ...
    holder, err := decryptEnvelopeV2ForTest(svc, resp.AuthToken)
    if err != nil { t.Fatalf("decrypt: %v", err) }
    if holder.RawPOAChain != "" { t.Fatalf("expected RawPOAChain omitted due to size cap") }
