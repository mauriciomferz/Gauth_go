...
func TestCanonicalVersionWeightsPresence(t *testing.T) {
    ...
    if err != nil { t.Fatalf("digest single err: %v", err) }
    ...
    if len(d1) == 0 { t.Fatalf("empty digest") }
    ...
    if err2 != nil { t.Fatalf("digest multi err: %v", err2) }
    if d1 == d2 { t.Fatalf("expected different digest for multi-sig domain vs single-sig") }
    ...
    if err3 != nil { t.Fatalf("digest modified weights err: %v", err3) }
    if d2 == d3 { t.Fatalf("digest unchanged after weight modification") }
    if !containsExact(c3, "\"carol\":\"5\"") { t.Fatalf("updated weight not reflected in canonical: %s", string(c3)) }
}
...
func containsExactPermutedWeights(b []byte, w map[string]int) bool {
    ...
    var keys []string
    for k := range w { keys = append(keys, k) }
    a := keys[0]; bKey := keys[1]
