...
func ensureRegistry(t *testing.T) {
    if internalCrypto.GlobalEdDSARegistry == nil {
        ...
        for i := 0; i < 5; i++ { if _, err := km.Rotate(); err != nil { t.Fatalf("rotate %d: %v", i+1, err) } }
...
func TestPoAMultiSigThresholdSatisfied(t *testing.T) {
    ...
    for _, k := range keys { if k != nil { if _, ok := seen[k.ID]; !ok { uniq = append(uniq, k.ID); seen[k.ID] = struct{}{} } } }
    if len(uniq) < 2 { t.Fatalf("need at least 2 unique keys for test have=%d", len(uniq)) }
    // Use first two kids
    ...
    if len(poaObj.Signatures) != 2 || len(poaObj.SignerKids) != 2 { t.Fatalf("expected 2 signatures, got %d", len(poaObj.Signatures)) }
    vcount, satisfied, req := VerifyMultiSig(poaObj)
    if vcount != 2 || !satisfied || req != 2 { t.Fatalf("multisig verify mismatch valid=%d satisfied=%v req=%d", vcount, satisfied, req) }
}
...
func TestPoAMultiSigThresholdUnsatisfied(t *testing.T) {
    ...
    seen := map[string]struct{}{}
    for _, k := range keys { if k != nil { if _, ok := seen[k.ID]; !ok { uniq = append(uniq, k.ID); seen[k.ID] = struct{}{} } } }
    if len(uniq) < 2 { t.Fatalf("need at least 2 unique keys for unsatisfied test have=%d", len(uniq)) }
    os.Setenv("GAUTH_POA_MULTISIG_SIGN", "1")
    os.Setenv("GAUTH_POA_MULTISIG_KIDS", uniq[0] + "," + uniq[1])
    ...
    if err != nil { t.Fatalf("issue: %v", err) }
    vcount, satisfied, req := VerifyMultiSig(poaObj)
    if vcount >= req || satisfied || req != 3 { t.Fatalf("expected unsatisfied threshold vcount=%d req=%d satisfied=%v", vcount, req, satisfied) }
}

func TestPoAMultiSigTamperSignature(t *testing.T) {
    ensureRegistry(t)
    keys := internalCrypto.GlobalEdDSARegistry.ListCurrent()
    if len(keys) < 1 { t.Fatalf("need at least 1 key") }
    os.Setenv("GAUTH_POA_MULTISIG_SIGN", "1")
    ...
    if err != nil { t.Fatalf("issue: %v", err) }
    if len(poaObj.Signatures) != 1 { t.Fatalf("expected one signature") }
    ...
    if vcount != 0 || satisfied { t.Fatalf("tamper should invalidate signature valid=%d satisfied=%v", vcount, satisfied) }
}
