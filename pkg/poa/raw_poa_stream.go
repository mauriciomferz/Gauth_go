...
func (a RawPOAHashAlg) String() string {
	switch a {
	case RawPOAHashSHA256: return "sha256"
	case RawPOAHashBLAKE2b256: return "blake2b256"
	case RawPOAHashSHA3_256: return "sha3_256"
...
func EncodeRawPOAChain(items []RawPOAItem) ([]byte, error) {
	...
	for _, it := range items {
		...
		if len(b) > int(^uint32(0)) { return nil, errors.New("item too large to encode") }
		var lenBuf [4]byte
...
func marshalCBORItem(it RawPOAItem) ([]byte, error) {
	...
	if len(it.PrevHash) > 0 { pairsCount++ }
	if len(it.Claims) > 0 { pairsCount++ }
	// encode map length
	if pairsCount < 24 {
	...
	writeText := func(s string) {
		if len(s) < 24 { buf.WriteByte(0x60 | byte(len(s))) } else { buf.WriteByte(0x78); buf.WriteByte(byte(len(s))) }
		buf.WriteString(s)
	}
	writeBytes := func(b []byte) {
		if len(b) < 24 { buf.WriteByte(0x40 | byte(len(b))) } else { buf.WriteByte(0x58); buf.WriteByte(byte(len(b))) }
		buf.Write(b)
	}
	writeInt := func(i int64) {
		if i >= 0 && i < 24 { buf.WriteByte(0x00 | byte(i)) } else if i >= 0 && i < 256 { buf.WriteByte(0x18); buf.WriteByte(byte(i)) } else { return /* simplified for small timestamps */ }
	}
	// Emit simple kv
	for _, p := range kv { writeText(p[0]); writeText(p[1]) }
	// ts
	writeText("ts"); writeInt(it.Timestamp)
	// sig
	writeText("sig"); writeBytes(it.Signature)
	if len(it.PrevHash) > 0 { writeText("prev_hash"); writeBytes(it.PrevHash) }
	if len(it.Claims) > 0 {
		writeText("claims")
		// encode nested map of claims
		cCount := len(it.Claims)
		if cCount < 24 { buf.WriteByte(0xA0 | byte(cCount)) } else { return nil, errors.New("claims too many") }
		for k, v := range it.Claims { writeText(k); writeText(v) }
	}
...
func DecodeRawPOAStreamWith(r io.Reader, limits StreamLimits, algo RawPOAHashAlg, verifyPrev bool) (*RawPOAChain, error) {
	if limits.MaxItems == 0 { limits.MaxItems = DefaultStreamLimits.MaxItems }
	if limits.MaxItemBytes == 0 { limits.MaxItemBytes = DefaultStreamLimits.MaxItemBytes }
	if limits.MaxTotalBytes == 0 { limits.MaxTotalBytes = DefaultStreamLimits.MaxTotalBytes }
	h, err := newHash(algo)
	if err != nil { return nil, err }
	// Peek first byte to detect indefinite-length array start (0x9f)
	var first [1]byte
	n, _ := r.Read(first[:])
	if n == 0 { return &RawPOAChain{Items: nil, ChainHash: h.Sum(nil), HashAlgo: algo}, nil }
	...
	if first[0] != 0x9f {
		...
		for len(items) < limits.MaxItems {
			...
			if err == io.EOF { break }
			if err != nil { return nil, fmt.Errorf("length read: %w", err) }
			sz := int(binary.BigEndian.Uint32(lenBuf[:]))
			if sz <= 0 || sz > limits.MaxItemBytes { return nil, fmt.Errorf("invalid item size %d", sz) }
			if total+sz > limits.MaxTotalBytes { return nil, errors.New("exceeds total byte limit") }
			buf := make([]byte, sz)
			if _, err := io.ReadFull(bufReader, buf); err != nil { return nil, fmt.Errorf("item read: %w", err) }
			total += sz
			...
			if verifyPrev {
				...
				if err != nil { return nil, fmt.Errorf("decode item %d: %w", len(items), err) }
	...
	if err != nil { return nil, fmt.Errorf("read indefinite array: %w", err) }
	if len(rest) > limits.MaxTotalBytes { return nil, errors.New("exceeds total byte limit") }
	if len(rest) == 0 || rest[len(rest)-1] != 0xff { return nil, errors.New("indefinite array missing break code") }
	...
	for pos < len(content) && len(items) < limits.MaxItems {
		...
		hdr := content[pos]
		if hdr>>5 != 5 { return nil, fmt.Errorf("expected map major type at pos %d", pos) }
		mapLen := int(hdr & 0x1F)
		...
		if err != nil { return nil, fmt.Errorf("indefinite decode item %d: %w", len(items), err) }
		if consumed > limits.MaxItemBytes { return nil, errors.New("item exceeds MaxItemBytes") }
		rawMapBytes := content[start : start+consumed]
		if verifyPrev {
			if len(items) == 0 && len(it.PrevHash) > 0 { return nil, errors.New("first item must not contain prev_hash") }
...
func unmarshalMinimal(b []byte) (*RawPOAItem, error) {
	...
	if len(b) == 0 { return nil, errors.New("empty bytes") }
	if b[0]>>5 != 5 { return nil, errors.New("not CBOR map major type") }
	count := int(b[0] & 0x1F)
	pos := 1
	readText := func() (string, error) {
		if pos >= len(b) { return "", errors.New("trunc") }
		hdr := b[pos]; pos++
		if hdr>>5 != 3 { return "", errors.New("expected text") }
		ln := int(hdr & 0x1F)
		if ln == 24 { if pos >= len(b) { return "", errors.New("trunc") }; ln = int(b[pos]); pos++ }
		if pos+ln > len(b) { return "", errors.New("trunc") }
		s := string(b[pos: pos+ln]); pos += ln; return s, nil
	}
	readBytes := func() ([]byte, error) {
		if pos >= len(b) { return nil, errors.New("trunc") }
		hdr := b[pos]; pos++
		if hdr>>5 != 2 { return nil, errors.New("expected bytes") }
		ln := int(hdr & 0x1F)
		if ln == 24 { if pos >= len(b) { return nil, errors.New("trunc") }; ln = int(b[pos]); pos++ }
		if pos+ln > len(b) { return nil, errors.New("trunc len") }
		out := make([]byte, ln); copy(out, b[pos: pos+ln]); pos += ln; return out, nil
	}
	readInt := func() (int64, error) {
		if pos >= len(b) { return 0, errors.New("trunc") }
		hdr := b[pos]; pos++
		if hdr>>5 != 0 { return 0, errors.New("expected unsigned int") }
		val := int64(hdr & 0x1F)
		if val == 24 { if pos >= len(b) { return 0, errors.New("trunc") }; val = int64(b[pos]); pos++ }
		return val, nil
	}
	it := &RawPOAItem{Claims: make(map[string]string)}
	for i:=0; i<count; i++ {
		k, err := readText(); if err != nil { return nil, err }
		switch k {
		case "id": v, err := readText(); if err!=nil { return nil, err }; it.ID = v
		case "iss": v, err := readText(); if err!=nil { return nil, err }; it.Issuer = v
		case "sub": v, err := readText(); if err!=nil { return nil, err }; it.Subject = v
		case "algo": v, err := readText(); if err!=nil { return nil, err }; it.Algo = v
		case "ts": v, err := readInt(); if err!=nil { return nil, err }; it.Timestamp = v
		case "sig": v, err := readBytes(); if err!=nil { return nil, err }; it.Signature = v
		case "prev_hash": v, err := readBytes(); if err!=nil { return nil, err }; it.PrevHash = v
		case "claims":
			// nested map header
			if pos >= len(b) { return nil, errors.New("claims trunc") }
			h := b[pos]; pos++
			if h>>5 != 5 { return nil, errors.New("claims not map") }
			cCount := int(h & 0x1F)
			for j:=0; j<cCount; j++ {
				ck, err := readText(); if err!=nil { return nil, err }
				cv, err := readText(); if err!=nil { return nil, err }
				it.Claims[ck] = cv
			}
		default:
...
func unmarshalMinimalAt(b []byte, count int) (*RawPOAItem, int, error) {
	if len(b) == 0 { return nil, 0, errors.New("empty") }
	if b[0]>>5 != 5 { return nil, 0, errors.New("not map") }
	...
	it := &RawPOAItem{Claims: make(map[string]string)}
	for i:=0; i<count; i++ {
		k, err := readText(); if err != nil { return nil, 0, err }
		switch k {
		case "id": v, err := readText(); if err!=nil { return nil,0, err }; it.ID = v
		case "iss": v, err := readText(); if err!=nil { return nil,0, err }; it.Issuer = v
		case "sub": v, err := readText(); if err!=nil { return nil,0, err }; it.Subject = v
		case "algo": v, err := readText(); if err!=nil { return nil,0, err }; it.Algo = v
		case "ts": v, err := readInt(); if err!=nil { return nil,0, err }; it.Timestamp = v
		case "sig": v, err := readBytes(); if err!=nil { return nil,0, err }; it.Signature = v
		case "prev_hash": v, err := readBytes(); if err!=nil { return nil,0, err }; it.PrevHash = v
		case "claims":
			if pos >= len(b) { return nil,0, errors.New("claims trunc") }
			h := b[pos]; pos++
			if h>>5 != 5 { return nil,0, errors.New("claims not map") }
			cCount := int(h & 0x1F)
			for j:=0; j<cCount; j++ {
				ck, err := readText(); if err!=nil { return nil,0, err }
				cv, err := readText(); if err!=nil { return nil,0, err }
				it.Claims[ck] = cv
			}
		default:
