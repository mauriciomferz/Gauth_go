...
func TestCanonicalDigestStability(t *testing.T) {
	...
	if CanonicalDigest(poa) != orig {
		t.Fatalf("digest changed after metadata mutation")
	}
	// Mutate Attestation evidence map (excluded) should not change digest.
	if poa.Attestation == nil {
		t.Fatalf("attestation missing")
	}
	poa.Attestation.Evidence["extra"] = 42
	if CanonicalDigest(poa) != orig {
		t.Fatalf("digest changed after attestation evidence mutation")
	}
}
...
func TestCanonicalDigestTamper(t *testing.T) {
	...
	if !VerifyDigest(poa) {
		t.Fatalf("initial digest verify failed")
	}
	orig := poa.Digest
	// Tamper core field
	poa.Action = "delete"
	...
	if CanonicalDigest(poa) == orig {
		t.Fatalf("digest did not change after tamper")
	}
}
...
func TestCanonicalDigestDelegationIncluded(t *testing.T) {
	...
	if CanonicalDigest(poa) == d1 {
		t.Fatalf("digest did not change after delegation scope modification")
	}
}
