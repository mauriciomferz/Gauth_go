...
| T5 | Embedded PoA Verifier Helper | P1 | `pkg/aap001/embedded_verify.go` | Oversize PoA rejected; digest equivalence confirmed |
| T6 | Discovery Endpoint | P1 | `/well-known/agentauth/config` handler | Returns algorithms, required claims consistent with runtime config |
...
| Algo Agility | Digest invariance cross-algo | `pkg/aap001/algorithm_agility_test.go` |
...
| Embedded PoA | Size cap + digest equivalence | `pkg/aap001/embedded_verify_test.go` |
| Discovery | Schema + value correctness | `web/discovery_endpoint_test.go` |

## Metrics & Observability Enhancements (Post-Remediation)
- Add counters: `agentauth_replay_wal_recoveries_total`, `agentauth_replay_wal_errors_total`
- Histogram: `agentauth_token_validation_latency_seconds` (ensure p95 tracked)
- Gauge: `agentauth_ledger_head_age_seconds` after signature integration
...
## Rollback / Safety
- Each remediation feature gated by env flags (`AGENTAUTH_ENABLE_WAL`, `AGENTAUTH_ENABLE_ECDSA_STUB`, etc.).
