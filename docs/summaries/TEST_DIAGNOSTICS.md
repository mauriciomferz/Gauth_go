...
| Flag | Purpose |
|------|---------|
| AGENTAUTH_CAPABILITY_ENFORCE | Enable capability enforcement logic |
| AGENTAUTH_CAP_LIFECYCLE_SUNSET_ENFORCE | Deny usage after sunset timestamp |
| AGENTAUTH_SKIP_SMOKETEST | Skip CSP / smoketest hitting live server |
| AGENTAUTH_DISABLE_BG_POLLS | Suppress autosave/anomaly/metrics loops |
| SKIP_WEB_ASSETS | Skip JS bundling step (faster quiet test build) |
| AGENTAUTH_TEST_TRACE_HEARTBEAT_MS | Override heartbeat phase duration (default 120) when tracing |

Recommended single-test command (no background polls):

```bash
AGENTAUTH_DISABLE_BG_POLLS=1 AGENTAUTH_SKIP_SMOKETEST=1 go test -run ^TestCapabilitySunsetEnforcement$ -count=1 -v ./web
```

Background loops may be re-enabled (omit `AGENTAUTH_DISABLE_BG_POLLS`) when profiling metrics or anomaly sampling.
...
```
AGENTAUTH_TEST_TRACE_SIGPIPE=1 go test -run ^TestCapabilitySunsetEnforcement$ -count=1 -v ./web | head -n 15
```
...
3. Disable noisy background loops: `AGENTAUTH_DISABLE_BG_POLLS=1 AGENTAUTH_SKIP_SMOKETEST=1` reduces early log flood so essential lines appear before truncation.
4. Harness heartbeat: Set `AGENTAUTH_TEST_TRACE_SIGPIPE=1` to emit a `[trace] test harness starting` and timed heartbeats for early visibility (file: `web/testmain_trace.go`).
...
6. Heartbeat duration override: Adjust pre-test window with `AGENTAUTH_TEST_TRACE_HEARTBEAT_MS=250` (milliseconds).
...
```
SKIP_WEB_ASSETS=1 AGENTAUTH_DISABLE_BG_POLLS=1 AGENTAUTH_SKIP_SMOKETEST=1 go test -run ^TestCapabilitySunsetEnforcement$ -count=1 -v ./web
```
...
|------|----------------------------|
| See early harness start | `AGENTAUTH_TEST_TRACE_SIGPIPE=1` |
| Reduce noise | `AGENTAUTH_DISABLE_BG_POLLS=1 AGENTAUTH_SKIP_SMOKETEST=1` |
...
| Longer pre-test window | `AGENTAUTH_TEST_TRACE_HEARTBEAT_MS=500 AGENTAUTH_TEST_TRACE_SIGPIPE=1` |
