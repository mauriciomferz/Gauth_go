...
| Key rotation & lifecycle | `keyRing` with active/previous keys; env-driven scheduler & disk persistence; rotation log hash chain | Missing multi-tenant segregation, external HSM/KMS integration, rotation policy exposure API | Partial | P1 | Security | Integrate Vault/KMS provider; expose rotation schedule/status API; add tenant partitioning & external anchoring |
...
| Policy versioning & rollback | None | No version metadata in active evaluator | Missing | P1 | Governance | Add version field & retention store |
...
| Requirement | Current Implementation | Gap | Status | Priority | Impact | Suggested Action |
|-------------|------------------------|-----|--------|----------|--------|------------------|
| Full semantic validation (Parties/Scope/Requirements) | `BasicPoAValidator` (minimal invariants: non-wildcard self-delegation block, temporal ordering, jurisdiction & joint placeholder, numeric sanity checks) + `AdvancedPoAValidator` (transaction currency & 30d cap, wildcard gating, business-hour & weekday restriction syntax, inline multisig control, aggregate scope length limit). Runtime conditional evaluation (valid_hours / valid_weekdays) enforced in `VerifyToken`. Environment selection via `GAUTH_POA_VALIDATOR=basic advanced| Missing: warning channel, persistent daily limit counters/export, richer conditional language (amount/time expressions), modular validator chaining registry, advanced restriction audit metrics. | Partial | P0 | Compliance | Implement validator registry & warning emission, persist daily limit usage, add richer conditional DSL & audit/metrics instrumentation |
...
| Requirement | Current Implementation | Gap | Status | Priority | Impact | Suggested Action |
| Jurisdiction-specific enforcement | Static docs | No runtime branching | Missing | P1 | Compliance | Add `LegalFrameworkValidator` with rule sets |
...
| Requirement | Current Implementation | Gap | Status | Priority | Impact | Suggested Action |
| Secure secret storage | Environment / memory | No vault/HSM integration | Missing | P0 | Security | Integrate HashiCorp Vault/KMS provider |
