...
> Scope: AAP-001 / AAP-002 Compliance Enhancements

## Summary

This beta release focuses on deepening integrity, rotation invariants, revocation transparency, and reactive semantic safeguards aligned with AAP-001 (Attestation & Transparency) and AAP-002 (Proof-of-Authorization Definition).

## Key Additions

### Attestation Integrity (AAP-001)
...
- Signed rotation summary artifact with canonical serialization prefix: `AGENTAUTH_ROTATION_SUMMARY:`.
- Error codes: `rotation_continuity_gap`, `rotation_signature_missing`, `rotation_signature_invalid`.
- Anchoring optional via `AGENTAUTH_ANCHOR_ROTATIONS=1` (single anchoring per head hash).
...
### Reactive Semantic Throttle (AAP-002 Resilience Extension)
...
| Flag | Purpose |
|------|---------|
| `AGENTAUTH_ROTATIONS_SIGN=1` | Enables signing rotation descriptors & summary. |
| `AGENTAUTH_ANCHOR_ROTATIONS=1` | Anchors rotation summary once per unique head hash. |
| `AGENTAUTH_SEMANTIC_INTEGRITY_PERSIST_PATH` | File path for persisted hash chain continuity. |
| `AGENTAUTH_SEMANTIC_ANOMALY_Z_THRESHOLD` | Z-score threshold that activates semantic throttle. |
| `AGENTAUTH_ROTATION_LEDGER_PATH` | Path for rotation ledger persistence. |
...
### Multi-Signature Rotation Summary (AAP-002 Extension)
Runtime now supports multi-signature emission for rotation summaries when enabled:
| Flag | Behavior |
|------|----------|
| `AGENTAUTH_ROTATIONS_MULTISIG=1` | Aggregates signatures from all active Ed25519 keys into `Signatures[]`. |
| `AGENTAUTH_ROTATIONS_THRESHOLD` | Optional integer >0 declaring required signature quorum; if unmet returns 400 `rotation_threshold_unsatisfied`. |
...
4. Monitor throttle activations to tune `AGENTAUTH_SEMANTIC_ANOMALY_Z_THRESHOLD`.
