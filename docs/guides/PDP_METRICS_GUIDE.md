...
**Metric:** `agentauth_aap001_pep_enforcements_total`  
...
# Denied actions rate
sum(rate(agentauth_aap001_pep_enforcements_total{allowed="false"}[5m]))
...
sum(rate(agentauth_aap001_pep_enforcements_total[5m]) by (action_type)
```
...
**Metric:** `agentauth_aap001_pep_violations_total`  
**Type:** Counter (labeled)  
**Labels:**
...
sum(rate(agentauth_aap001_pep_violations_total[5m]))

# Critical violations rate (alerting threshold)
sum(rate(agentauth_aap001_pep_violations_total{severity="critical"}[5m]))

# High-severity violations by type
sum(rate(agentauth_aap001_pep_violations_total{severity=~"critical|high"}[5m]) by (violation_type)

# Violation severity distribution
sum(rate(agentauth_aap001_pep_violations_total[5m]) by (severity)
```
...
**Metric:** `agentauth_aap001_pep_enforcement_latency_seconds`  
**Type:** Histogram  
...
rate(agentauth_aap001_pep_enforcement_latency_seconds_sum[5m]) / 
rate(agentauth_aap001_pep_enforcement_latency_seconds_count[5m]))
```
...
(agentauth_aap001_pep_audit_buffer_violations / 10000) * 100

# Total audit entries in memory
agentauth_aap001_pep_audit_buffer_enforcements + agentauth_aap001_pep_audit_buffer_violations
```
...
- alert: HighEnforcementDenialRate
  expr: |
    sum(rate(agentauth_aap001_pep_enforcements_total{allowed="false"}[5m]) / 
    sum(rate(agentauth_aap001_pep_enforcements_total[5m]) > 0.5
  for: 5m
  labels:
    severity: warning
  annotations:
...
- alert: HighEnforcementLatency
  expr: |
    histogram_quantile(0.95, rate(agentauth_aap001_pep_enforcement_latency_seconds_bucket[5m]) > 0.1
  for: 5m
  labels:
    severity: warning
  annotations:
...
- alert: AuditBufferNearFull
  expr: |
    (agentauth_aap001_pep_audit_buffer_enforcements > 9000) or
    (agentauth_aap001_pep_audit_buffer_violations > 9000)
  for: 5m
  labels:
    severity: warning
  annotations:
...
func main() {
    ...
    auditLogger := agentauth.NewProductionPEPAuditLogger(
        10000,  // maxEntries
        true,   // enableConsole
        true,   // enableMetrics
    )
...
// Enforce policies
decision, err := pdp.Enforce(ctx, &agentauth.EnforcementRequest{
    Action:     "write",
...
```json
{
  ...
  "targets": [
    {
      "expr": "sum(rate(agentauth_aap001_pep_enforcements_total{allowed=\"true\"}[5m]))",
      "legendFormat": "Allowed"
    },
    {
      "expr": "sum(rate(agentauth_aap001_pep_enforcements_total{allowed=\"false\"}[5m]))",
      "legendFormat": "Denied"
    }
  ],
  "yaxes": [{"format": "reqps"}]
...
```json
{
  ...
  "targets": [
    {
      "expr": "sum(rate(agentauth_aap001_pep_violations_total[5m]) by (severity)",
...
```json
{
  ...
  "targets": [
    {
      "expr": "histogram_quantile(0.50, rate(agentauth_aap001_pep_enforcement_latency_seconds_bucket[5m]))",
      "legendFormat": "P50"
    },
    {
      "expr": "histogram_quantile(0.95, rate(agentauth_aap001_pep_enforcement_latency_seconds_bucket[5m]))",
      "legendFormat": "P95"
    },
    {
      "expr": "histogram_quantile(0.99, rate(agentauth_aap001_pep_enforcement_latency_seconds_bucket[5m]))",
      "legendFormat": "P99"
    }
  ],
  "yaxes": [{"format": "s"}]
...
```promql
# Check label values
count(agentauth_aap001_pep_enforcements_total) by (allowed, action_type)
count(agentauth_aap001_pep_violations_total) by (violation_type, severity)
```
...
- [AAP-001 Authorization Framework](../../docs/RFC_IMPLEMENTATION_COVERAGE.md)
