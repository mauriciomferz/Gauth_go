---
title: AgentAuth+ Grafana Dashboard Guide
category: guide
status: draft
...
4. Configure query:
   ...
   rate(agentauthplus_custom_metric[5m])
   ```
...
(rate(agentauthplus_cache_hits_total[5m]) + rate(agentauthplus_cache_misses_total[5m]))
...
# Success rate
rate(agentauthplus_validations_total{result="success"}[5m]) /
rate(agentauthplus_validations_total[5m])

# P95 validation duration
histogram_quantile(0.95, rate(agentauthplus_validation_duration_seconds_bucket[5m]))
```

### Policy & Violations
```promql
# Violations by severity
sum(rate(agentauthplus_policy_violations_total[5m]) by (severity)

# Critical violations only
rate(agentauthplus_policy_violations_total{severity="critical"}[5m])
```

### Operational
```promql
# Successor activation rate
rate(agentauthplus_successor_activations_total[5m])

# Average delegation depth
rate(agentauthplus_delegation_depth_sum[5m]) / 
rate(agentauthplus_delegation_depth_count[5m])

# Dual control rejection rate
rate(agentauthplus_dual_control_approvals_total{status="rejected"}[5m]) /
rate(agentauthplus_dual_control_approvals_total[5m])
```
...
3. Manually import dashboard:
   - Copy content of `agentauthplus-monitoring.json`
...
2. Check Prometheus is scraping:
   ...
   - Verify `agentauth-service` target is UP

3. Test query in Prometheus:
   - Navigate to **Graph** tab
   - Enter query: `agentauthplus_validations_total`
   - Click **Execute**
...
1. Verify metric labels match queries:
   ```bash
   curl -s http://localhost:8080/metrics | grep 'agentauthplus_cache_hits{cache_type'
   ```
...
3. **Use recording rules for expensive queries**:
   ...
   # prometheus.yml
   groups:
     - name: agentauthplus_recordings
       interval: 30s
       rules:
         - record: agentauthplus:cache_hit_rate:5m
           expr: rate(agentauthplus_cache_hits_total[5m]) / 
                 (rate(agentauthplus_cache_hits_total[5m]) + 
                  rate(agentauthplus_cache_misses_total[5m]))
   ```
...
curl http://localhost:8080/metrics | grep agentauthplus_ | head -10
```

**Expected output**:
```
# HELP agentauthplus_validations_total Total number of AgentAuth+ validations performed
# TYPE agentauthplus_validations_total counter
agentauthplus_validations_total{feature="successor",result="success"} 42
agentauthplus_validations_total{feature="delegation",result="success"} 128
...
```
...
```go
// Automatic timing in agentauthplus_integration.go
start := time.Now()
defer func() {
...
- **AgentAuth+ Implementation**: See `AGENTAUTH_PLUS_CACHING_IMPLEMENTATION.md`
...
### Key File Locations
- **Dashboard**: `deployments/docker/monitoring/grafana/dashboards/agentauthplus-monitoring.json`
