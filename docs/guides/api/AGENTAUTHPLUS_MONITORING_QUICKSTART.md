---
title: AgentAuth+ Monitoring Quickstart
category: guide
status: draft
...
```bash
curl http://localhost:8080/metrics | grep agentauthplus | head -20
```
...
agentauthplus_validations_total{feature="successor",result="success"} 0
agentauthplus_validations_total{feature="delegation",result="success"} 0
agentauthplus_cache_hits_total{cache_type="capability"} 0
agentauthplus_cache_misses_total{cache_type="capability"} 0
...
```
...
```bash
curl http://localhost:9090/api/v1/targets | jq '.data.activeTargets[] | select(.labels.job=="agentauth-service")'
```

**Or visit**: http://localhost:9090/targets

Should show `agentauth-service` with status **UP**.
...
- Filter: `component=agentauthplus`
...
docker compose logs -f agentauth
```
...
- Look for errors in `agentauth-service` target
...
```bash
curl http://localhost:9090/api/v1/rules | jq '.data.groups[] | select(.name=="agentauthplus_alerts")'
```
...
See `AGENTAUTH_PLUS_GRAFANA_DASHBOARD_GUIDE.md` for:
...
- **Complete Guide**: `AGENTAUTH_PLUS_GRAFANA_DASHBOARD_GUIDE.md` (700+ lines)
