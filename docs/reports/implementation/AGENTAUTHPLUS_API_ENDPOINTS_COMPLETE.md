---
title: AgentAuth+ Api Endpoints Complete
category: guide
status: draft
...
**Status**: ✅ COMPLETE  
**Version**: 1.0  
**Base Path**: `/api/v1/agentauthplus`
...
| File | Lines | Purpose |
|------|-------|---------|
| `web/agentauthplus_routes.go` | 67 | Route registration for all AgentAuth+ endpoints |
| `web/handlers/agentauthplus/successor_handlers.go` | 167 | Successor management HTTP handlers |
| `web/handlers/agentauthplus/delegation_handlers.go` | 208 | Delegation service HTTP handlers |
| `web/handlers/agentauthplus/dual_control_handlers.go` | 232 | Dual control approval HTTP handlers |
| `web/handlers/agentauthplus/capability_handlers.go` | 175 | Capability assessment HTTP handlers |
| `web/handlers/agentauthplus/fiduciary_handlers.go` | 148 | Fiduciary duty HTTP handlers |
...
| File | Changes |
|------|---------|
| `web/aap001_init.go` | Added global service storage and `InitializeAgentAuthPlusEndpoints()` method |
...
#### POST `/api/v1/agentauthplus/capabilities/certify`
...
#### POST `/api/v1/agentauthplus/capabilities/certifications/:id/revoke`
...
AGENTAUTH_AGENTAUTH_PLUS_ENFORCE=0  # 0=advisory, 1=strict
AGENTAUTH_AGENTAUTH_PLUS_ENFORCE_CAPABILITIES=0
AGENTAUTH_AGENTAUTH_PLUS_ENFORCE_DUAL_CONTROL=0
AGENTAUTH_AGENTAUTH_PLUS_ENFORCE_FIDUCIARY=0
```

### Server Startup

```bash
$ AGENTAUTH_AGENTAUTH_PLUS_ENABLED=1 go run ./cmd/web-server
...
// After AAP-001 initialization with AGENTAUTH_AGENTAUTH_PLUS_ENABLED=1
server.InitializeAgentAuthPlusEndpoints()

// Endpoints are now available at /api/v1/agentauthplus/*
```
...
```bash
curl http://localhost:8080/api/v1/agentauthplus/delegations/chain/agent-002
```
...
```bash
$ go build ./web/handlers/agentauthplus/...
# Success
