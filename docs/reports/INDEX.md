# Reports Index

This folder contains the project’s canonical reports grouped by area.

## Security

- Folder: [docs/reports/security/](security/)
- Key docs:
  - [SECURITY.md](security/SECURITY.md)
  - [SECURITY_HARDENING.md](security/SECURITY_HARDENING.md)
  - [SECURITY_COMPLIANCE_GUIDE.md](security/SECURITY_COMPLIANCE_GUIDE.md)
  - [SECURITY_PRODUCTION_CHECKLIST.md](security/SECURITY_PRODUCTION_CHECKLIST.md)
  - [SECURITY_AUDIT_RESPONSE_SUMMARY.md](security/SECURITY_AUDIT_RESPONSE_SUMMARY.md)

## Compliance

- Folder: [docs/reports/compliance/](compliance/)
- Key docs:
  - [GAP_CLOSURE_REPORT.md](compliance/GAP_CLOSURE_REPORT.md)
  - [GAP_CLOSURE_SUMMARY.md](compliance/GAP_CLOSURE_SUMMARY.md)
  - [EXECUTIVE_SUMMARY_NOV_12_2025.md](compliance/EXECUTIVE_SUMMARY_NOV_12_2025.md)

## Implementation

- Folder: [docs/reports/implementation/](implementation/)
- Key docs:
  - [IMPLEMENTATION_STATUS.md](implementation/IMPLEMENTATION_STATUS.md)
  - [FINAL_IMPLEMENTATION_SUMMARY.md](implementation/FINAL_IMPLEMENTATION_SUMMARY.md)
  - [FINAL_COMPLETION_REPORT.md](implementation/FINAL_COMPLETION_REPORT.md)
  - [SECURITY_IMPLEMENTATION_COMPLETE.md](implementation/SECURITY_IMPLEMENTATION_COMPLETE.md)

## Phase Completion

- Folder: [docs/reports/phase-completion/](phase-completion/)
- Key docs:
  - [PHASE_2_COMPLETE_SUMMARY.md](phase-completion/PHASE_2_COMPLETE_SUMMARY.md)
  - [PHASE3_LOAD_TEST_REPORT.md](phase-completion/PHASE3_LOAD_TEST_REPORT.md)
  - [GAP_G10_PHASE7_PERFORMANCE_REPORT.md](phase-completion/GAP_G10_PHASE7_PERFORMANCE_REPORT.md)

## Testing

- Folder: [docs/reports/testing/](testing/)
- Key docs:
  - [TESTING_GUIDE.md](testing/TESTING_GUIDE.md)
  - [TESTING_SUMMARY.md](testing/TESTING_SUMMARY.md)
  - [GAP_G10_TESTING_GUIDE.md](testing/GAP_G10_TESTING_GUIDE.md)
  - [OIDC_TESTING_GUIDE.md](testing/OIDC_TESTING_GUIDE.md)
  - [PERFORMANCE_TEST_REPORT.md](testing/PERFORMANCE_TEST_REPORT.md)

## Legacy root (stubs)

These documents used to live at repo root and are now archived under `docs/reports/legacy-root/`.
The repo-root files are kept as stubs to preserve legacy links.

- [AGENTAUTH_PLUS_FINAL_STATUS.md](legacy-root/AGENTAUTH_PLUS_FINAL_STATUS.md)
- [AGENTAUTH_PLUS_PROJECT_STATUS.md](legacy-root/AGENTAUTH_PLUS_PROJECT_STATUS.md)
- [AGENTAUTH_PLUS_README.md](legacy-root/AGENTAUTH_PLUS_README.md)
- [API_REFERENCE.md](legacy-root/API_REFERENCE.md)
- [COMMIT_GUIDE.md](legacy-root/COMMIT_GUIDE.md)
- [DEPLOYMENT_GUIDE.md](legacy-root/DEPLOYMENT_GUIDE.md)
- [DIRECTORY_REORGANIZATION.md](legacy-root/DIRECTORY_REORGANIZATION.md)
- [FINAL_HANDOFF.md](legacy-root/FINAL_HANDOFF.md)
- [IMPLEMENTATION_COMPLETE.md](legacy-root/IMPLEMENTATION_COMPLETE.md)
- [POLICY_STORE_IMPLEMENTATION.md](legacy-root/POLICY_STORE_IMPLEMENTATION.md)
- [SIGNATURE_VALIDATION_IMPLEMENTATION.md](legacy-root/SIGNATURE_VALIDATION_IMPLEMENTATION.md)
