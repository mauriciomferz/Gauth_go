...
- `agentauth_notarization_failures_total{kind,provider}`
- `agentauth_multisig_completion_ratio` (gauge)
- `agentauth_key_rotation_total` / `agentauth_key_rotation_failures_total`
- `agentauth_token_public_verify_failures_total`
- `agentauth_obligation_execution_failures_total`
- `agentauth_poacondition_evaluation_latency_seconds` (histogram)
- `agentauth_replay_wal_recovery_total`
- `agentauth_envelope_issuance_cadence_seconds` (histogram) – interval between consecutive issuance events
- `agentauth_envelope_v1_sunset_phase` (gauge enum) – 0 Pilot,1 Broad,2 Stabilization,3 SoftDep,4 Sunset,5 PostVerify
- Future: labeled digest mismatch counter `agentauth_envelope_digest_mismatch_total{reason}` (canonicalization_error|tamper_suspected|domain_conflict)
...
## Follow-Up Tracking
Create GitHub issues per epic with labels: `aap001`, `aap002`, `integrity`, `sunset`, `priority:P0/P1`.
