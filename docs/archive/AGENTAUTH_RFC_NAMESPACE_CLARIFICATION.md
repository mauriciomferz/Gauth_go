...
- **IETF AAP-001**: "Standard Host Names" (1971) - Network Control Protocol
- **IETF AAP-002**: "Some Network Information Center Clerks Should Be Told About Network Procedures" (1971)
...
|--------------|--------------|---------|
| AAP-001      | **AAP-001** | Active |
| AAP-002      | **AAP-002** | Active |
...
- **Package**: `github.com/mauriciomferz/AgentAuth/pkg/agentauth_rfc_001`
...
- **Package**: `github.com/mauriciomferz/AgentAuth/pkg/agentauth_rfc_002` (planned)
...
✅ **AAP-001/002**: Internal AgentAuth standards (this framework)  
❌ **IETF AAP-001/115**: Historic IETF network protocols (not related)

### Verification Questions

**Q**: "Does this system implement IETF AgentAuth-RFC-002 (formerly AAP-002)?"  
...
**A**: Review the specification at `docs/specifications/AgentAuth-RFC-001.md` and compare against the implementation in `pkg/agentauth_rfc_001/`. The test suite provides comprehensive compliance verification.

**Q**: "What is the relationship to IETF standards?"  
**A**: AgentAuth uses standard IETF protocols (JWT/RFC 7519, Ed25519/RFC 8032, WebAuthn/W3C) but AgentAuth-RFC-001/002 are framework-specific extensions. There is no connection to IETF AgentAuth-RFC-001 (formerly AAP-001)/115.
...
- Avoid: "AgentAuth-RFC-001 (formerly AAP-001) compliant" (ambiguous)
...
❌ **Incorrect**: "This module implements AgentAuth-RFC-001 (formerly AAP-001) authentication."
...
3. **Documentation Updates**:
   ...
   - "AAP-001" (legacy) → "AAP-001" (AgentAuth Identity Profile)
   - "AAP-002" (legacy) → "AAP-002" (AgentAuth Delegation Profile)
...
- **Technical Questions**: `dev@agentauth.io`
- **Compliance Inquiries**: `compliance@agentauth.io`
- **Security Disclosures**: `security@agentauth.io` (PGP: [link])
...
**For Auditors**: If you are conducting a compliance audit and need verification that this system meets specific requirements, please reference the **AgentAuth-RFC-001 specification** rather than searching for "AgentAuth-RFC-001 (formerly AAP-001)" (which will return unrelated IETF network protocols).
