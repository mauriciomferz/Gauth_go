...
Abstract
The GAuth authorization framework enables an artiﬁcial intelligence (AI, e.g., a digital
...
for Comment). Deﬁned exclusions of GAuth must refer to separate license conditions.
...
1. Scope
GAuth concerns the technical ﬁeld of AI and particularly the governance of AI. With the
...
These exclusions are excluded from the GAuth open source standard. Users Must Not
...
The use of Gimel ID, speciﬁcally, provides a next level of assurance for identity
...
3. Nomenclature
The following paragraphs explain deﬁnitions for technical terms used by the GAuth
...
the deﬁnitions. Accordingly, GAuth also includes other possible implementations than
the variants mentioned in the following paragraphs.
As the GAuth protocol builds on the OAuth protocol, GAuth builds on the role deﬁnitions
...
In addition to this nomenclature, GAuth uses following speciﬁc roles:
...
GAuth integrates the speciﬁc aspects of comprehensively authorizing an AI, i.e., it takes
...
The following description sets out the abstract GAuth protocol ﬂow. This description is
not intended to limit the scope of the GAuth standard, i.e., it also encompasses
combinations and modiﬁcations of the abstract ﬂow described herein. The GAuth
...
Page 11
September 2025Figure 2: Abstract GAuth protocol ﬂow
...
7. Beneﬁts
GAuth provides several beneﬁts, which can be summarized by following adjectives:
...
│  │ PRACTICAL                                                             │ │
│  │                                                                       │ │
│  │ GAuth offers several key advantages over the current state of the     │ │
...
│  │ COMPREHENSIVE                                                         │ │
│  │                                                                       │ │
│  │ GAuth addresses the limitations of current authorization protocols    │ │
...
│  │ mechanisms, GAuth creates a comprehensive basis for authorizing and   │ │
...
│  │ VERIFIABLE                                                            │ │
│  │                                                                       │ │
│  │ GAuth ensures a high degree of transparency towards relying parties,  │ │
...
│  │ AUTOMATED                                                             │ │
│  │                                                                       │ │
│  │ Another significant advantage of GAuth is that the protocol, more     │ │
...
│  │ rules (not limited to it, in line with GAuth's exclusions). This      │ │
...
│  │ COMPOUNDING                                                           │ │
│  │                                                                       │ │
│  │ GAuth builds on current standards like OAuth and OpenID Connect, so   │ │
...
│  │ UPGRADABLE                                                            │ │
│  │                                                                       │ │
│  │ The - within this specification - out-scoped features of GAuth        │ │
...
│                                                                            │
└────────────────────────────────────────────────────────────────────────────┘
Veriﬁable: GAuth ensures a high degree of transparency towards relying parties, and an
...
Automated: Another signiﬁcant advantage of GAuth is that the protocol, more
...
with GAuth’s exclusions). This leads to signiﬁcantly higher e]iciency in the approval of
...
Upgradable: The - within this speciﬁcation - out-scoped features of GAuth (exclusions)
