...
- Added optional BoltDB persistence for PoA records via `AGENTAUTH_AI_DEMO_POA_DB_PATH` (fallback to in-memory map when unset). Includes secondary index bucket for principal queries (future expansion).
...
- Existing SQLite `decisions` tables automatically migrated: code issues `ALTER TABLE` for missing `poa_id` / `poa_digest` columns during startup (legacy manual migration guidance retained for reference). Recommended manual verification:
	...
	```
- Extended token issuance depends on `AGENTAUTH_AI_DEMO_JWT_SECRET`. Without it PoA issuance still functional but token endpoint returns error (`jwt_secret_not_configured`). Integrity claims (`poa_digest`, `poa_version`) only present when token issued.
...
### Demo Feature Expansion
 ...
 - Added optional decision log retention pruning via `AGENTAUTH_AI_DEMO_DB_MAX_ROWS` (keeps most recent rows, deletes oldest beyond limit after each insert).
 - Added age-based decision pruning (`AGENTAUTH_AI_DEMO_DB_MAX_AGE_DAYS`) and statistics endpoint `/demo/decisions/stats` (total, oldest/newest timestamps, top actions). Introduced prune operations counter `ai_demo_prune_operations_total`.
...
- RS256 JWT verification via JWKS (env: `AGENTAUTH_AI_DEMO_JWKS_URL`, `AGENTAUTH_AI_DEMO_JWKS_CACHE_SECONDS`, optional `AGENTAUTH_AI_DEMO_JWT_EXPECT_ALG`) with new error codes (`unsupported_alg`, `jwks_fetch_error`, `kid_not_found`, `rsa_verification_failed`). Added JWKS metrics: `ai_demo_jwks_fetch_total{result}` and `ai_demo_jwks_keys_loaded` gauge for key cache size.

- JWKS resilience improvements:
	- Background proactive refresh (env: `AGENTAUTH_AI_DEMO_JWKS_BG_REFRESH_FACTOR`, default 0.5) to refetch keys before cache expiry, smoothing latency and reducing cold path fetch races.
	- Negative KID caching (env: `AGENTAUTH_AI_DEMO_JWKS_NEGATIVE_TTL_SECONDS`) to store missing `kid` lookups for a short TTL, suppressing redundant JWKS fetch attempts for invalid or noisy tokens.
	...
	- Introduced `AGENTAUTH_AI_DEMO_JWKS_NEGATIVE_MAX_ENTRIES` to cap negative cache size with oldest-entry eviction to prevent unbounded memory growth under malicious random kid sprays.
		...
		- Added optional jitter control `AGENTAUTH_AI_DEMO_JWKS_BG_REFRESH_JITTER_SECONDS` to de-synchronize background JWKS refresh across horizontally scaled replicas, reducing coordinated thundering herd after deploy or TTL boundary.
...
- Strict authenticity enabled by default (missing signature public key -> integrity failure). Override only via `AGENTAUTH_STRICT_AUTHENTICITY=0` for transitional migration.
- Mandatory `jti` claim enforced unless `AGENTAUTH_ALLOW_MISSING_JTI=1` set, closing trivial replay gaps without a replay store.
...
- Adjusted rotation & strict authenticity tests to reflect default strict mode (`aap001_rotation_test.go`, `aap001_strict_auth_test.go`).

### Documentation
- New compliance matrix: `docs/aap001_compliance_matrix.md` summarizing Implemented / Partial / Missing clauses & gap roadmap.
...
- Multi-sig PoAs previously relying on `AGENTAUTH_MULTI_SIG_WEIGHTS` will produce different digests when re-issued with embedded weights; re-issue recommended.
