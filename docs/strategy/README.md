# AgentAuth Strategic & Commercialization Pack

This directory contains the strategic assets for the commercialization and distribution of the AgentAuth framework.

## 📄 Core Assets

| File | Description | Target Audience |
|------|-------------|-----------------|
| **[STRATEGIC_ADVISORY_DECEMBER_2025.md](./STRATEGIC_ADVISORY_DECEMBER_2025.md)** | **START HERE.** Executive summary of IP strategy, monetization models, and legal risks. | Internal / Founders |
| **[BOOK_MANUSCRIPT.md](./BOOK_MANUSCRIPT.md)** | Final 10-chapter manuscript: *"The Agent's Signature: Identity & Law in the Age of AI"*. | Public / Publishers |
| **[COMMERCIAL_PROPOSAL_GIMEL.md](./COMMERCIAL_PROPOSAL_GIMEL.md)** | B2B proposal for spinning out AgentAuth as a vendor to Gimel/Siemens. | Dr. Götz G. Wehberg |
| **[STRATEGIC_WHITEPAPER.md](./STRATEGIC_WHITEPAPER.md)** | Technical/Business whitepaper for investors and public positioning. | Investors / Partners |

## 📚 Publishing Assets

| File | Description | Usage |
|------|-------------|-------|
| **[AMAZON_METADATA.md](./AMAZON_METADATA.md)** | SEO-optimized title, description, and keywords for Amazon KDP. | KDP Dashboard |
| **[BOOK_MANUSCRIPT_KINDLE.md](./BOOK_MANUSCRIPT_KINDLE.md)** | Manuscript formatted specifically for Kindle conversion (page breaks, front matter). | KDP Upload |

## 🛡️ Handoff & Integrity

For the technical handoff and cryptographic verification of the codebase, see:
- **[../../FINAL_HANDOFF.md](../../FINAL_HANDOFF.md)**
